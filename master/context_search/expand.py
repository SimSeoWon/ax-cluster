...
1. 🔴 **짧은 별칭은 확장하지 않는다** (`MIN_TERM`). 「적」 같은 한 글자는 어디에나 들어간다
2. 🔴 **개수 상한** (`MAX_EXPAND`). 질의 하나가 식별자 수십 개로 불어나면 BM25 가 원래
...
class Expansion:
    ...
    @property
    def note(self) -> str:
        """🔴 무엇을 왜 붙였는지 — 조용히 바꾸면 사람이 엉뚱한 곳을 의심한다."""
        if not self.added:
            return ""
...
def expand(query: str, aliases: dict, *, min_term: int = MIN_TERM,
    ...
    🔴 **원본을 지우지 않는다.** 덧붙이기만 한다 — 확장이 틀렸을 때 원래 질의가 남아 있어야
    결과가 0건으로 떨어지지 않는다.
    """
    q = (query or "").strip()
