...
##### ② 그래서 이식 범위는 — 검색 코어 **와** 온톨로지 **둘 다**

"소스 한정" 은 온톨로지를 빼자는 말이 **아니다.** 오히려 반대다:
**이 온톨로지가 곧 소스의 트윈**이므로 트윈을 하겠다면 온톨로지가 핵심이다.

실측 규모 (`mcp/context_search/` 40파일 14,401줄, 2026-08-08):

| 기능군 | 파일 | 줄수 | 이식 |
|---|---|---|---|
| **검색 코어** (`search_*`·`bm25_index`·`double_buffered_index`·`client_index`·`cs_common`·`http_server`) | 10 | **3,552** | ✅ 1단계 |
| **온톨로지** (`ontology_*`) | 15 | **4,832** | ✅ 2단계 |
| **도메인** (`domain_*`) | 3 | 1,750 | ✅ 2단계 (온톨로지 조회 경로) |
| 감사(`audit_*`·`context_audit`) | 3 | 1,416 | ⬜ §5.2-D 로 별도 판단 |
| 위키 | 2 | 198 | ⬜ 팀 기능 후보 |
| 웹 UI (`web_ui`·`llm_chat`) | 2 | 892 | ⬜ 마스터엔 화면이 없다 |
| 기타 (`class_graph_query` 등) | 5 | 1,761 | ✅ 부분 |

여기에 `watcher/class_graph*.py` **4파일 1,592줄**(파서·DB·온톨로지 연계)이 붙는다 —
색인을 *만드는* 쪽이라 `mcp/` 밖에 있다.

⚠️ **감사·위키·웹UI 를 빼는 것은 "소스 한정" 때문이 아니다.** 그쪽은 §5.2-D(팀 전용 컴포넌트
선별) 기준이고 **일괄 폐기 금지·개별 판단** 원칙이 그대로 적용된다. 두 축을 섞지 말 것.
