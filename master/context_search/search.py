...
[중요] **뺀 것들은 전부 "얹는" 기능이다.** 원본에서도 추론 실패 시 무필터로 폴백하므로
...
# [중요] **대괄호 = "이게 중요한 토큰이다" 는 사용자와의 규약** (사용자 제안 2026-08-08).
#
...
#
# [중요] **규약: 대괄호는 클래스에만 쓴다** (사용자 확정 2026-08-08). 상태·개념(`[완료]`·`[연출]`)에
...
# 걷어낸다. [중요] 이 개선은 **추측이 아니라 사용자가 준 신호**라 정답 집합 없이도 안전하다 —
...
def fuse(vector_hits: list[dict], bm25_hits: list[tuple[str, float]]) -> list[Hit]:
    ...
    [중요] 점수가 아니라 **순위**를 쓴다. 코사인 유사도와 BM25 점수는 척도가 달라 직접 더할 수
...
class ContextSearch:
    ...
    def __init__(self, paths: ProjectPaths):
        ...
        # [중요] 벡터가 해석한 세대를 BM25 에 넘긴다. 각자 마커를 읽게 두면 그 사이 재색인이
    ...
    def search(self, query: str, limit: int = 8, *, pool: int | None = None,
        ...
        if not query.strip():
            return []
        # [중요] 시소러스 확장 (소 2.2.1) — 한글 말을 실제 식별자로 잇는다. 원본은 지우지 않고
