...
## 🔴 무엇이 `runner.py` 와 다른가 — 쓰는 주체가 하나다

    runner.py    (구 토폴로지) 워커가 고치고 커밋하고 attempt push → 마커로 판정
    infer.py     (현 토폴로지) 🔴 워커는 **추론만** → 응답을 모아 **통합자에게 순서대로 넘긴다**

사용자 확정 2026-08-11. `runner.py` 를 **지우지 않는다** — N-writer 기계장치는 측정으로
뒤집힐 여지가 있는 동안 되돌릴 수 있게 둔다(§8.4 ⚠️). 워커에는 스킬이 둘 배달돼 있고
(`ax-work`·`ax-infer`), **파견 지시가 이름으로 고른다.**

## 🔴 응답은 stdout 이 아니라 **파일로** 받는다
...
# 🔴 ASCII 만. 한글을 명령줄에 실으면 워커 콘솔에서 깨진다(`runner` 의 § 참조).
...
# 🔴 응답 **파일 끝**에 붙은 마커 — 실전 첫 구동에서 나왔다(2026-08-12). 아래 § 참조.
...
class DispatchError(RuntimeError):
    """파견 자체가 불가능하다 — 🔴 태스크를 집은 채로 죽지 않게 호출자가 반납한다."""


# ── 대상 파일 — 🔴 마스터가 **말한 것**만 받는다 ────────────────────────────────

def target_files_from(manifest_text: str) -> list:
    """매니페스트의 「대상 파일」 절을 되읽는다. **LLM 0 · 결정적.**

    🔴 **큐 레코드가 아니라 매니페스트에서 읽는 이유**: 워커가 본 것이 이것이다. 큐의
...
@dataclass
class Response:
    """워커 한 번의 **추론 결과**. 🔴 모르는 값을 성공으로 접지 않는다."""
    ...
    notes: list = field(default_factory=list)      # 🔴 로 시작하는 것은 치명적이다
    reason: str = ""
    ...
    @property
    def fatal(self) -> list:
        """🔴 로 표시된 메모. `parse_files` 가 이미 그 규약으로 쓴다."""
        return [n for n in self.notes if n.startswith("🔴")]

    @property
    def ok(self) -> bool:
        """🔴 **fail-closed.** 마커·파일·본문 셋이 다 맞아야 통합자에게 넘긴다."""
...
def read_response(facts, task_id: str, *, reader=None) -> str:
    ...
    🔴 셸 경유(`type`·`Get-Content`)를 쓰지 않는다 — `_remote_read` 의 주석에 그 사고가
...
def strip_trailing_markers(raw: str) -> tuple:
    ...
    🔴 **실전 첫 구동에서 나온 결함이다** (2026-08-12). 지시문이 *"the last two non-empty lines
    ...
    **한 번 일어난 실수는 다시 일어난다.** 다만 ⚠️ 로 **말하고** 뗀다: 조용히 지우면 지시를
    안 지킨 사실이 사라지고, 통째로 버리면 옳게 만든 본문까지 버려 같은 추론을 다시 산다.

    ⚠️ **맨 끝의 연속된 마커 줄만** 본다. 파일 중간의 같은 글자는 건드리지 않는다.
    """
...
def parse_markers(stdout: str) -> tuple:
    """`(상태, 사유, 꼬리, 계측)`. 🔴 **마지막 줄이 `RESULT:` 여야 한다 — 없으면 BLOCKED.**
    ...
    **요구**하고, 없으면 *"제출할 근거가 없다"* 를 사유로 적는다. 추론 파견에는 브랜치도
    ...
    # 🔴 비대칭은 의도다 — 사유가 붙은 `BLOCKED — …` 는 받고 `DONE — …` 은 받지 않는다.
...
def judge(stdout: str, raw_response: str, *, want: list) -> Response:
    ...
    🔴 **두 근거를 다 본다.** 마커만 보면 워커가 `DONE` 이라 말하고 파일을 안 쓴 경우를
    ...
    try:
        ...
        return res
    # 🔴 파일 끝 개행을 하나로 맞춘다 (실측 2026-08-12). `parse_files` 는 블록마다 `strip()` 을
    ...
    #    → 개행 하나가 지배적(69%)이고 git 관례이기도 하다. ⚠️ 나머지 50개는 EOF 가 한 줄
    #      바뀐다 — **완전 보존은 불가능**하고(strip 이 정보를 지운다), 이쪽이 잡음이 가장 적다.
    files = {k: (v.rstrip("\n") + "\n") for k, v in files.items()}
    res.files, res.notes = files, list(notes)
    if stripped:
        # ⚠️ 지시를 안 지킨 사실을 남긴다 — 통과시키되 말한다
        res.notes.append(f"⚠️ 응답 파일 끝의 판정 마커 {stripped}줄을 뗐다 — "
                         f"마커는 stdout 에만 낸다(파일에 넣으면 본문에 섞인다)")

    # 🔴 마커가 신고한 개수와 실제가 다르면 **말해 준다** (조용히 넘기지 않는다).
...
def infer_task(facts, task_id: str, manifest_text: str, *, want=None, beat=None, runner_=None,
    ...
    🔴 **큐를 만지지 않고, 응답을 적용하지도 않는다.** 이 함수는 *워커가 무엇을 답했는가* 만
    ...
    try:
        ...
        # 🔴 "못 읽었다" 를 "없다" 로 접지 않는다 — 그게 `_remote_read` 가 배운 교훈이다
        raw = ""
    ...
    if rc != 0 and res.status == runner.DONE:
        # 🔴 워커는 DONE 이라는데 껍데기가 실패했다 — 성공으로 접지 않는다
        res.status = runner.BLOCKED
...
# ── 스풀 — 🔴 **돈을 낸 출력이고, 진행 상황의 유일한 사본이다** (소 1.3.2·1.3.3) ──────
...
def spool(paths, res: Response, *, work_id: str = "", order: int = 0) -> str:
    ...
    🔴 **실패도 쓴다.** 무엇을 시도했고 어디서 막혔는지가 사라지면 피드백을 줄 근거가 없다
    ...
    tmp.replace(p)                      # 🔴 통합자가 반쯤 쓰인 응답을 읽으면 안 된다
    return str(p)
...
@dataclass
class Handoff:
    """통합자에게 넘기는 것. 🔴 **적용 순서대로**이고, 통과분만 들어간다.
    ...
    def ordered(self) -> list:
        """🔴 **통과분만**, 적용 순서대로. 실패는 통합자에게 넘기지 않는다."""
    ...
    def summary(self) -> str:
        ...
        for tid, why in self.skipped:
            out.append(f"  [–] {tid}: 🔴 파견하지 않았다 — {why}")
        if ok:
            out.append("")
            out.append("적용 순서 (통합자가 이 순서로 순차 적용한다):")
            for n, r in enumerate(ok, 1):
                out.append(f"  {n}. {r.task_id} — " + ", ".join(sorted(r.files)))
        # ⚠️ 없는 것을 있는 척하지 않는다 — 중 1.4 가 아직 없다는 사실이 결과에 실린다
        out.append("")
        out.append("⚠️ **적용은 아직 아무도 하지 않는다** — 통합자(중 1.4)가 비어 있다. "
                   "응답은 스풀에 남았고, 태스크는 `claimed` 로 남아 리스(1200초) 만료 뒤 "
                   "재확보된다. 🔴 그때도 같은 추론을 다시 사지는 않는다(스풀 재사용).")
        if self.note:
...
def _free_workers(facts, *, clean=None) -> tuple:
    ...
    전부에 적용해 목록으로 받는다. 🔴 판정 규칙은 하나뿐이어야 하니 `pick_worker` 를 재사용한다.
    """
    chosen: list = []
    rejected: list = []
    pool = list(facts)
...
def dispatch_task(paths, facts, task, *, api=runner._api, work_id: str = "", order: int = 0,
    ...
    🔴 **큐에 제출하지 않는다** (§ 모듈 머리말). 대신 하트비트로 리스를 살려 두고, 응답을
    ...
    try:
        ...
        # ⚠️ 기준 커밋을 모르면 **재사용 판정을 못 한다** — 그래도 파견은 막지 않는다.
...
def run_many(paths, *, limit: int = 4, facts=None, project: str = "", api=runner._api,
    ...
    🔴 **워커가 빌 때만 집는다.** 집어놓고 못 돌리는 것이 가장 나쁘다 — `run_once` 가 이미
    같은 규칙이다(리스가 1200초 붙잡히고 사람은 *"왜 아무것도 안 도나"* 를 본다).

    🔴 **병렬은 기본값이 아니다** (실측: 워커 2대 = 12% 빠르고 90% 비쌌다). `parallel=True` 는
    사람이 켜는 것이고, 조각이 클 때만 이득이다.

    🔴 **실패한 태스크의 후속(`depends_on`)은 파견하지 않는다.** 큐는 `failed` 를 의존 해소로
    ...
    **헛일이 확실하다.** 이번 실행에서 실패한 것을 마스터가 기억해 막는다.
    ...
    if not workers:
        hand.note = ("🔴 파견할 워커가 없어 claim 조차 하지 않았다 — "
                     + ("; ".join(f"{h}: {r}" for h, r in rejected) or "후보 자체가 없다"))
        return hand
    if not parallel:
        workers = workers[:1]
        hand.note = ("⚠️ 직렬로 돌렸다 (기본값) — 워커 2대는 실측에서 12% 빠르고 90% 비쌌다. "
...
#   python -m master.work.infer run-p  [프로젝트] [N]     🔴 병렬 — 사람이 켠다
#   python -m master.work.infer spool  [프로젝트] [work]  스풀에 무엇이 있나
#
# 🔴 데몬으로 만들지 않는다(§5.5.4). 상주가 필요하면 systemd timer 로 `run` 을 부른다.
...
def _cli_spool(paths, work_id: str) -> int:
    ...
    for p in recs:
        ...
        mark = "✅" if rec.get("status") == runner.DONE and n else "🔴"
...
def main(argv) -> int:
    ...
    if cmd == "probe":
        ...
        for f in facts:
            print("  " + f.summary)
        print(f"추론 가능: {', '.join(f.host for f in workers) or '🔴 없다'}")
        for h, r in rejected:
            print(f"  제외 {h}: {r}")
        return 0
    if cmd in ("run", "run-p"):
