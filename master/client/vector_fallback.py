...
## 🔴 원전과 다르게 둔 것 — 임베더는 **우리 것**을 쓴다

원전은 Chroma **기본 임베딩 함수**(내장 `ONNXMiniLM_L6_V2` = `all-MiniLM-L6-v2`)를 쓴다.
🔴 **우리 `context_search/embedding.py` 가 이미 그 모델을 기각해 뒀고 근거도 적혀 있다**:
...
저장 증가 0)을 쓴다. ⚠️ 이것은 정합 작업(`#194`)이 아니라 **이식이 성립하기 위한 조건**이다 —
σ.9.0 의 `mtime` 을 git 시각으로 바꾼 것과 같은 부류다.

⚠️ **마스터 색인과 유사도를 직접 비교하지 말 것.** 같은 모델이라 결과가 일관되긴 하지만, 이
...
class VectorFallback:
    ...
    def upsert(self, results: list) -> dict:
        ...
        for r in results or []:
            ...
            cands.append({
                ...
                "meta": {
                    ...
                    # ⚠️ Chroma 메타데이터는 스칼라만 받는다 — 목록·매핑을 문자열로 접는다
        ...
        try:
            ...
            # 🔴 비교를 못 하면 **전부 적재**한다 — 원전과 같은 방향(폴백이 비는 쪽이 더 나쁘다)
    ...
    def search(self, query: str, *, limit: int = 5, category: str = "") -> list:
        """로컬 사본에서 의미 검색. 🔴 결과에 `source` 를 박아 **폴백임을 표시한다.**
        ...
        **가진 것 중에서** 가장 가까운 것이다.
        ...
        for i, fid in enumerate(ids):
            ...
            out.append({
                "file": fid,
                # 원전과 같은 변환. 🔴 음수가 나오지 않게 자른다(거리 > 1 인 경우가 있다)
