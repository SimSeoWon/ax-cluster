...
| `work/` | 워크플로우 — **골조·동결 → 분해 → 파견 → 검증**. 3-tier 브랜치(`#319` — base·조각 durable·attempt) · 매니페스트 · 도메인 규범(`norms.py`) · 헤더 선언(`declarations.py`). [중요] **「쓰는 주체는 하나」는 맞지만 그 하나는 통합자가 아니라 서버다** — 원전 `cluster_coordinator.py`(인용 0회)의 `verify_and_merge` 주석이 *"durable 단일 writer 보존"* 이고, 작업장은 **ephemeral `attempt/` 를 push** 한다. 리눅스가 강제하는 변경은 **UE5 빌드가 `.2` 로 가는 것 하나뿐**. [주의] **그 「작업장이 attempt 를 push 한다」는 원전 서술이고 우리 실물이 아니다** — `#317` 미결 ⑦ 이 **「검증 → 통과분만 커밋」을 정본으로 확정**했고(2026-08-28), 마스터측 attempt 배선은 `#327` 로 걷혔다. `coordinator.py` 의 원전 이식은 **`selftest` 드라이런이 쓰므로 남는다**. 지도는 `docs/4-work-loop.md` §4.5·§4.7, 권한은 `docs/8-git-authority.md` §8.4 |
...
| `work/infer.py` | [중요] **추론 파견** (중 1.3) — 워커는 **읽고 추론하고 텍스트로 답한다.** `python -m master.work.infer probe\|run\|run-p\|spool\|collect`. [중요] **큐에 제출하지 않는다** — 태스크는 `claimed` 로 남고 제출은 통합자(소 1.4.3)가 한다. 응답은 **파일로** 온다(`.ax/work/<task>/response.txt` → `scp`): `.2` 콘솔이 CP949 라 stdout 은 한국어에서 깨진다(층3 이 같은 자리에서 죽었다). 판정은 **마커 + 응답 파일 둘 다** — 마커만 보면 *"DONE 이라 말하고 파일 안 씀"* 을, 파일만 보면 *"반쯤 쓰이다 죽음"* 을 통과시킨다. [중요] 스풀(`<프로젝트>/responses/`)이 **복구 단위**다 — 같은 기준 커밋의 통과분은 **다시 사지 않는다**. [중요] 실패한 선행의 후속은 **파견 않고 반납**(큐는 `failed` 를 의존 해소로 센다). 기본은 **직렬**(실측: 2대 = 12% 빠르고 90% 비쌈). [완료] 실전: 단건 45초·$0.27 · **워커 2대 동시 36초·$0.4725·통과 2/2**(윈도우+BC-250, 채널 동일). [중요] 실측 **캐시 생성 고정비 워커당 24~26K 토큰** — N대면 N번 낸다. [중요] **pull 회수 = `collect`** (#204 2단계) — 데몬(claimer)이 남긴 result.json·response.txt 를 scp 로 읽어 **push 파견과 같은 judge(마커+층1)·같은 스풀**에 넣는다. blob 불일치 fail-closed · epoch 어긋남은 큐 fencing 에 위임 |
| `work/integrate.py` | [중요] **통합자** (중 1.4). [주의] *"유일한 쓰기 주체"* 라고 적어 뒀던 것은 정정됐다 — 원전에서 durable 을 쓰는 것은 **서버**이고(위 `work/` 행), 이 파일은 그 자리에서 **빌드 단계**를 맡는다. `python -m master.work.integrate plan\|run`. **조각마다** 층1(무료·결정적) → 적용 → **UE5 빌드** → [중요] 통과하면 커밋, 실패하면 **되돌리고 커밋하지 않는다**(소 1.4.4). [중요] §4.5 의 *"전부 적용 후 빌드 한 번"* 을 뒤집었다 — 증분 빌드 35~65초라 조각마다 세워도 싸고, **어느 조각이 깨졌는지 알 수 있다**. [중요] **첫 실패에서 멈춘다**(적용 순서 = 의존 순서). 격리 트리는 체크아웃의 **형제**(`ax-wt-<work>`), detached HEAD, 재사용 시 기준으로 자동 복원. push 는 **ff-only · force 금지** — 거부되면 커밋은 트리에 남고 사람이 판단한다. [중요] **durable 팁 위에 짓는다**(`durable_base`) — #185 의 매니페스트 커밋과 형제 분기가 되면 ff 가 구조적으로 거부된다(첫 실전 실측). 안전 조건: 팁이 기준의 자손 + 기준→팁 diff 가 대상 파일 무접촉. 동결 대조는 계속 추론 기준 커밋. 실측: 격리+빌드+커밋 **159초**(콜드 빌드 142초). [중요] **층2 를 빌드 앞에** 두고(비싼 것 앞에 싼 것) 선언부 grounding 을 싣는다 — 못 실으면 `l2_grounded=False` 로 보고. [중요] 층3 `RunTests` 는 **work 단위 한 번**(실측 74초 — 문서의 미측정 DDC 비용이 이 값) · [주의] **테스트 0개라 fail-open + 경고**. 실패는 큐 되돌리기 + `[FEEDBACK]` + **레드마인 등재** + **실패 카탈로그 적립** |
...
| `work/cleanup.py` | **찌꺼기 정리** — 워커의 로컬 브랜치·부산물. `python -m master.work.cleanup plan\|apply\|remote [--apply]`. [중요] **워커는 스스로 못 지운다**(스킬이 브랜치 삭제 금지 — 의도). 삭제는 **팁이 `origin/<base>` 에서 도달 가능할 때만**, 나머지는 **보존**(fail-closed 의 방향이 보존이다). [중요] **원격은 `remote` 서브커맨드가** 병합 확인분(attempt/task)만 지운다 (#219, 사용자 결정 2026-08-18 — 원전 cleanup_attempts 회귀 · pre-push 훅 #125 가 이중 가드). 미병합 원격은 계속 사람 몫. `failed` 부산물은 증거로 남긴다. [주의] `--format` 은 셸별로 감싼다 — `%(...)` 의 괄호가 POSIX 문법이라 안 감싸면 리눅스 워커만 조용히 실패한다 |
| ~~`work/claimer.py`~~ · ~~`work/ax_safety.py`~~ | **철거됨** (#320, 2026-08-27) — 상주 폴링 claimer 와 그 안전 가드. 파견은 마스터 SSH push 로 일원화(`layer3_verify.run_build_on_workshop`). 되살릴 조건은 `#320` 노트 |
| `work/buildjob.py` | **빌드 게이트 큐 잡** (#204) — `enqueue\|watch\|close`. 등록하고 사실을 보여줄 뿐 판단 없음. [중요] `close` 는 verify 후 **쓰고-되읽고-재시도** — verify 의 종결 되덮기가 동기 flip + 비동기 auto-cleanup **2회**라(실측) 먼저 쓰면 뒤집힌다. 단건 GET works 는 `{"work":{...}}` 포장(목록과 다름) |
| `work/coordinator.py` | [중요] **2단 브랜치 조율 — 원전 `cluster_coordinator.py`(200줄) 이식** (2026-08-14, 대 1). `assign_attempt`(서버가 브랜치명을 배정, **git 접촉 0**) · `push_attempt`(ephemeral `attempt/<id>/<workshop>/<ts>`, **force 허용** — durable 엔 절대 금지) · `verify_and_merge`([중요] **epoch 펜싱** — `submit_epoch < current_epoch` 면 좀비로 보고 병합 거부, **게이트는 notify 쪽이지 push 쪽이 아니다**) · `cleanup_attempts`([중요] **기본 `delete=False` — 세기만 한다**, 우리 08-09 결정인 증거 보존과 일치). [주의] **`run_round`(드라이런 합성)는 `#327` 로 걷혔다** — 부르는 곳이 테스트뿐이었고, `selftest` 는 *"real 모드는 합성을 쓰지 않는다"* 며 일부러 세 함수를 따로 불렀다. 그 합성은 이제 `test_coordinator` 안에 있다. [중요] **이 파일이 남는 이유는 그 드라이런이다** — 소비자 0 인 것만 걷는 것이 `#327` 의 절단면이다(사용자 결정). **일부러 다르게 한 것**: 원전의 `worker` → 우리는 `workshop`(이 저장소에서 `worker` 는 BC-250 추론 노드라 세 뜻이 겹친다) · [중요] **`remote` 가 인자다**(기본 `origin` = 원전 그대로). 마스터 정본은 `origin` push 가 봉인돼 있어 그 인자 하나가 환경 차이를 흡수한다 |
| `work/attempt.py` | [중요] **쓰기 표면 가드 + 원격 정리** (`#327` 이후). `install-hook`/`status` 가 `work/pre_push_hook.sh`(SSOT)를 정본 클론에 설치·대조하고, [중요] **git-lfs 훅과 공존한다**(실측 2026-08-28: `git lfs install` 이 덮어 NS 가드가 통째로 사라져 있었다 — stdin 을 버퍼에 담아 가드 먼저, 그다음 LFS 에 위임). `plan_remote_cleanup`/`apply_remote_cleanup`(소 1.2.4 — [중요] **도달 가능한 것만**, durable 은 보존·세기만). [주의] **마스터측 attempt 배선(`push_attempt_for`·`merge_verified`·`apply_responses`·`ensure_attempt_tree`)은 `#327` 로 걷혔다** — `#317` 미결 ⑦ 이 「검증 후 커밋」을 정본으로 확정했고, 그 배선은 **프로덕션 호출자 0 · bare 에 attempt 브랜치 0** 이었다. `python -m master.work.attempt status|install-hook|plan|apply` ([중요] **`plan` 이 기본**) |
...
| `work/carry.py` | [중요] **매니페스트 git-carried 배달** (#185 판정 — scp 의 전제 「마스터 push 불가」가 Flow Y 로 소멸). `publish` 가 조각 durable `task/<work_id>/<task_id>` 에 멱등 commit·push(기준은 **작업 브랜치 tip**, `#319`), 워커는 fetch+`git show` 로 자기 클론에서 실체화(경로·형식은 scp 시절과 동일 — 운송만 교체) + **blob sha 대조**. 조용한 scp 폴백 없음 — `runner.deliver_manifest` 는 부활 조건용 보존 |
...
| `ontology/log_tags.py` | **실증 테스트 로그 태그 사이드카** (#169, 원전 η.12.3) — `domains/<D>/log_categories.yaml` CRUD. [중요] 저작 방향 (b): 게임팀이 코드에 구현한 뒤 사람이 확정 기입, 자동 생성·편입 없음. 매치 없음 = `log_tags` 필드 **부재**(null 아님 — 실증 불가와 미확인의 구분). 뷰어(/ontology)가 invariant 에 자동 첨부. CLI `log-tags` · MCP 3종 |
| `ontology/log_candidates.py` | **태깅 권장 지점 리포트** (제안형, LLM 0) — invariant evidence 기반, `<트윈>/ontology/_log_tagging_candidates.md`. 확정 등록분은 버리지 않고 [확정 등록됨] 표기(원전은 파서 한계로 dedup 포기). [주의] 후보에 **레거시 모듈**(빌드 안 되는 Source/Project_Alpha)이 섞인다 — 실증 대상은 실 모듈에서 고를 것 (첫 실전 실측, 리포트 23 §1) |
| `../client/runtime/client_mcp.py` | [주의] **`master/` 밖으로 나갔다** (#263, 2026-08-22) — 클라 기계에서 도는 코드이므로 `client/` 소속이다. **배달 위치는 `axmaster/clientside/client_mcp.py` 그대로**(`.33` 의 `.mcp.json` args). **`.33` 로컬 stdio MCP** (stdlib 단일 파일, 번들로 배달) — 도구 8종: 조회 5(works/검색/도메인, `_stale` 캐시 폴백) + 마스터 대행 쓰기 3(redmine_note·log_writer_signal·log_history). 결정 도구는 없다 — 검수는 배달된 review 모듈이 유일 절차 |
...
- **추론 요청은 stateless**, **피드백 누적은 git**(`task/<work_id>/<task_id>` 브랜치). 두 경로 분리.
