...
`package.merge_preserve_locked` 가 그 표식을 보고 지킨다. [중요] **지금까지 그 보존 로직은
死코드였다** — 표식을 *거는 경로가 아예 없었기 때문이다(실측 2026-08-09). 이 모듈이 그 절반을
채운다.

## [중요] `name` 과 `layer` 는 패치로 바꾸지 않는다
...
# [중요] 패치로 바꿀 수 없는 것 — 파일 경로가 이 둘에서 나온다.
IMMUTABLE = ("name", "layer")


class EditError(RuntimeError):
    """편집할 수 없다. [중요] **조용히 새 파일을 만들지 않는다.**"""
...
class EditResult:
    ...
    ignored: list = field(default_factory=list)     # [중요] 불변이라 무시한 키
    locked: bool = False
...
def find_item(paths: ProjectPaths, domain: str, kind: str, name: str):
    """항목 파일. [중요] **전 계층을 훑는다** — 계층은 합성기가 옮길 수 있다."""
...
def _load(paths: ProjectPaths, domain: str, kind: str, name: str):
    ...
    if p is None:
        raise EditError(
            f"{domain}/{kind}/{name} 를 찾지 못했다. [중요] **새로 만들지 않는다** — 이름을 잘못 준 "
...
def edit(paths: ProjectPaths, domain: str, kind: str, name: str, patch: dict,
    ...
    [중요] 중첩 값은 **키 전체를 다시 넘겨야** 한다 — 부분 병합을 지원하면 무엇이 남고 무엇이
    ...
    for k, v in (patch or {}).items():
        if k in IMMUTABLE:
            res.ignored.append(k)           # [중요] 조용히 적용하지 않는다 — 보고한다
            continue
        if k == LOCK_FIELD:
    ...
    if not lock and res.was_locked:
        res.note = "[중요] 잠금을 풀었다 — 다음 재합성이 이 항목을 덮을 수 있다"
...
class RemoveResult:
    ...
    def summary(self) -> str:
        if self.removed:
            s = f" {self.domain}/{self.kind}/{self.name} 제거"
            if self.was_locked:
                s += " ([중요] 잠긴 것을 force 로 지웠다)"
            return s + ("" if self.manifest_updated else " · [주의] manifest 갱신 실패")
...
# ── 보호 표식 (일괄) ─────────────────────────────────────────────────────────
#
# [중요] **`verified_by_user` 와 다른 것이다.** 섞으면 둘 다 못 쓰게 된다:
#
...
# [중요] **언제 기준으로 보호했나.** 없으면 drift 감사(중 2.4.1)가 *"이 항목은 커밋 X 기준인데
...
def protect_all(paths: ProjectPaths, *, domain: str = "", kinds=(), reason: str = "",
    ...
    """항목들에 보호 표식을 일괄로 건다/푼다. [중요] **기본은 계획만**(`apply=False`).

    247건을 손으로 부를 수는 없다. 다만 일괄이라서 더 위험하므로 **먼저 무엇이 바뀌는지
    보여주고**, 실행은 명시할 때만 한다 — `sync`·`cleanup` 과 같은 규칙이다.

    [중요] `reason` 을 요구한다. *왜* 보호하는지 없으면 다음 사람이 풀어도 되는지 판단할 수 없다.
    """
