# worker

BC-250 추론 노드의 설정/래퍼가 들어갈 자리.

> [중요] **보드는 #1 한 대뿐이다** (2026-08-08). #2 는 Bazzite 게임 머신이다 — 아래 참조.
>
> [주의] 단 **추론 엔드포인트는 2개**다 — 나머지 하나는 보드가 아니라 **RTX 3060 윈도우 PC**
> (192.168.0.2, `14b` 상주)다. 이 디렉터리는 **보드 쪽** 설정만 다룬다.
> `35B` 는 3060 에 안 들어가므로 **이 보드는 여전히 단일 장애점**이다.

> [주의] **이름 주의** — 이 디렉터리 이름은 "worker" 지만, AgentTest 코드의 `worker/`(윈도우에서 도는
> 작업자 하네스)와는 **다른 것**이다. 여기는 **추론 노드** 쪽이다. `../PLAN.md` §4.1 참조.

## 여기 두지 않는 것 — 마스터/워커 경계

노드는 **순수 추론 엔드포인트**로 유지한다. 상태도 판단 로직도 갖지 않는다.

| 여기(worker/) | 마스터(master/) |
|---|---|
| Ollama + Vulkan 구동, 모델 적재 | 생존 감시·페일오버·라우팅 |
| 노드 OS/드라이버/메모리 튜닝 | 요청 정규화·응답 후처리·검증 |
| — | 컨텍스트 매니페스트 조립 |

**추론 클라이언트 래퍼는 마스터 몫이다**(`../master/README.md`). 노드에 에이전트 코드를 심으면
일회용성이 깨지고, 검증에 필요한 태스크 컨텍스트가 여기 없다.
생존 신호도 노드가 따로 보내지 않는다 — `stream:true` 의 토큰 청크가 그대로 하트비트다(`../PLAN.md` §6.2).

## BC-250 #1 — [완료] 가동 중 (192.168.0.43)

2026-08-06 기준 실측 검증 완료. 상세 이력은
`~/bc250-backup-staging/reports/24-ttm-gtt-tuning-headless-cluster-link.md`(인프라·속도),
`25-model-quality-comparison-14b-vs-35b.md`(모델 품질).

**환경**

| 항목 | 값 |
|---|---|
| OS | Fedora 43, **헤드리스**(`multi-user.target`) |
| 커널 / Mesa | 6.17.1-300 / 25.3.6 (`AMD BC-250 (RADV GFX1013)`) |
| GPU / CPU | **38/40 CU** · **8c/16t** · SMU OC 3500MHz |
| 메모리 | 시스템 14 GiB, VRAM carve-out 0.5 GiB, **GTT 15.10 GiB** |
| Ollama | 0.32.5, `OLLAMA_VULKAN=1` + `OLLAMA_IGPU_ENABLE=1`, 가용 15.6 GiB |

**핵심 설정 (재현 시 필수)**

- `UMA_SIZE=512` — BIOS UMA 는 상한이 아니라 **하한**이라 작게 잡아야 GTT 가 커진다.
  [`fanoush/bc250_memcfg`](https://github.com/fanoush/bc250_memcfg) 로 리눅스에서 CMOS 직접 변경(BIOS 플래시 불요)
- 커널 파라미터 `ttm.pages_limit=3959290 ttm.page_pool_size=3959290`
- `RADV_DEBUG=nohiz` (안정화), `OLLAMA_HOST=0.0.0.0:11434`
- ROCm 은 gfx1013 미지원이라 자동 드롭됨 — **Vulkan 전용이 정상**

**네트워크**

- `11434/tcp` 는 방화벽에서 **192.168.0.57(마스터)에서만** 허용. 다른 클라이언트 추가 시 rich rule 필요.
- API 는 stateless 로 호출한다 — Ollama `/api/generate` 의 `context` 필드를 재사용하지 않는다.
- **`35B-A3B` 호출 시 `"think": false` 필수** — 없으면 사고 과정을 출력해 토큰을 낭비한다.

**모델**

| 용도 | 모델 | 비고 |
|---|---|---|
| 코드 생성 | `qwen2.5-coder:14b` | 8.9GB, 49/49 레이어 GPU |
| 컨텍스트 쿠킹 | `hf.co/bartowski/Qwen_Qwen3.5-35B-A3B-GGUF:IQ2_M` | 13GB, 42/42 레이어 GPU |
| (탈락) | `devstral:24b` | 14GB. tool-calling 관문 3/4 — 아래 참조 |

**동시 상주 불가**(`MAX_LOADED_MODELS=1`, 8.9+13GB > 15.6GB).
[주의] **전환 비용 재측정 (2026-08-08)** — 기존에 적혀 있던 "36초"는 **14b 방향만** 본 값이었다:

| 호출 | 시간 | 전환 페널티 |
|---|---|---|
| 35B (전환) | **66.9s** (재현 64.6s) | ~64s |
| 35B (상주) | 2.7s | — |
| 14b (전환) | **37.5s** | ~36s |
| 14b (상주) | 1.4s | — |

**왕복 1회 ≈ 100초.** 보드 1대에서 두 모델을 번갈아 쓰는 설계는 적재 비용에 잡아먹힌다.

### [중요] tool-calling 관문 — 에이전틱 하네스를 붙일 수 있는가 (2026-08-08)

하네스: `~/claude-workspace/bin/bench_toolcall.py`. 관문 4종 —
①단발호출(구조화 방출) ②도구선택 ③멀티턴(도구 결과 되먹임) ④억제.

| 모델 | ① | ② | ③ | ④ | 판정 |
|---|---|---|---|---|---|
| `35B-A3B IQ2_M` | [완료] | [완료] | [완료] | [완료] | **4/4 — 가능** |
| `devstral:24b` | [완료] | [완료] | [실패] | [완료] | 3/4 — **탈락** |
| `qwen2.5-coder:14b` | [실패] | [실패] | [실패] | [완료]* | **1/4 — 불가** (*호출을 못 해 자동 통과) |

- **14b 는 구조화된 `tool_calls` 를 못 낸다** — 호출 JSON 이 `content` 에 **문자열로** 나온다
  (3/3, `temperature:0`). `/api/show` 가 `capabilities:['completion','tools','insert']` 를
  **광고하는데도** 그렇다. **광고를 믿지 말 것.**
- `devstral` 은 프롬프트에 **형식 지시가 붙으면 호출을 빠뜨리고** "하겠다"고 말만 하고 멈춘다(재현됨).
- 유일 통과자인 35B 는 **코드 생성 시 API 환각**이 있는 모델이다(리포트 25).

### [주의] 컨텍스트 상한 — `num_ctx=32768` 은 못 쓴다

35B 에 `num_ctx: 32768` 을 주면 **152토큰 프롬프트에도 9분 59초 후 HTTP 500** 이 난다
(보드 메모리 `available 0GiB`). 이미 `OLLAMA_KV_CACHE_TYPE=q4_0` + `OLLAMA_FLASH_ATTENTION=1`
인데도 그렇다. **`8192` 는 정상**(3,891토큰 입력 79초). 그 사이 실용 상한은 **미측정**.

##  BC-250 #2 — 지금은 게임 머신이다 (추론 노드 아님)

**Bazzite 를 깔아 스팀 게임용으로 쓰고 있다** (사용자 확정 2026-08-08).
**"미구성"도 "도착 대기"도 아니다** — 다른 용도로 가동 중이다.

**#1 을 완성한 뒤 추론 노드로 전환할 의향**은 있으나 **확정이 아니다.**
즉 노드 증설은 셋업 작업이 아니라 **게임 머신을 내주는 자원 배분 결정**이다 —
계획이 "언젠가 #2 가 붙는다"를 전제로 삼아선 안 된다.

[주의] **전환 시 확인할 것: Bazzite 는 rpm-ostree 기반 이미지형(immutable)이다.**
#1 에서 필수였던 **TTM/GTT 캡 상향 · 커널 부팅 파라미터 · 헤드리스(`multi-user.target`) 전환**이
이미지형 시스템에서 같은 절차로 되는지 **미확인**. 아래 셋업 가이드는 일반 Fedora 를 전제할
가능성이 높다 — **일반 Fedora 재설치가 더 쌀 수 있다.**

- 셋업 가이드: `sim@192.168.0.43:~/bc250-backup-staging/reports/18-second-board-setup-guide.md`
- #1 에서 확정된 설정을 그대로 적용한다 — 헤드리스 · UMA 512MB · TTM `pages_limit=3959290` ·
  거버너 `min=350` · Ollama+Vulkan(`RADV_DEBUG=nohiz`) · 방화벽 `192.168.0.57` 한정 ·
  `/etc/sudoers.d/bc250-power`
- 구성 후 **모델을 보드별로 고정 배정**한다: `#1 = qwen2.5-coder:14b(생성)` / `#2 = 35B-A3B(쿠킹)`
  — 전환 비용(35B ~64s / 14b ~36s, 왕복 ~100초)을 없애기 위함(`../PLAN.md` §4.4)

[주의] **그때까지 성립하지 않는 것들** — 계획에서 전제로 쓰지 말 것:
`../PLAN.md` §4.5 "노드 수만큼 병렬"(보드 1대 = 직렬) · §6.5 추론 노드 페일오버(재할당할 곳 없음) ·
§4.4 보드별 모델 고정 배정 · §9.4.2 35B/14b 역할 분리(왕복 전환 ~100초).
**기본 전제는 "보드 1대, 모델 1개 상주"다.**

## 알려진 리스크

- **GPU 크래시 = 호스트 하드 행.** 단일 APU 라 드라이버가 GPU 를 리셋할 수 없어 원격 복구가 불가능하다.
  무인 운영 시 워치독 + 원격 전원 재투입 수단이 필요하다.
- **장시간 안정성 미검증** — 단기(30분/65사이클)는 통과했다(성공률 100%, tok/s 26.57±0.27,
  커널 오류 0, GTT 고정). 다만 시스템 `MemAvailable` 이 -620MB 단조 감소한 관찰이 남아 있어
  **수 시간 이상 재검증 전에는 무인 상시가동을 판단하지 말 것**. 상세 = 리포트 24 부록 B.
- prefill 이 구조적 약점 — 긴 컨텍스트 주입 설계("소스=정답")와 정면으로 부딪힌다.
- **절전모드 복귀 불가**(SMU 한계) — "켜짐 아니면 꺼짐"뿐이다. 유휴 **74.3W**(거버너 하한
  350MHz 복원 후, 이전 91.1W). 원격 전원 제어가 확보돼 미사용 시간대 차단은 가능하다.

상세 계획: `../PLAN.md`
