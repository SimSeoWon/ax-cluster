...
| 역할 | 머신 | 상태 |
|---|---|---|
| **마스터 / 인프라** | `sim-desktop` (192.168.0.57, Ubuntu 22.04) | **오케스트레이터 ② + 워커 풀 인계**. 추론 링크 개통, 코드 이식 대기. GPU 증설 검토 중(1070 Ti) |
| **추론 노드** | BC-250 #1 (192.168.0.43, Fedora 43) | ✅ 가동 중 — Ollama + Vulkan, 헤드리스. 단기 안정성 검증 통과 |
| **추론 노드** | BC-250 #2 | 미확인 |
| **작업 PC** | 윈도우 UE5 PC (`client/`) | **오케스트레이터 ①**(사용자 접점 Claude Code) + 엔진 바운드(UE5 빌드·RunTests) |
...
| 분산 워커 풀 | 윈도우 PC N대(팀원 각자) | **리눅스가 인계** |
...
사용자 ─지시─▶ 오케스트레이터 (윈도우 Claude Code 또는 마스터)
                  ...
                  ▼
        ┌─────────┴──────────┐
   claim(코드 생성)      claim(requires: ue5)
        ▼                    ▼
 마스터의 워커 N개        윈도우 작업 PC
  · ② stateless 추론 ─▶ BC-250 (텍스트 in/out)
  · ③ 층1 자기검증        · 층3 UE5 빌드 + RunTests
  · ④ 층2 문법검사
        └─────────┬──────────┘
                  ▼
      git `task/<id>` 브랜치에 누적
...
- **워커 풀은 마스터가 인계** — 워커는 I/O 바운드라 여러 기계에 흩어도 병렬성이 늘지 않는다.
  **병목은 추론 용량(보드 수)** ([`PLAN.md`](PLAN.md) §2.3)
- **BC-250 은 파일을 만지지 않는다** — 텍스트만 왕복. 워커가 아니라 추론 노드
- **도구는 서브에이전트가 몬다** — 마스터는 RAG·온톨로지로 **컨텍스트를 대주는 쪽**이지
  직접 작업을 수행하지 않는다. Unreal MCP 도 `ue-editor-operator` 서브에이전트가 윈도우에서 로컬로 몬다
- 윈도우에 남는 것은 **오케스트레이션 + 엔진 바운드(층3)** 뿐
...
- ⚠️ 오케스트레이터가 둘이라 **공유 자원(git·task_queue·RAG) 조율이 필수** — 1인이라고
  claim/fencing 을 걷어내면 안 된다
