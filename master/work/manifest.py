...
[중요] **배달은 원전대로 git-carried 다 (#185 판정, 사용자 확정 2026-08-17 — `carry.py`).**

이 자리에 있던 *"매니페스트를 git 에 싣지 않는다 — 마스터는 push 할 수 없다"* 는
**Flow Y(2026-08-14)로 전제가 죽은 문장**이었고, M5 대조(리포트 19 §1)에서 뒤집혔다.
지금 구조: 이 모듈이 수집·본문 생성을 하고 트윈 사본(`<Project>/manifests/<task_id>.md`)을
남기며(마스터 쪽 기록·refresh 의 원천), **배달은 `carry.publish` 가 durable `task/<id>` 에
commit·push** 하고 워커가 fetch 로 받아 간다. "1회 수집 → 다회 소비" 성질은 그대로,
거기에 재배정 무손실·이력 영구·전송 0 이 돌아왔다.
