"""골조 생성 레인 A/B — [중요] **`DEFAULT_MODEL` 을 측정으로 정한다** (소 1.1.1 후속).

## 왜 재는가

`DEFAULT_MODEL = "claude:opus"` 는 **추론으로 정한 값**이다 — 원전이 Opus 를 썼다는 사용자
기억 + *"골조는 1회이고 모든 조각의 계약이라 값을 쓸 자리"* 라는 논리. [중요] **이 저장소의 규칙은
추측 대신 재는 것**이고(가중치를 눈으로 만지다 한 번 나빠진 전례), 여기도 예외가 아니다.

## 무엇으로 점수를 매기나 — [중요] **결정적 지표만**
...
LLM 에게 *"어느 골조가 낫냐"* 를 묻지 않는다. 골조의 좋음은 우리가 이미 코드로 정의해 뒀다:
    ...
    관계 준수         [중요] 그래프가 말한 부모를 썼는가   지어낸 부모는 실패다
    태그 보존         [DOC]/[STATE]/[BIND] 를 썼나
    펜스 잔재         마크다운을 섞었나 (감점)
    시간·글자수

[중요] **「관계 준수」가 이 벤치마크의 핵심이다.** 골조가 틀린 부모를 상속하면 N개 조각이 전부
...
[주의] 실패한 레인도 **기록한다** — *"응답이 없었다"* 와 *"응답이 나빴다"* 는 다른 사실이다.
"""
...
# [중요] 실제 코드베이스에 붙는 과제로 잰다 — 신규 클래스 하나 + **실재하는 협력 클래스**.
...
def score(paths, sk: S.Skeleton, *, elapsed: float, expect_parents: list) -> dict:
    ...
    want = set(SPEC.files)
    # [중요] 지어낸 부모를 잡는다 — 골조가 틀린 부모를 쓰면 N개 조각이 전부 깨진다
    invented_parent = ""
...
def main(argv) -> int:
    ...
    print(f"[중요] 그래프가 말하는 협력 클래스 조상: {expect}")
