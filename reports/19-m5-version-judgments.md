...
## §10 #211 한글 정답 집합 — 구축 완료 · 1차 측정 · 판정 보류

현실: search_log 의 한글 질의는 4건뿐 (로그 배선이 #203 에서 갓 시작). "질의 3개를 눈으로
보는 것은 측정이 아니다"(bm25.py)에 걸리는 크기라, **판정이 아니라 그릇과 하네스**를 만들었다.

구축:
    정답 집합    analysis/kr_ground_truth.tsv — 자연 4건(로그 실물, 확정 1) +
                구조 182건 (온톨로지 별칭 = 사람 등록 · 판별력 있는 한글 태그 ≤5문서)
    하네스       master/bench_kr.py — 4변형(baseline/kr_on/gated/kr+gated) ×
                hit@1·hit@5·MRR. 자연 30건 미만이면 스스로 「판정 보류」를 찍는다

1차 측정 (구조 183건):
    baseline 0.153/0.814/0.459 · kr_on 0.158/0.809/0.461 (거의 무변화 — RRF 가 도로 누름,
    옛 실측 그대로) · gated 0.787/0.967/0.867 · kr+gated 0.798/0.973/0.875

읽기: **bm25.py 에 기록된 가설(진범 = RRF 의 벡터 상관 오답)이 정답 집합 위에서 확인** —
게이팅이 hit@1 을 5배. KR 라우팅은 게이팅 위 +1%p.

[중요] 실전은 아직 안 바꾼다: 구조 계층은 구성상 BM25 친화(별칭→시소러스, 태그→가중치 3.0
정확 매칭)라 벡터가 이기는 자리(의역·서술형 = 자연 계층)가 표본 1건뿐. **판정 조건 = 자연
확정 30건** — 로그가 쌓이면 bench_kr.py 재실행 한 줄이다. 미확정 자연 3건은 사람이 기대
문서를 채우면 즉시 채점에 들어간다.

## §11 세션 결산 (2026-08-17, 사용자 지시로 마감)

한 세션에서: **되먹임 기계 7종 완주(M4 마감) → M3 처분·마감 → M5 판정 4건 완주·마감 →
M6 착수(측정 2건 종결·1건 구축) + 아이디어 바구니 개설.** 커밋 15개, 전 스위트 3447/3447.

마일스톤 지형: 1~5 닫힘 · M6 열림(잔여 8건 — #211 진행, #204 가 다음) ·
아이디어 바구니(비일감, #212 SAE).

사용자 확인 대기 (이월):
    1. gitea 검증 브랜치 task/selftest-carry-0817 삭제 — [완료] 사용자 직접 삭제 (리포트 20 §4)
