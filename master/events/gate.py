...
PAUSE_STATUS = "in_progress"        # 🔴 이것만. 위 절 참조.
HTTP_TIMEOUT = 3
...
class Verdict:
    ...
    def line(self) -> str:
        if not self.paused:
            ...
            base = (f"🔴 색인 일시 중지 — 진행 중 분산 작업 {len(self.works)}건 "
                    f"({', '.join(self.works[:3])}{'…' if len(self.works) > 3 else ''})")
        if self.degraded:
            base += f" · ⚠️ {self.degraded}"
        return base
...
def _from_disk(spool_root: Path) -> tuple:
    ...
    ⚠️ 큐가 답하지 않을 때 **멈추지 않는 쪽으로 기울지 않는다** — 못 읽었으면 그 사실을 싣고
...
def check(*, queue_url: str = DEFAULT_QUEUE_URL, spool_root: Path | None = None,
    ...
    🔴 **큐에도 디스크에도 못 물으면 멈추지 않는다.** 그 대신 `degraded` 에 사유를 실어
    ...
    if works is None:
        ...
        if works is None:
            return Verdict(paused=False, reason="상태를 확인할 수 없다", source="none",
                           degraded=f"{degraded} — 🔴 멈추지 않고 진행한다(판단 근거 부재)")
