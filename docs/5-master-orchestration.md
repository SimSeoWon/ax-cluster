...
> [주의] **이 수치는 과대평가다 (2026-08-07 실측 정정).** grep 이 **주석·문자열까지 세고 있다.**
...
|---|---|---|---|
| `watcher/` 전체 | 21,171줄 | 19줄 | git watch · RAG · 온톨로지 · 클래스 그래프. 정리 대상: `review.py`(5) · `network_firewall.py`(4) · `skill_defs_distribute.py`(4) · `process_lifecycle.py`(3) · `watch.py`/`common.py`/`agent_defs.py`(각 1). **[주의] `process_lifecycle.py` 는 이식이 아니라 폐기 — §5.4** |
...
| **`mcp/agy_query/`** | 194줄 | — | [중요] **§5.2 목록에 빠져 있던 것 추가** — Antigravity CLI(`agy`) 위임. 마스터가 오케스트레이션하려면 필수(층2 검증). **[주의] 마스터에 `agy` 미설치** |
| **`mcp/local_llm_runner/`** | 144줄 | — | [중요] **누락분 추가** — Ollama 위임. **§6.4 추론 래퍼와 역할이 겹치므로 재사용/대체를 먼저 정할 것**(§3 미결) |
...
> **[주의] 2026-08-07 정정** — 한때 `worker/` 를 A(마스터 이전)로 옮겼으나 **되돌렸다.**
...
**D. [중요] 팀 때문에 존재하는 것 — 이식이 아니라 축소·폐기 후보 (2026-08-07 신설)**
...
**[주의] 반대로 걷어내면 안 되는 것**: claim/lease/fencing·reclaim. 1인이지만 **오케스트레이터가 셋이고
파일을 소유한 작업장이 2대**라
공유 자원 경쟁이 실재한다(§2.2-5).

#### 5.2-E [완료] 디지털 트윈의 범위 — **소스 한정** (사용자 지시 2026-08-08)
...
| | |
|---|---|
| [완료] **대상** | `Source/**/*.{cpp,h,cs}` · `Config/**` — **코드와 그 코드에서 파생되는 것 전부** |
| [실패] **비대상** | `Content/**` (uasset 바이너리), 레벨·블루프린트·에셋 트윈 |
...
##### ② 이식 범위 — [중요] **기능으로 갈라라. 그리고 "만드는 쪽" 이 목표다** (2026-08-08 재정정)
...
> [중요] **사용자가 확정한 목표 ①이 "온톨로지 문서를 자동으로 작성" 이므로 만드는 쪽이 곧 목표다.**
...
[중요] **줄 수를 작업량으로 쓰지 않는다.** `class_graph` 1,341줄은 `data-schema.md` 기준
*"파일 삭제 → 재기동 시 풀 스캔"* 이고, 반대로 829줄짜리 `domain_index.py` 하나에 도메인
색인·규범 검색·**시소러스 빌더**가 같이 들어 있다.

| 기능 | 어디 | 마일스톤 2 |
|---|---|---|
| 컨텍스트 MD 합성 | `watcher/context*` | [중요] 중 1.2 — **상류** |
| 클래스·의존 그래프 | `watcher/class_graph*` · `dependency.py` | [완료] **중 1.1 완료** → `master/graph/` |
...
##### ②-1 [중요] `domain_index.py` 는 의존성이 stdlib 뿐이다 (실측)
...
[중요] **`search_domain_norms` 의 노이즈 차단 정책을 반드시 함께 옮길 것** — 원본이 회사 환경에서
검증했다고 기록한 규칙이다:
...
##### ②-2 [중요] 우리가 이미 밟은 함정 — 임포트는 됐는데 sync 를 안 했다
...
[중요] **위 '재기동' 이 우리에게는 없다.** 원본 `watch.exe` 는 상주 데몬이라 재기동이라는 순간이
...
| **도메인 인덱스**(`bm25_domains.db` + `ontology_domains`) | [중요] **자동 감지 없음 — `sync_domain_index` 를 명시 호출해야 한다** |
...
[중요] **단, 완전한 독립은 아니다.** 검색 코어가 `class_graph_query` 를 통해 `class_ontology`
...
[주의] **그 "얹힘" 이 여전히 빠져 있다 — 다만 이유가 바뀌었다** (2026-08-08 갱신).
...
[중요] 이 결정만 남긴다: **"완료" 는 영역 전체에만 쓴다.** 부분이면 분모를 같이 적는다.
만든 것만 골라 범위를 좁힌 뒤 완결을 선언하면 남은 일이 보이지 않는다 — 마일스톤 1 이
그렇게 실패했다.

##### ④-2 웹 UI — [중요] 이전 판단 **"마스터엔 화면이 없다" 를 뒤집는다**
...
- [중요] **읽기 전용 뷰어는 색인 없이도 된다** — yaml 을 직접 파싱해 트리로 보여주면 되므로
...
| 도구 | 상태 | 필요한 이유 |
|---|---|---|
| `claude` | [완료] 있음 (`~/.local/bin/claude`, v2.1.223) | 층2 검증 폴백 · 마스터 오케스트레이션 |
| `agy` (Antigravity CLI) | [완료] **설치·인증·검증 완료 2026-08-07** (`~/.local/bin/agy`, **v1.1.10**) | 층2 검증 1순위(`syntax_check` 가 agy 우선) · `mcp/agy_query/` |
| `bun` / `node` | [실패] **없음** | gajae-code 가 TypeScript/Bun (§1) |

**[중요] `agy` 이식 부담이 크게 줄었다 — AgentTest 의 윈도우 대응책이 리눅스에선 불필요하다.**
...
|---|---|
| `agy -p` 가 비-TTY 면 0바이트 + 20s hang → **가상 PTY(pywinpty) 전환** | [완료] 순수 파이프로 정상 |
| 출력에 터미널 init ANSI 섞임 → **정규식 strip** | [완료] 깨끗한 텍스트 |
| **argv 순서 버그**(`-p` 를 맨 끝에, 6곳 정정) | [완료] 순서 무관 |
| `agy models` 스피너 TUI 미종료 | [완료] 즉시 반환 |
...
[주의] 윈도우(`client/`)에도 적용되는지는 **미확인** — 별도 실측 필요.
...
**[주의] agy 자율 에이전트에 저장소 작업을 위임하지 말 것** — AgentTest 가 사고를 기록했다
...
- **`post-receive` 훅** — push 순간 서버에서 스크립트 직접 실행. 네트워크조차 안 탄다.
  [주의] 단 **훅 안에서 무거운 작업(RAG 색인 등)을 하면 그동안 push 가 멈춘다.** 훅은 알림만 던지고 즉시 종료할 것.

#### 5.4.2-a [중요] 훅 설치 지점 — `post-receive.d/` 다 (실측 2026-08-08)
...
2. [중요] **훅이 0 이 아닌 값을 반환하면 push 자체가 거부된다.** 즉 이벤트 큐가 죽어 있을 때
...
**[주의] 데몬이 없어지는 게 아니다.** 사라지는 건 폴링 루프뿐이고 상주 프로세스는 여전히 필요하다:
...
#### 5.4.4 [완료] 확정 — **systemd + venv** (2026-08-08)
...
#### 5.4.5 [중요] 폴링이 공짜로 주던 3가지 — 웹훅으로 가면 직접 만들어야 한다
...
> **[주의] 오케스트레이터가 셋이라 더 중요해졌다 (2026-08-08 갱신, 이전엔 "둘"로 적었다).**
...
**[주의] `task_queue` 와 합치지 말 것** — 성격이 다르다.
...
#### 5.4.5-a [완료] 이벤트 큐 설계 확정 (2026-08-08)
...
```
~/ax-spool/                       ← [중요] git 저장소가 아니다. ax-state 에 두지 않는다
...
[중요] **`~/ax-state` 에 두지 않는 이유**: 그쪽은 상태 전이마다 **감사 커밋이 도는 git 저장소**다
...
[중요] **버려도 되는 이유는 워터마크가 커밋 해시이기 때문이다**(§5.5.3-c).
...
[중요] **거부한 이벤트를 지우지 않는다.** 조용히 사라지면 "훅이 안 오네" 와
"오는데 거부되네" 를 구분할 수 없다. 이 세션에서 대소문자 불일치 하나로 **모든 이벤트가
조용히 거부될 뻔한 것**(§5.5.4-④)이 바로 그 시나리오다.
...
[완료] **큐 자체**(`master/events/`) — enqueue · claim · coalesce · reject · 재기동 복구.
[실패] **소비자(색인기)** — **의도적으로 안 만들었다.** 무엇을 색인할지가 미결이다
(§3 "RAG 이식 범위 결정": 검색 코어만 vs 온톨로지까지). 큐는 그 결정과 무관하게 필요하고
지금 만들 수 있으므로 분리했다. 소비자는 `claim_batch()` 를 호출해 배치를 받아 가면 된다.
[실패] **Gitea 훅 설치** — 위 스크립트는 형태만 확정했다. 설치는 §5.4.2-a 의 `post-receive.d/`
...
**[주의] 단 더블 버퍼가 전부를 덮지는 않는다 — 확인할 것 3가지**
...
#### 5.4.7 [완료] 기존 "서버 모드"와의 중복도 판정 완료 (2026-08-07)
...
**[주의] "서버 모드 = 중앙 오케스트레이션"이 아니다.** `--serve` 는 **RAG 전용**이고
...
**→ 소비 쪽(대 2)은 그대로 쓸 수 있는 자산이 많아 싸다. [중요] 진짜 작업은 `watcher/` 쪽 —
