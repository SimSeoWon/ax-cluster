...
SKILLS_BY_ROLE = {
    # [중요] **`ax-work` 는 배달하지 않는다** (사용자 결정 2026-09-02). 구 토폴로지(워커가
    #    커밋·push·검증까지)의 절차서인데 `#320`(2026-08-27)이 상주 클레이머를 철거해 그
    #    경로 자체가 없어졌고, `infer.INFER_INSTRUCTION` 은 *"not ax-work"* 로 명시적으로
    #    배제한다. 그런데도 배달되고 있었다.
    ...
    # [주의] 파일은 지우지 않는다 — 되살릴 조건이 `#320` 노트에 있다. `DEPRECATED_SKILLS` 에
    #    넣어 **이미 놓인 사본까지 정리**한다(목록에서 빼는 것만으로는 기계가 안 치워진다).
    "worker": ("ax-infer",),
...
DEPRECATED_SKILLS = (
    ...
    "distribute",
    "review-work",
    # 2026-09-02 제거 — 구 토폴로지 절차서. 근거는 `SKILLS_BY_ROLE` 주석에 있다.
    "ax-work",
)
