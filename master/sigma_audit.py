...
🔴 **σ.2 의 위험도는 「주기 호출인가」가 정한다** (원전 표기 ★/☆). 매 폴링에서 조용히 실패하는
것은 영구 유실로 누적되고, 이벤트 트리거는 사람이 다시 부른다.

## 🔴 모든 silent fallback 이 결함은 아니다 — 그게 이 감사의 핵심 절제다
...
찾아 줄 뿐이다. 의도가 적혀 있으면 그것은 **설계**다. ⚠️ 자동으로 «결함» 딱지를 붙이면
...
class Finding:
    ...
    broad: bool = True      # 🔴 `except Exception`/bare 인가 (좁은 것은 의도적)
    covers_most: bool = False  # `try` 가 함수 큰 블록을 감싸나

    def line_str(self) -> str:
        star = "★" if self.periodic else "☆"
        doc = "의도 적힘" if self.intent_documented else "🔴 의도 없음"
...
class Report:
    ...
    def undocumented(self) -> list:
        """🔴 **이것만 사람이 봐야 한다** — silent 인데 의도가 안 적힌 자리."""
    ...
    def summary(self) -> str:
        ...
        s = (f"σ.2 정적 검사 — 파일 {self.scanned}개 · silent fallback {len(self.findings)}건 "
             f"· 🔴 의도 없음 {len(u)}건 (그중 ★주기 {star}건)")
        if self.parse_failed:
            s += f" · ⚠️ 파싱 실패 {len(self.parse_failed)}건"
        return s


# 주기 호출 경로 — 색인기·타이머·큐가 반복해서 부르는 모듈.
# ⚠️ **정적으로는 알 수 없다.** 원전도 사람이 ★/☆ 를 달았다. 여기서는 모듈로 **추정**하고,
...
def _swallow_kind(handler: ast.ExceptHandler) -> str:
    ...
    🔴 **조용함의 정의는 「호출자가 실패를 알 수 없다」다.** 그래서 둘은 조용하지 않다:

        ① 핸들러가 로그·카나리·raise 를 한다                    → `_LOUD`
        ② 🔴 **바인딩한 예외를 값으로 돌려준다** (`except E as e: return _fail(e)`)
           → 실패가 **데이터로 전파**된다. 우리 MCP 도구 24개가 전부 이 형태다.

    ⚠️ ②를 안 넣었을 때 54건이 나왔고 그중 상당수가 `mcp_server.py` 의 `_fail(e)` 래퍼였다 —
...
# 🔴 **넓은 핸들러만 본다.** 원전 패턴 원문: *"전 함수 또는 큰 블록을 감싸는 `try` +
# **`except Exception` (또는 bare except)** + silent return / pass + 로그 부재"*.
#
# ⚠️ 첫 이식에서 이 조건을 빠뜨려 **114건**이 나왔다(원전은 6건). `sqlite3.OperationalError` ·
...
def _is_broad(name: str) -> bool:
    ...
    ⚠️ 첫 판은 `strip("()")` 로 `"(bare)"` 를 벗겨 놓고 원래 문자열과 비교해서 **bare 를 못
...
def _covers_most(fn, node: ast.Try) -> bool:
    ...
    ⚠️ 근사다 — 원전도 사람이 봤다. 좁히는 신호로만 쓰고 단독 판정에 쓰지 않는다.
    """
...
def _handler_intent(lines: list, h: ast.ExceptHandler) -> bool:
    """🔴 **핸들러 블록의 원문 줄**에서 의도를 찾는다 (주석 포함).

    ⚠️ 첫 판은 `ast.unparse` + 독스트링만 봤는데 **주석이 소실**된다. 그래서 의도를 주석에
    적어 둔 자리(`ontology/stale.py:104`)를 *"의도 없음"* 으로 계속 잡았다 — 내가 그 한계를
    모듈 주석에 적어 놓고 **해결하지 않은** 것이다(σ.2 를 자기 자신에게 돌려 발견, 2026-08-14).

    🔴 핸들러별 의도는 **주석이 자연스러운 자리**다 — 함수에 핸들러가 셋이면 독스트링 하나로는
        ...
        for node in ast.walk(fn):
            ...
            for h in node.handlers:
                ...
                if not _is_broad(exc):
                    continue          # 🔴 좁은 예외는 의도적이다 (위 _BROAD 주석)
...
def scan(root: Path, *, skip_tests: bool = True) -> Report:
    """저장소를 훑는다. 🔴 테스트는 뺀다 — 테스트의 `except` 는 검사 대상이 아니다."""
    r = Report()
...
# σ.1 — 발동 매트릭스
#
# 🔴 **이쪽은 자동화하지 않는다.** 원전도 사람이 표를 만들었고, 그 값은 **세 축을 독립으로
...
#
# 🔴 셋이 **서로를 대신하지 못한다** — 원전 실측: 코드는 ✓ 인데 산출물이 ❌ 였던 것이
# *"도메인 자동 승급"*(회사 환경 수개월), 산출물이 ❌ 인 것을 아무도 몰랐던 이유가 카나리 부재다.
#
