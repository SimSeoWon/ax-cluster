...
def test_extract_section_is_bounded():
    ...
    check("🔴 다음 헤딩에서 끊는다 (파일 끝까지 삼키지 않는다)",
...
def test_ue_prohibitions_always_present():
    with tempfile.TemporaryDirectory() as td:
        ...
        check("🔴 문서가 없어도 UE 금지는 실린다", "리네이밍하지 않는다" in body, body[:80])
        ...
        check("⚠️ 문서 부재가 note 로 보고된다", bool(note) and "CLAUDE.md" in note, note)


def test_project_section_is_read_not_hardcoded():
    """🔴 파일을 바꾸면 결과가 바뀌어야 한다 — 사본이면 안 바뀐다."""
    with tempfile.TemporaryDirectory() as td:
        p = Path(td) / "CLAUDE.md"
        ...
        check("🔴 파일을 바꾸면 결과가 바뀐다 (하드코딩 사본이 아니다)",
...
def main() -> int:
    for fn in (test_extract_section_is_bounded, test_ue_prohibitions_always_present,
        ...
        fn()
    total = PASS + FAIL
    print(f"{'✅' if not FAIL else '🔴'} test_conventions: {PASS}/{total} 통과")
