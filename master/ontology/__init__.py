...
[중요] **범위가 좁다.** 원본 η.1 원안은 1,799 클래스 전체 분류였으나 **active 도메인 멤버십
합집합만**으로 좁혔다(24 클래스로 confidence ≥0.7 100% 달성, 36배 절감). 사용자 발화가
근거다 — *"도메인에 잡혀있는 것만 온톨로지 대상으로 처리"*.

[중요] **폐지된 것을 이식하지 말 것** — Obsidian `.md` 병행 출력 · 검수 큐 · sidecar JSON ·
...
- [중요] **액션은 함수 1:1 매핑이 아니다** — 누가/누구에게/무엇을/어떤 플로우의 **의미 단위
  종합 기술**. 온톨로지는 컨텍스트 문서들을 **합치고 그 사이 관계·호출을 담는 개념 문서**다
- [완료] 분류는 `classes` 컬럼이 아니라 `class_ontology` **별도 테이블 + FK CASCADE** —
  원본도 `save_graph()` 의 DELETE+INSERT 가 분류를 덮는 사고를 겪고 그렇게 갔다
- [중요] **멤버 수집은 대화형** — 원본은 자식 1홉을 자동 확장하지만(`expand_seeds_with_one_hop_children`)
  우리는 **제안 → 확인**이다 (`collect.propose`)
- [중요] **개념 계층은 사람이 요청한다** — `[미션 시스템]` 아래 `[미션 에디터]`. 도구가 추측하지
