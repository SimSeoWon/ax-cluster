...
## §16 `.2` 의 AgentWiki — 세어 보니 지울 것이 아니었다 (2026-08-20)

사용자 지시 *"`.2` AgentWiki 설치본부터 정리해"* 로 시작해, 실측 후 *"그럼 그냥 냅둬"* 로 끝났다.
**`.2` 에는 아무 변경도 하지 않았다** (삭제·설정 변경 0건).

무엇이었나 — 원전의 **비개발자용 위키 클라이언트**(`build_wiki_client.bat` → `AgentWiki.zip`,
90파일 191.4MB, git 미추적):

| 실측 | 결과 |
|---|---|
| 누가 쓰나 | [중요] **사람의 Claude Desktop** — `%APPDATA%\Claude\claude_desktop_config.json` 이 `agentwiki` → `AgentWiki\.claude\mcp\context_search.exe` 로 등록. WindowsApps `claude.exe`(부모 `sihost.exe` = 로그온 셸)가 20:56 부터 물고 돈다 |
| 서버 주소 | [주의] `config.json` → `http://192.168.0.33:8100` — **옛 토폴로지** (`.33` 이 서버였던 시절). 실측 **응답 없음** |
| 데이터 | **2026-07-12 에 멈춰 있다** — context 63 MD · 벡터 캐시 · 온톨로지 캐시 전부 그 날짜 |
| 사람 자산 | `my_thesaurus.md`(개인 시소러스) — 파일 스스로 *"이 PC 에만 저장되고 지워지면 다시 만들어야"* 라고 적는다 |

[중요] **왜 거기 있나 — 사용자가 목적을 갖고 둔 것이다** (2026-08-20): *"`.33` 에서 비개발자용
처리 테스트 하려고 둔거라고"*. 즉 **원전의 비개발자 경로를 실제로 시험해 본 설치본**이고,
그래서 `config.json` 이 `.33:8100` 을 가리킨다 — 그 시험은 **`.33` 이 서버였던 시절**의 것이다.
낡은 것은 그 사실의 결과이지 방치의 증거가 아니다.

[중요] **내가 세 번 틀렸고, 셋 다 세기 전에 이름만 보고 판단한 결과다.** ⑴ 「잠들어 있다」
(→ 프로세스 4개가 돌고 있었다, §15) ⑵ 「원전 잔재니 지운다」(→ Desktop 에 등록된 입구였다)
⑶ 「깨진 채 쓰이는 사람의 도구」(→ 목적을 갖고 둔 **시험 설치본**이었다). **원전 함대라는
이름표가 붙었다고 다 잔재가 아니다.**

우리 이식본의 대응물은 8103 웹 UI(무인증 뷰)와 `ax-client` 조회 11종이다 — 비개발자 경로를
그쪽으로 옮길지는 열린 결정이고, 지금은 **그대로 둔다**(사용자 결정: *"그럼 그냥 냅둬"*).
[완료] 조사 중 마스터에 떴던 사본은 **삭제했다** — 사용자 판단: 그 설치본은 시험용이라 마스터가
사본을 들고 있을 이유가 없다.

## §17 큐·브랜치 정리, 그리고 `finalize_work` 를 원전 6단계로 (2026-08-21)

### 정리 — 센 뒤에 지웠다

| 대상 | 실측 | 조치 |
|---|---|---|
| 미종결 태스크 4건 | 2026-08-09 e2e 잔여 · **부모 work 은 이미 cancelled** (부모는 닫혔는데 자식만 열려 있었다) | 큐 API `cancel` → 미종결 **0건** |
| `.ax/work/` 부산물 7개 | 태스크가 cancelled 로 바뀌자 `cleanup` 판정이 「삭제 가능」으로 뒤집혔다 | `cleanup apply` |
| `bench-*`·`smoke-runner-1` 9개 | 파일은 **`manifest.md` 하나씩**(입력 프롬프트) · 결과·로그 0 · 측정치는 리포트에 있다 | 삭제 (양쪽 `.ax/work/` 0개) |
| 원격 attempt/task 6개 | 전부 **단일 커밋** — 주석 보강 3건 · probe 2건 · 골조 context 1건 | 삭제 (원격 attempt·task **0개**) |
| 워커 로컬 attempt 3개 | 원격이 사라져 **마지막 사본**이 됐다 | 번들로 뜬 뒤 삭제 (세 대 모두 `main` 만) |

[중요] **훅이 막는 자리를 사람이 열었다.** pre-push 훅(`#125`)은 *"`<remote>/main` 에서 도달
가능한 것만 삭제"* 이고, 그 근거가 바로 이 브랜치들이다 — *"원격 attempt 4건이 전부 도달
불가였고 스풀에 없었다. 일괄 삭제했다면 실패 시도 3건의 유일한 사본이 사라졌다"*(리포트 16 §9).
훅 문서 스스로 *"도달 불가는 여전히 사람 몫"* 이라 적어 뒀으므로 사용자 지시가 그 예외 경로다.
`--no-verify` 로 우회하기 전에 **번들을 떴다** — `~/claude-workspace/backups/attempt-refs-20260821.bundle`
(**4.9KB**, 6개 팁 전부, `git fetch <bundle> <ref>` 로 복구).
[주의] 처음엔 `--all` 로 떠서 **1GB** 가 나왔다(게임 저장소 전체 이력). `--not main` 으로 다시
만들어 4.9KB. **thin 번들이라 복구 시 선행 커밋이 저장소에 있어야 한다**(`b1ba6af`·`bc4b38f` — 있다).

### `finalize_work` — 원전 6단계 동등 (사용자 결정 2026-08-21)

사용자: *"모든 병렬 작업 끝나고 자동 머지하고 정리해야 할 꺼같은데"*. 원전을 읽고 대조했다
(`docs/distributed-workers.md` § Phase 2-A):

| 원전 6단계 | 이식 전 | 이식 후 |
|---|---|---|
| ① `main` `--no-ff` 머지 + push | [완료] 자동(`confirm=True`) | 그대로 |
| ② Redmine 완료 기재 + 머지 커밋 | [주의] **본문만** 만들었다 | [완료] **자동 전송** — 대행 경로(`/api/v1/redmine/note`) |
| ③ 원격 브랜치 정리 | [주의] **명령만** 돌려줬다 | [완료] **자동 삭제**(`apply_branch_cleanup`) |
| ④ main 복구 | 해당 없음 — worktree 라 사람 트리를 안 건드린다 | 그대로 |
| ⑤ `merge_status → merged` | [완료] 자동 | 그대로 |
| ⑥ RAG 최신화 | [완료] 자동 — `ax-indexer.path` | 그대로 |

[중요] **2026-08-16 의 「브랜치 삭제는 사람」을 뒤집었다.** 뒤집어도 되는 근거는 가드가
**둘**이라는 점이다 — `plan.delete` 는 도달 가능분만 담고, 그 위에서 pre-push 훅이 **같은
판정을 한 번 더** 한다. 훅이 거부하면 우리 판정이 틀렸다는 뜻이므로 **밀어붙이지 않는다**
(`apply_branch_cleanup` 은 `--no-verify` 를 쓰지 않는다).

[중요] **순서가 계약이다** — 정리는 **머지 뒤**다. 앞에서 하면 같은 ref 가 「미병합」으로 보여
아무것도 안 지워진다(그래서 `plan_branch_cleanup` 호출도 `fetch` 뒤에 있다).

[주의] 상태는 **「해결」**로 적는다 — 「완료」로 닫는 것은 사람의 결정이라는 규약
(`redmine.STATUS_ON_FINALIZE` 과 같은 방향). 원전은 「완료」로 닫지만 우리 규약이 다르다.

[중요] **키는 이 코드가 도는 기계에 없다** — `review.py` 는 `.33` 에도 배달되는 페이로드다.
그래서 ② 는 마스터 대행을 지나간다(오늘 §12 에서 이식한 그 자리). 코드가 만드는 것은 본문뿐이다.

실패 처리: 기재 실패·삭제 실패는 **머지를 되돌리지 않는다**. 못 지운 것만 사람 몫으로
`commands` 에 남기고(성공분을 또 주면 혼선이다), 기재 실패는 사유를 로그에 싣는다.

    유닛 3681 → 3697 통과 · 실패 0 (계약 3벌 갱신 + 가장자리 3케이스 신설)
    [중요] 옛 계약을 고정한 테스트 둘이 빨간불이 됐다 — *"그래도 우리가 지우지는 않았다"* ·
    *"삭제는 명령으로만 준다"*. 뒤집힌 결정이라 그 자리를 **「도달 가능한 것만 지웠나」**로 갈았다.
    재배달: `review.py` 는 `.33` 페이로드다 — 안 하면 그쪽이 옛 로직으로 검수한다(배달·대조 완료)
