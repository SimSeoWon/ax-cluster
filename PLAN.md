...
- [ ] 🔴 **BC-250 #2 — 아직 물리적으로 없다. 주문 후 도착 대기 중** (사용자 확정 2026-08-08).
      ⚠️ **이전 문구("미구성 — #1 완성 후 복제 구성")는 보드가 이미 손에 있는 것처럼 읽혀 오해를 불렀다.**
      순서상 나중인 게 아니라 **물건이 아직 없다.** 도착 전까지는 착수 자체가 불가능하다.
      셋업 가이드는 준비돼 있다: `sim@192.168.0.43:~/bc250-backup-staging/reports/18-second-board-setup-guide.md`
