...
- [~] **BC-250 #2 구성** — **의도적으로 나중 (사용자 확정 2026-08-07).**
      #1 을 완성한 뒤 그 결과를 복제해 구성·연결한다. "상태 미확인"이 아니라 **순차 진행**이다.
      셋업 가이드 존재: `sim@192.168.0.43:~/bc250-backup-staging/reports/18-second-board-setup-guide.md`
      ⚠️ **그때까지 §4.5 의 "노드 수만큼 병렬"은 실효가 없다**(보드 1대 = 직렬).
      클래스 단위 분할의 이득은 당분간 **충돌 방지**에 그친다
...
⚠️ **선결: BC-250 #2.** 두 모델 동시 상주 불가(`MAX_LOADED_MODELS=1`, 전환 ~30s 실측)라
보드 1대에서는 이 배치가 전환 비용에 잡아먹힌다. **#2 구성의 실질적 근거가 하나 늘었다.**
