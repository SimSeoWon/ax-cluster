...
def test_updates_notes_only():
    ...
    check("🔴 상태를 안 줬으면 건드리지 않는다", "status_id" not in payload["issue"], str(payload))


def test_status_name_resolved_dynamically():
    """🔴 id 를 박지 않는다 — 이 Redmine 의 상태 이름은 **한국어**다."""
    s = Sender()
...
def test_http_failure_is_reported_not_raised():
    ...
    check("🔴 예외를 던지지 않는다 (검수 흐름을 막지 않는다)", isinstance(out, str))
    check("🔴 실패를 문장으로 남긴다", "실패" in out and "#7" in out, out)


def test_missing_key_is_loud():
    """🔴 키가 없을 때 **조용히 성공처럼** 끝나지 않는다."""
    import os
    ...
    try:
        ...
        check("🔴 결과에 '갱신' 만 적히지 않는다", not out.startswith("레드마인 #7 갱신"), out)
    finally:
        if old is not None:
...
def test_api_key_never_returned_from_logs():
    """🔴 키 조회는 **주입 가능**해야 테스트가 실 컨테이너를 안 두드린다."""
...
def main() -> int:
    for fn in (test_updates_notes_only, test_status_name_resolved_dynamically,
        ...
        fn()
    total = PASS + FAIL
    print(f"{'✅' if not FAIL else '🔴'} test_redmine: {PASS}/{total} 통과")
