...
| 역할 | 머신 | 상태 |
|---|---|---|
| **마스터 / 인프라** | `sim-desktop` (192.168.0.57, Ubuntu 22.04) | **인프라 — 작업장이 아니다.** RAG·온톨로지·큐·추론 브로커. **task_queue 이식 완료**(포트 8101), RAG·브로커는 대기. GPU 증설 검토 중(1070 Ti) |
...
- **task_queue systemd 등록** — 유닛 파일(`master/systemd/ax-task-queue.service`)은 작성했으나
  아직 설치·enable 하지 않았다. 수동 기동으로만 검증한 상태
