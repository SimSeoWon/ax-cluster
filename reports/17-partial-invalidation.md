...
- 🔴 **실 트윈에서 한 번 돌려야 끝난다.** 지금까지는 복사본 실증뿐이다. 실 재합성은
  ① 로컬 노드 LLM(무료·수 분) ② 도메인 하나의 actions/invariants 를 실제로 갱신 ③ 트윈이
  git 밖이라 **되돌릴 수 없음** — **사용자 승인 사항**. 추천 대상은 `MissionRuntime`
  (md_hash 부재라 full 로 돌면서 그 스탬프까지 찍혀, 다음 사이클부터 partial 이 걸린다)
