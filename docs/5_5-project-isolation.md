...
드러났다). 두 축을 섞지 말 것:
    ...
    요청 표면      [실패] **이쪽이 지금 열려 있다.** `paths.resolve(name)` 은 `name` 이 있으면
                   `reg.active` 를 **아예 보지 않고**(등록 여부만 본다), `task_queue/server.py:430`
                   ·`:443` 이 `resolve(req.project or "")` 로 **요청이 준 태그를 그대로 따른다.**
                   `resolve_mounted_only()` 는 프로덕션 호출자 **0** 이다
    claim 축       [실패] `ClaimReq` 에 project 필드가 없고 `claimer.py:93` 이 값을 읽고도 안 보낸다
                   → 3단계 태그 계약(`#257`·`#247`·`#258`·`#248`·`#243`)
...
[주의] 그래도 `NS` 에 claimer 상주를 켜지 않은 이유가 그것이다 — 켜는 순간 두 조건이 채워진다.
