...
### 🔴 이식 후 분리 — 자동 로드 문서의 길이는 세금이다 (사용자 지시 *"기능에 따라 분리해"*)
...
매 작업의 세금"* 이라 적어 뒀으므로, 사용자 지시로 **기능별 문서**를 신설해 근거를 옮겼다:
    ...
    docs/11-agent-conduct.md   §11.1 묻기 · §11.2 만들기 전에 읽기 · §11.3 수정 권한
                               §11.4 승인과 검토 비용 · §11.5 실행 권한 · 🔴 §11.6 원전 5조 대조표

`CLAUDE.md` 양쪽에는 **한 줄 규칙 + 포인터**만 남겼다. 🔴 **규칙을 지운 것이 아니라 근거를 옮긴
것이다** — 규칙 자체는 인라인에 남아야 한다(*"놓치면 안 되는 것만 남긴다"*).

    CLAUDE.md                 214 → 253 → **226줄**  (세션 순증 +12)
    machines/sim-desktop.md   219 → 228 → **225줄**  (세션 순증 +6)

⚠️ 순증 +12 의 대부분은 5조가 아니라 **§9 의 토폴로지 정정**이다(틀린 하드 규칙을 고친 것이라
...
**② `now` 주입 가능.** 원전은 `datetime.now()` 를 직접 부른다. 🔴 **원전 자신이 같은 이유로
`attempt_branch(ts=…)` 를 인자로 뒀다**(*"테스트에서 시간을 고정할 수 있어야 한다"*).

⚠️ **`rag_analysis` 게이트만 이식했다.** `weekly_plan`·`recipe_synthesis` 는 그 배치(`#155`·
`#157`)가 없다 — 게이트만 옮기면 §12.3 에서 한 번 피한 *"입력 없는 배치"* 와 같은 실수다.

#### 🔴 순서가 설계다 — 일시 중지 게이트가 **밖**에 있다
...
#### ⚠️ `--force` 의 문서와 코드가 어긋나 있었다 (스스로 잡음)

독스트링은 *"시간 게이트만 넘는다"* 였는데 코드는 **활성 플래그까지** 넘고 있었다.
🔴 **운영자가 명시적으로 끈 것을 도구가 되살리면 안 된다** → 코드를 문서에 맞췄다:

    넘는다     시각 · 요일 · 최소 간격 · 입력 최소치 (얇아도 안전 — 분석기가 판단을 보류한다)
    못 넘는다  🔴 `batch.<key>=false` · 🔴 일시 중지 게이트

**첫 배치** `context_search/search_log_analyzer.py` — 원전 696줄 이식, **stdlib·LLM 0**.
임계값·판정 문구는 **원전 그대로**(바꾸면 과거 리포트와 비교 불가). ⚠️ `tag` 집계는 우리에게
...
산출물 확인 — `rag_channel_report.md` · `language_gap_report.md`:
    ...
    🔴 표본 가드가 작동했다   "표본 2건 < 최소 30건 — 자동 판정 보류 (수치는 참고용)"
    ...
    도메인 빈도             Source 7 · 🔴 **_archive 1**

🔴 **첫 리포트가 「판단 보류」로 나온 것이 정상이고 옳다** — 로그를 오늘 심었으므로(§12.3)
...
🔴 **실 systemd 경로로 게이트가 막는 것을 확인했다** — 유닛이 도는 것과 **배치가 도는 것**은
