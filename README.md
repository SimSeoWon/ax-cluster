...
| **추론 노드** | BC-250 #2 | 🔴 **아직 없다 — 주문 후 도착 대기 중.** 순서상 나중인 게 아니라 물건이 없다 |
...
- 🔴 **BC-250 #2 도착 대기** — 주문했으나 아직 물리적으로 없다. 도착 전까지 **병렬 추론(§4.5)·
  추론 노드 페일오버(§6.5)·보드별 모델 고정 배정(§4.4)·§9.4.2 역할 분리가 전부 성립하지 않는다.**
  **기본 전제는 "보드 1대, 모델 1개 상주"** ([`PLAN.md`](PLAN.md) §3)
- **작업자↔마스터 경로 이관**
