...
2. 토큰이 마스터를 떠나지 않는다. 세 서비스는 LAN-only + 베어러인데, 두 대에 더 복사하면
   노출면만 늘고 얻는 게 없다.
3. 🔴 **리스 하트비트에는 오래 사는 호출자가 있어야 한다.** 리스 1200초, 재확보 루프 30초
...
## ⚠️ 승인은 열려 있다 (사용자 결정 2026-08-09)

*"승인은 뭐든 할 수 있어도 될 것 같아, 깃이니까 언두 해도 되고"* 에 따라
`--dangerously-skip-permissions` 로 파견한다. 🔴 **다만 git 이 되돌려 주는 것은 추적 파일까지**
...
# 🔴 **비대칭은 의도다.** 뒤에 사유가 붙은 `RESULT: BLOCKED — …` 는 받아 주고,
...
# 🔴 ASCII 만. 위 § 참조 — 한글을 명령줄에 실으면 워커 콘솔에서 깨진다.
INSTRUCTION = (
...
class DispatchError(RuntimeError):
    """파견 자체가 불가능하다. 🔴 **태스크를 집은 채로 죽지 않게** 호출자가 반납해야 한다."""


@dataclass
class Outcome:
    """워커 한 번의 결과. 🔴 **모르는 값을 성공으로 접지 않는다.**"""
    status: str = BLOCKED
    branch: str = ""
    head: str = ""
    reason: str = ""
    ...
    def submittable(self) -> bool:
        """큐에 `submit` 할 수 있나. 🔴 브랜치·커밋이 없으면 성공이라도 제출할 게 없다."""
        return self.ok and bool(self.branch) and bool(self.head)

    @property
    def summary(self) -> str:
        if self.submittable:
            return f"✅ {self.branch} @ {self.head[:8]}"
        s = f"🔴 {self.status}"
        if self.ok and not self.submittable:
            s = "🔴 DONE 이라 했지만 ATTEMPT/HEAD 가 없다"
...
def parse_result(stdout: str) -> Outcome:
    ...
    if m:
        ...
        if loose:
            ...
            return out
        # 🔴 마지막 줄이어야 한다. 중간에 있으면 그 뒤에 무슨 일이 더 있었다는 뜻이다.
...
@dataclass
class Pick:
    """워커 선택 결과. 🔴 **왜 떨어졌는지까지 들고 나온다** — 큐가 조용히 굶는 걸 막는다."""
    chosen: object = None
    ...
    def summary(self) -> str:
        if self.chosen is not None:
            return f"✅ {self.chosen.host}"
        if not self.rejected:
            return "🔴 후보 자체가 없다 (레지스트리에 워커가 없나)"
        return "🔴 파견할 워커가 없다 — " + "; ".join(f"{h}: {r}" for h, r in self.rejected)
...
def pick_worker(facts: list, requires=(), *, clean=None) -> Pick:
    ...
    🔴 `role` 로 먼저 거른다 — `.33` 은 SSH 가 닿지만 `requester` 이고, 사람이 편집 중인
    작업 트리에 파견하면 안 된다(`driven` ≠ `role`).

    🔴 `clean` 을 주면 **워킹트리까지 보고 고른다.** 실측 2026-08-09: `.2` 의 추적 파일이
...
def deliver_manifest(facts, task_id: str, text: str, *, writer=None) -> str:
    """매니페스트를 워커 체크아웃 안에 놓는다. 🔴 **되읽어 sha256 대조까지 `bundle` 이 한다.**"""
...
class _Beater:
    """SSH 가 도는 동안 리스를 살려 둔다. 🔴 **실패해도 작업을 죽이지 않는다** — 하트비트
...
def run_task(facts, task_id: str, manifest_text: str, *, beat=None, runner=None,
    ...
    🔴 **큐에 제출하지 않는다.** 제출은 호출자(마스터)의 일이다 — 이 함수는 *워커가 무엇을
    ...
    if rc != 0 and not res.ok:
        ...
        # 🔴 워커는 DONE 이라는데 껍데기는 실패했다 — 성공으로 접지 않는다
...
# ── 큐 배선 — 🔴 집었으면 반드시 놓는다 ──────────────────────────────────────
#
# 🔴 claim 과 submit 사이에서 예외가 나면 태스크는 **리스가 만료될 때까지(1200초) 붙잡힌
...
def _api(method: str, path: str, payload=None, *, timeout: int = HTTP_TIMEOUT):
    """큐 호출 한 번. 🔴 **토큰은 마스터에만 있다** — 이 함수가 워커로 건너가면 안 된다."""
...
def claim(worker_id: str, capabilities, *, api=_api):
    """태스크 하나를 집는다. 없으면 `None`. 🔴 **능력은 실측값**(`facts.capabilities`)이다."""
...
def submit(task_id: str, out: Outcome, *, epoch=None, api=_api):
    """성공 제출. 🔴 `submittable` 이 아닌 것을 여기로 보내지 않는다 — 근거가 없다."""
...
def _manifest_for(paths, task_id: str) -> str:
    """등록 때 마스터가 써 둔 매니페스트. 🔴 **없으면 파견하지 않는다.**
...
AUTO = "auto"          # 🔴 기본은 **재는 것**이다. 끄려면 호출자가 명시적으로 None 을 준다.
...
def run_once(paths, *, facts=None, project: str = "", api=_api, runner=None, writer=None,
    ...
    if pick.chosen is None:
        return {"picked": pick.summary, "task": None,
                "note": "🔴 워커가 없어 claim 조차 하지 않았다 — 집어놓고 못 돌리는 게 더 나쁘다"}
    ...
    try:
        ...
        try:
            ...
            res["submitted"] = f"🔴 반납 실패 — 리스 만료까지 {LEASE_SEC}s 붙잡힌다 ({e2})"
        raise
    return res
...
#
# 🔴 데몬으로 만들지 않는다(§5.5.4). 상주가 필요하면 systemd timer 로 `once` 를 부른다 —
