...
## [중요] 휴리스틱과 **짝**이지만 반대쪽이다

    소 1.1.6  휴리스틱     사람의 판단   → 다음 골조   [중요] **사람이 답한 것만** 적립
    소 2.3.4  실패 카탈로그 기계의 판정   → 다음 골조   [완료] **기계가 모은다**
...
[중요] **왜 이쪽은 기계가 모아도 되나**: 근거가 **결정적 판정**이기 때문이다 — 컴파일러가 낸 오류
문자열이고, 모델의 의견이 아니다. 휴리스틱에서 자동 적립을 금지한 이유(*"모델이 자기 말을
근거로 같은 선택을 반복한다"*)가 여기엔 적용되지 않는다.

## [중요] 무엇을 적립하나 — **판정 원문에서 뽑은 것만**

    층1  동결 위반 · 휘발 태그 잔재
    층2  상용 모델이 지적한 것
    층3  UE5 빌드 오류 (`error C####: …`) · 테스트 실패

[주의] **산문 요약을 적립하지 않는다.** *"헤더를 잘못 물었다"* 같은 문장은 다음 프롬프트에서 아무
...
# [중요] 컴파일러 오류에서 **판정 부분만** 뽑는다. 파일·줄 번호는 다음 작업에서 달라지므로
...
def extract_build_errors(log: str) -> list:
    """빌드 로그에서 **오류 판정만** 뽑는다. [중요] 결정적 — 요약하지 않는다."""
    out: list = []
...
def from_applied(a) -> list:
    """`integrate.Applied` → 카탈로그 항목들. [중요] **판정 원문에서만** 만든다."""
    items: list = []
    ...
    if b is not None and not getattr(b, "passed", True):
        ...
        if errs:
            ...
            # [주의] 오류 문자열을 못 뽑았으면 **그 사실을** 남긴다 — 조용히 빈손으로 끝내지 않는다
...
def load(paths) -> list:
    ...
    try:
        ...
        # [주의] 못 읽는 카탈로그로 생성을 막지 않는다 — 되먹임은 보조 재료다
        return []
    out: list = []
...
def add(paths, items: list) -> int:
    ...
    for it in items:
        ...
        if old is None:
            ...
            have[it.key] = it
            continue
        # [중요] 같은 실수는 한 줄 + 횟수다 (프롬프트 예산을 아낀다)
        old.count += 1
        old.last = now
        for t in it.tasks:
...
def render(items: list, *, budget: int = BUDGET, limit: int = MAX_ENTRIES) -> str:
    """골조 프롬프트에 실을 블록. [중요] **자주 나는 것부터.** 없으면 빈 문자열."""
    if not items:
        return ""
    ...
    for f in ranked:
        ...
        kept += 1
    if kept < len(items):
        # [주의] 자른 것을 말한다 — 조용한 절단은 "다 실었다" 로 읽힌다
