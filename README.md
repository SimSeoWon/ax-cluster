...
**전력 (콘센트 실측)** — 유휴 **80W** / GPU+CPU 최대 부하 174W (PSU+팬 오버헤드 20~27W 포함,
보드 단독 유휴는 53~60W). BC-250 은 **절전모드 복귀가 불가**(SMU 한계)라 "켜짐 아니면 꺼짐"뿐이다.

## 열려 있는 것

- **장시간 안정성 재검증** — 위 메모리 감소 추세 확인. 무인 운용의 전제
- **원격 전원 재투입 수단**(스마트 플러그/PDU) — GPU 크래시 = 호스트 하드 행이라 소프트웨어 복구 불가.
  같은 장치로 미사용 시간대 절전과 마스터 전력 실측도 겸할 수 있다
