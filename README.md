...
- 🔴 **포트 8101 은 LAN 에서 닿지 않는다 + 인증이 없다** — `ufw` 기본 정책이 `deny (incoming)` 인데
  8101 규칙이 없어 **윈도우 작업장에서 접근 불가**하고, `task_queue` HTTP 서버엔 **인증 코드가 0건**이다.
  `client/` 를 붙일 때 **LAN 한정**(`from 192.168.0.0/24`)으로 열 것 — `ufw allow 8101` 전체 개방은
  금지(Gitea 처럼 포트포워딩까지 걸리면 큐를 누구나 조작한다). 인증 추가 여부도 그때 판단
  ([`PLAN.md`](PLAN.md) §9.5.6)
