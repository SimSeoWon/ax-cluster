...
> 이 기계는 `ax-request` 로 **등재만** 한다(`docs/8` §8.4).
> 🔴 [주의] **「단일 writer 는 서버」를 근거로 쓰지 말 것** — 오인용이다(`docs/8` §8.4 정정
> 2026-09-02). 정본 7단계 ⑸ 는 **워커가 자기 서브 브랜치에 커밋**하고, ⑺ 은 **통합 승인을
> 이 기계가 사람에게 묻는다**. 등재만 하는 것은 「골조 제작 위치」 결정(`#339`) 때문이고
> 쓰기 권한 이론 때문이 아니다.
