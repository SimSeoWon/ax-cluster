...
[중요] 지키는 계약: **`verified_by_user` 와 섞지 않는다.** 뜻이 다르고, 섞으면 둘 다 못 쓴다.
...
def test_plan_then_apply() -> None:
    """[중요] 일괄이라 더 위험하다 — 기본은 계획만 (`sync`·`cleanup` 과 같은 규칙)."""
    with tempfile.TemporaryDirectory() as t:
        p = _seed(Path(t))
        plan = E.protect_all(p, reason=WHY)
        check("계획은 세운다", len(plan.changed) == 3, str(plan.summary))
        check("[중요] 계획은 아무것도 안 쓴다", not E.protected_items(p), str(E.protected_items(p)))
...
def test_reason_is_required() -> None:
    """[중요] *왜* 보호하는지 없으면 다음 사람이 풀어도 되는지 판단할 수 없다."""
    with tempfile.TemporaryDirectory() as t:
        p = _seed(Path(t))
        try:
            E.protect_all(p, apply=True)
            check("[중요] 사유 없이는 못 건다", False, "사유 없이 통과했다")
        except E.EditError as e:
            check("[중요] 사유 없이는 못 건다", "사유" in str(e), str(e)[:50])
...
def test_not_the_same_as_verified() -> None:
    """[중요] 이것이 이 기능의 존재 이유다 — 신호를 오염시키지 않는다."""
    with tempfile.TemporaryDirectory() as t:
        p = _seed(Path(t))
        E.protect_all(p, reason=WHY, apply=True)
        item = yaml_io.read(E.find_item(p, "D", "actions", "Spawn"))
        check("보호 표식이 붙는다", item.get(E.PROTECT_FIELD) is True, str(item))
        # [중요] 247건에 "사람이 검수했다" 를 찍으면 진짜 검수한 것을 구분할 수 없게 된다
        check("[중요] verified_by_user 는 건드리지 않는다",
...
def test_protection_survives_resynthesis() -> None:
    """[중요] 효과는 검수 잠금과 같아야 한다 — 재합성이 덮지 않는다."""
    with tempfile.TemporaryDirectory() as t:
        p = _seed(Path(t))
        E.protect_all(p, reason=WHY, apply=True)
        ...
        check("[중요] 보호된 것은 안 덮인다", got["description"] == "스냅샷 설명", str(got))
        check("[중요] 보호된 것은 안 지워진다 (prune 대상 밖)",
...
def main() -> int:
    for fn in (test_plan_then_apply, test_reason_is_required, test_not_the_same_as_verified,
        ...
        fn()
    total = PASS + FAIL
    print(f"{'OK' if not FAIL else 'FAIL'} test_protect: {PASS}/{total} 통과")
