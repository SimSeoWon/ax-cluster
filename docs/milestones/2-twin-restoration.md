...
## 작업 분류 — 대 2 · 중 8 · 소 33 → **완료 14 / 33**

### 대 1. 트윈 데이터 생성 (목표 ①) — 14 / 23 · 🔴 **이번 마일스톤 메인**
...
트윈을 **먹는 쪽**이다(`cluster_context.py` = *"매니페스트 수집 RAG+온톨로지"*). 컨텍스트
문서는 이제 자라지만 **온톨로지 채널이 아직 비어 있다**(중 1.3 의 LLM 합성 미완). 그 상태로
세우면 매니페스트가 **RAG 절반만 든 근거**로 코드를 짜게 한다. 이름·규모만 남긴다.
...
1. **"완료" 는 영역 전체에만.** 부분이면 분모를 같이 적는다 (`14 / 33`)
