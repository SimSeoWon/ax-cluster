...
SECTIONS = (
    ...
    ("핵심 책임", "- "),
    ("도메인 경계", "[중요] **무엇이 이 도메인이 아닌지** 적는다. 비워 두면 합성이 다른 "
                    "도메인의 클래스를 끌어온다(실측)."),
    ("핵심 invariants", "- "),
    ("협력 도메인", "- "),
    ("사용자 의도 메모", ""),
...
class CreateResult:
    ...
    def summary(self) -> str:
        ...
        if self.parent:
            s += f" · 상위 {self.parent}" + ("" if self.linked else " ([중요] 연결 실패)")
        if self.notes:
            s += " · " + "; ".join(self.notes[:2])
        return s
...
def render(domain: str, *, tags=(), parent: str = "", summary: str = "",
           collaborators=()) -> str:
    """씨앗 본문. [중요] **빈 절도 남긴다** — 없는 절은 다음 사람이 있는 줄도 모른다."""
    fm = ["---",
...
def create(paths: ProjectPaths, name: str, *, tags=(), parent: str = "",
    ...
    if parent_name and not md_path(paths, parent_name).exists():
        # [중요] 없는 상위를 조용히 받아 두면 계층이 끊긴 채로 남는다
    ...
    if not tags:
        # [중요] 실측: 태그 채널(가중치 3.0)이 한글 0개라 한글 질의가 최고 가중치를 놓친다
