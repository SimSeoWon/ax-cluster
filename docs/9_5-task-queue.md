...
### 9.5.6 ✅ 해소 — LAN 개방 + 인증 계층 (2026-08-08)
...
| 방식 | 평가 |
|---|---|
| `ufw allow 8101` (Anywhere) | ❌ **여전히 금지.** 토큰이 생겼어도 인터넷 노출은 별개다. Gitea 처럼 라우터 포트포워딩까지 걸리면 큐를 누구나 조작한다 |
...
**✅ LAN 한정으로 개방 완료 (2026-08-08, 사용자 지시):**
...
| 검증 | 결과 |
|---|---|
| 규칙 범위 | ✅ `8101/tcp ALLOW IN 192.168.0.0/24` — 출발지 한정(Anywhere 아님) |
| 실제 LAN 호스트에서 도달 | ✅ BC-250(192.168.0.43)에서 `/api/v1/health` 200 |
| 쓰기 경로 | ✅ 같은 곳에서 work 등록 성공 |
| 부팅 시 유지 | ✅ `ufw`·`ax-task-queue` 둘 다 `enabled` |
...
✅ **인증 계층이 생겼다 (2026-08-08).** 이제 방어선이 둘이다 — 토큰과 방화벽.
