...
## [중요] 실패는 커밋하지 않는다 (소 1.4.4) — **정본이다** (`#317` 미결 ⑦ 결정 2026-08-28)

> [중요] **「나란히 서기」가 닫혔다.** 2026-08-14 에 이 자리는 *"두 파일은 나란히 선다(§8.4
> 규약: 측정으로 뒤집힐 여지가 있는 동안 되돌릴 수 있게)"* 였다 — attempt 계층(커밋 후 검증)과
> 통합자(검증 후 커밋)를 둘 다 살려 두고 측정을 기다린 것이다.
> **측정이 났다**: attempt 배선의 프로덕션 호출자 **0** · bare 에 attempt 브랜치 **0** —
> 한 번도 안 돌았다. 사용자 결정으로 이 파일의 규칙이 정본이 됐고, 안 도는 층은 `#327` 로
> 걷었다. `docs/8-git-authority.md` §8.4 도 같은 말을 하도록 고쳤다.

옛 규칙은 *"실패도 attempt 로 push 한다 — 증거를 버리지 않는다"* 였다. **증거를 버리지 않는다는
원칙은 그대로이고, 남기는 자리가 git 이 아니라 스풀이다.** 대신:
...
def run(paths, facts, work_id: str, *, project: str = "", durable: str = "",
    ...
    🔴 [주의] **이 함수의 전제가 정본 7단계에 밀렸다** (사용자 확정 2026-09-02).
    ⑸ 는 **워커가 자기 서브 브랜치에 커밋**하고 ⑺ 은 **리더(`.33`)가 그 브랜치들을 모아
    적용·빌드**하는 것이다. 즉 조각 durable 은 「매니페스트 운반 전용」이 아니라 **코드가
    실리는 자리**이고(매니페스트는 2026-09-02 에 통째로 걷었다), 이 함수가 base 에 순차
    누적하는 모양은 되돌릴 목록에 있다(`#348`). 아래 서술은 **지금 도는 것**이다.

    [주의] 종전 근거: 통합자가 격리 트리 하나에 조각들을 순차 누적하므로 조각 2의 커밋이
    조각 1의 변경을 이미 포함한다 — 그것을 조각 2의 durable 로 밀면 남의 조각까지 실린다.
    그 사실 자체는 맞지만, **누가 쓰는가**가 바뀌면 전제가 사라진다.
    """
