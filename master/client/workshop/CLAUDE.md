...
| "X 시스템 추가", "Y 기능 만들어줘" — **신규 파일 3개 이상 + 클래스 간 상속/포함/import 관계** 또는 **여러 클래스가 묶이는 신규 시스템** (서버 PC 만 — `server_mode: true` 또는 단독 모드) | `/distribute` 스킬 | feature 브랜치 + 스켈레톤 + work/task 등재 + UE regen + 빌드 |
...
#### `code-writer` vs `/distribute` 분기 (사용자 확인 필수)
...
> - a) `/distribute` 스킬로 분산 처리 (feature 브랜치 격리 필수 + 스켈레톤 + task_queue 등재 + UE regen + 빌드 + 주간 리뷰)
...
이유: `code-writer` 는 단일 클래스·기존 코드 수정용. 다중 클래스 생성을 시키면 의사코드 단계 없이 본문까지 한 번에 써버려 분산 시스템이 무의미해진다. 반대로 모든 다중 클래스 작업을 자동으로 `/distribute` 로 보내면 빠른 일회성 작업까지 큐에 박히는 오버헤드. **결정은 사용자**.

**`/distribute` 진입 후 브랜치 격리 거부 시**: 스킬 자체적으로 종료됨 (main 보호가 양보 불가 안전장치). 격리 없이 진행하려면 a 가 아니라 b 선택해야 함.
...
#### `/distribute` 발동 조건 — 서버 PC 한정 (BLOCKING)

`/distribute` 는 **서버 PC 에서만** 발동한다. 호출 전 `config.json` 의 `server_mode` 와 `context_server_url` 을 확인할 것:
...
work_id: null or "<분산 작업 work_id>"            # /distribute 작업이면 채움
...
| 서버 | 실행 파일 | 주요 툴 |
|------|-----------|---------|
| `ax-client` | `py .ax/lib/axmaster/clientside/client_mcp.py` (마스터가 배달·손대지 말 것) | 조회 9: `search_context`, `list_domains`, `get_domain`, `get_domain_layer`, `get_object_spec`, `get_action_spec`, `find_invariants_by_class`, `list_works`, `get_work` · 쓰기 대행 5: `add_object_alias`, `mark_not_a_class`, `redmine_note`, `log_writer_signal`, `log_history` |
...
| `code-recipes` | `.claude/mcp/code_recipes.exe` | `log_writer_signal`, `find_recipes`, `get_anti_patterns` (code-writer 패턴 누적·합성 + 안티패턴 카탈로그) |
