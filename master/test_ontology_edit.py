...
지키는 계약: [중요] **사람이 손댄 것은 재합성이 덮지 않는다. 그리고 손대지 않은 것을 만들지도
...
def test_edit_locks() -> None:
    with tempfile.TemporaryDirectory() as t:
        ...
        check("[중요] 편집은 곧 잠금이다", r.locked, r.summary)
        ...
        # [중요] 값이 같으면 changed 에 넣지 않는다 — "바꿨다" 는 보고가 거짓이면 안 된다
...
def test_immutable_fields() -> None:
    """[중요] `name`·`layer` 는 파일 경로의 근거다 — 패치로 바꾸면 파일과 내용이 어긋난다."""
    with tempfile.TemporaryDirectory() as t:
        p = _seed(Path(t))
        r = E.edit(p, "D", "objects", "AMonster",
                   {"name": "ADragon", "layer": 3, "file": "d.h"})
        check("[중요] name/layer 는 무시한다", sorted(r.ignored) == ["layer", "name"], str(r.ignored))
        # [중요] 조용히 무시하면 호출자가 바뀐 줄 안다 — 보고해야 한다
        ...
        check("[중요] patch 로 잠금을 못 바꾼다", E.LOCK_FIELD in r2.ignored and r2.locked, str(r2.ignored))


def test_never_creates() -> None:
    """[중요] 없는 항목을 편집하면 **만들지 않고 예외** — 오타가 유령 항목을 낳으면 안 된다."""
    with tempfile.TemporaryDirectory() as t:
        p = _seed(Path(t))
        try:
            E.edit(p, "D", "actions", "없는것", {"description": "x"})
            check("[중요] 없는 항목은 만들지 않는다", False, "조용히 만들었다")
        except E.EditError as e:
            check("[중요] 없는 항목은 만들지 않는다", "찾지 못했다" in str(e), str(e)[:60])
...
def test_unlock() -> None:
    """[중요] 잘못 건 잠금이 영구가 되면 그 항목은 영영 낡는다."""
    with tempfile.TemporaryDirectory() as t:
        p = _seed(Path(t))
...
def test_remove() -> None:
    ...
    with tempfile.TemporaryDirectory() as t:
        ...
        check("[중요] 잠긴 것은 force 없이 안 지운다", not r2.removed, r2.summary)
        ...
        check("[중요] 잠긴 것을 지웠다고 티를 낸다", "force 로 지웠다" in r3.summary, r3.summary)
...
def main() -> int:
    for fn in (test_edit_locks, test_immutable_fields, test_never_creates,
        ...
        fn()
    total = PASS + FAIL
    print(f"{'OK' if not FAIL else 'FAIL'} test_ontology_edit: {PASS}/{total} 통과")
