...
드러났다). 두 축을 섞지 말 것:
    ...
    색인·합성 쪽   [완료] `#242` — `consumer.process_event` 가 **git 을 부르기 전에** 막고,
                   빈 스풀 폴링도 마운트된 하나만 돈다. **`active` 가 비어도 막는다**(fail-closed)
    요청 표면 쪽   [실패] `resolve_mounted_only()` 프로덕션 호출자 **0** · `claim` 에 프로젝트
                   축이 없다(`ClaimReq` 에 필드 없음, `claimer.py:93` 이 값을 읽고도 안 보낸다)
                   → 3단계 태그 계약(`#257`·`#247`·`#258`·`#248`·`#243`)

[주의] 그래서 지금 **워커가 남의 프로젝트 태스크를 집을 수 있다.** 단일 프로젝트인 동안은
사고가 없지만, 그것이 `NS` 에 claimer 상주를 켜지 않은 이유다(리포트 31·32).
