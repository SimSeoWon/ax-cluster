...
def main(argv=None) -> int:
    ...
    if once:
        ...
        #
        # [중요] **잠금 범위는 체크아웃이고 위 근거는 큐다 — 프로젝트가 둘이면 어긋난다**
        #    (`#251` 실측 2026-08-22). `find_root()` 가 체크아웃 루트를 돌려주므로 같은 기계에서
        #    ModularStage 상주와 NS 상주가 **각자** 잠금을 얻는다. 그러면 위 주석이 경고한
        #    「같은 큐를 두 입이 빤다」가 그대로 일어나고, claim 에 프로젝트 축이 없어(`#248`)
        #    서로의 태스크를 집는다.
        #    [주의] 그래서 `#248` 이 켜지기 전에는 **한 기계에 두 프로젝트의 상주를 켜지 않는다**
        #    (NS 에 claimer 를 등록하지 않은 이유). `#248` 뒤에는 체크아웃 단위가 옳다 —
        #    프로젝트마다 자기 몫을 병렬로 집는 것이 `#250`(설치는 여럿) 의 목적이다.
