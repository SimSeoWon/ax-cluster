...
[중요] **왜 조정자가 따로 필요한가.** 벡터와 BM25 는 각자 재색인할 수 있지만, 각자 뒤집으면
...
def _canary(paths: ProjectPaths, note: str) -> None:
    """[중요] **여기서 삼킨 실패는 self-heal 되지 않는다** (소 3.5.6 · `#182`).
    ...
    [주의] **fail-soft 는 그대로 둔다** — 도메인 색인이 죽어도 컨텍스트 검색은 멀쩡하고, 그래서
...
def rebuild_all(paths: ProjectPaths, *, progress=None) -> RebuildStats:
    ...
    #    [중요] **삼키지는 않는다** — 사유를 `domain_note` 로 들고 나와 요약에 찍는다.
    ...
    try:
        ...
        note = f"[중요] 도메인 색인 실패: {e}"
        _canary(paths, note)
    except Exception as e:                              # noqa: BLE001
        note = f"[중요] 도메인 색인 오류: {type(e).__name__}: {e}"
        _canary(paths, note)
