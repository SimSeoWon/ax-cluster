...
# ~/ax-data/config.yaml
version: 1
active: sim/<ue5-repo>          # 🔴 마운트 대상. 비어 있거나 projects 에 없으면 기동 실패
projects:                       # 추적 중인 프로젝트 목록 (디렉토리와 1:1)
  - sim/<ue5-repo>
```

```yaml
# ~/ax-data/projects/sim/<ue5-repo>/project.yaml
version: 1
project_id: sim/<ue5-repo>      # 디렉토리 경로와 반드시 일치
gitea:
  owner: sim
  repo: <ue5-repo>
  bare_path: /var/lib/gitea/gitea-repositories/sim/<ue5-repo>.git
  clone_url: http://192.168.0.57:3000/sim/<ue5-repo>.git
  branch: main
```
...
현재 소유자 `sim` 아래 저장소 5개(`geminiscripter`·`project_alpha`·`modularstage`·
`scenariowriter`·`servertest`) — **UE5 프로젝트 저장소는 아직 이 중에 없다.**
등록 시 실제 저장소명으로 `<ue5-repo>` 를 채운다.
