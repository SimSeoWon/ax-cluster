...
[중요] 지키는 계약 둘: **원본을 지우지 않는다** · **조용히 바꾸지 않는다.**
...
def test_particles() -> None:
    """[중요] 한국어엔 조사가 붙는다 — 토큰 일치로는 못 잡는다."""
    al = _al(("몬스터", "AMonster"))
    e = X.expand("몬스터를 스폰한다", al)
    check("[중요] 조사가 붙어도 찾는다", e.changed and "AMonster" in e.query, e.query)
    check("[중요] 원본을 지우지 않는다", "몬스터를 스폰한다" in e.query, e.query)
...
def test_short_terms_are_dangerous() -> None:
    """[중요] 한 글자 별칭은 어디에나 들어간다 — 부분 문자열 매칭의 대가다."""
    al = _al(("적", "AEnemy"), ("몬스터", "AMonster"))
    e = X.expand("목적지 설정", al)          # 「적」이 「목적지」에 들어 있다
    check("[중요] 짧은 별칭은 확장하지 않는다", not e.changed, e.query)
...
def test_cap_and_specificity() -> None:
    ...
    e = X.expand(q, al)
    # [중요] 확장은 거들 뿐 — 질의를 대체하지 않는다
    check("[중요] 개수 상한이 있다", len(e.added) == X.MAX_EXPAND, str(len(e.added)))
    check("생략한 수를 적는다", e.capped == 4 and "생략" in e.note, e.note)

    # 긴 별칭이 더 구체적이다
    al2 = _al(("태스크", "UTask"), ("미션 태스크", "UMissionTask"))
    e2 = X.expand("미션 태스크 실행", al2)
    check("[중요] 구체적인 것이 먼저다", e2.added[0][1] == "UMissionTask", str(e2.added))


def test_no_duplicate_noise() -> None:
    al = _al(("몬스터", "AMonster"))
    e = X.expand("AMonster 를 고친다 몬스터", al)
    check("[중요] 이미 질의에 있는 식별자는 안 붙인다", not e.changed, e.query)
...
def test_never_breaks_search() -> None:
    ...
    check("[중요] 예외가 검색을 막지 않는다", got.query == "몬스터", got.query)
...
def main() -> int:
    for fn in (test_particles, test_short_terms_are_dangerous, test_cap_and_specificity,
        ...
        fn()
    total = PASS + FAIL
    print(f"{'OK' if not FAIL else 'FAIL'} test_expand: {PASS}/{total} 통과")
