...
- **BC-250 은 순수 추론 노드다** — 워커가 아니다(§2.4).
...
6. **마스터는 컨텍스트를 보강해 주는 쪽이지 직접 작업을 수행하는 쪽이 아니다** (2026-08-07 확정).
   RAG·온톨로지로 매니페스트를 조립해 **밀어 넣고**(pull 아님), 실제 수행은 **에이전트/워커가 한다.**
...
- 같은 원리가 코드 생성에도 적용된다 — 마스터의 **워커 프로세스**가 수행 주체이고,
  마스터의 오케스트레이션 계층은 **매니페스트를 조립해 건네는** 역할이다.
...
### 8.2 파이프라인 워커 (마스터의 워커 프로세스 — 자율 실행)
