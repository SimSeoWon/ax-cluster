...
##### ② 이식 범위 — 🔴 **파일 이름이 아니라 기능으로 갈라야 한다** (2026-08-08 저녁 재분석)

> ⚠️ **이 절의 이전 판은 틀렸다.** `ontology_*` 4,832줄을 한 덩어리로 묶어 "2단계" 로 미뤘는데,
> 그 안에서 **합성 / 색인 / 조회 / 관리**가 갈리고 **우리에게 필요한 것은 색인·조회뿐**이다.
> 잘못 묶은 탓에 "온톨로지는 큰 작업" 이라는 오판이 오래 남았다.

**우리는 이미 온톨로지 데이터를 갖고 있다** — 247개 yaml / 7 도메인 / objects 112 · actions 72 ·
invariants 56 (실측). `--export-state` 로 받아 임포트한 것이다. 그러므로:

| 기능 | 어디 | 줄수 | 지금 필요한가 |
|---|---|---|---|
| **도메인 색인 + 규범 검색** | `mcp/context_search/domain_index.py` | **829** | 🔴 **필요. 이것만 있으면 247개가 살아난다** |
| 도메인 뷰어(웹) | `domain_viewer_routes.py` | 555 | ✅ 보고 싶다면 (§5.2-G) |
| 상태 임포트/익스포트 | `state_transfer.py` | 304 | ✅ 다음 임포트를 위해 |
| 온톨로지 **조회** 도구·라우트 | `ontology_query_tools`·`ontology_route_*`·`ontology_routes` | 1,553 | ⬜ MCP 로 노출할 때 |
| 온톨로지 **관리(CRUD)** | `ontology_manage*`·`ontology_mutation_tools`·`_task_manage` | 2,413 | ⬜ 편집이 필요해지면 |
| 드리프트·리프레시·캐시·로그태그 | `ontology_drift_mcp` 외 | 866 | ⬜ |
| **온톨로지 합성**(소스 → yaml 생성) | `watcher/ontology_*` | **7,173** | ❌ **지금은 불필요** — yaml 을 이미 갖고 있다 |
| 클래스 그래프 | `watcher/class_graph*` | 1,341 | ⬜ 도메인 자동 추론이 필요해지면 |
| 컨텍스트 MD 합성 | `watcher/context*` | 1,036 | ⬜ 소스 변경을 색인에 반영하려면 |

**핵심**: 데이터를 *만드는* 코드(합성 8,209줄)와 데이터를 *쓰는* 코드(색인·조회 829줄)는 다르다.
우리는 데이터를 받아왔으므로 **쓰는 쪽만 옮기면 된다.**
...
829줄이 곧 작업량의 상한이다 — "온톨로지 = 4,832줄" 이라는 인상이 만든 미룸이 근거가 없었다.
...
⚠️ **그 "얹힘" 이 지금 빠져 있다.** `class_graph.db` 가 없으므로 도메인 자동 추론은 항상
빈 결과다. 검색은 돌지만 **설계된 성능이 아니다** — "돈다" 와 "제대로 돈다" 를 구분할 것.
...
##### ④-1 🔴 트윈 진행 현황 — **"돈다" 와 "다 됐다" 를 구분한다** (2026-08-08)

작업 루프(등록→매니페스트→생성→층2)는 실측으로 돌지만, **그것은 트윈 위에 얹힌 얇은 층**이다.
트윈 자체의 이식률은 이렇다:

| 자산 | 데이터 | 색인 | 조회 |
|---|---|---|---|
| 컨텍스트 MD | ✅ 1,055개 | ✅ 961건 (벡터+BM25) | ✅ RRF |
| **온톨로지** | ✅ **247 yaml / 7 도메인** | ❌ **0건** | ❌ |
| 클래스 그래프 | ❌ | ❌ | ❌ (도메인 추론 항상 빈 결과) |

⚠️ **그래서 지금 생성되는 코드의 grounding 은 설계의 절반이다.** 매니페스트의
"도메인 규범" 섹션이 *"미구현"* 으로 나가고 있고, 실제로 그렇다. 층2 를 통과한 실측
결과(20.1s APPROVE)도 **규범 0건 상태에서 나온 것**이다.

🔴 **완료 표현 규칙**: "완료/완결" 은 그 영역 **전체**를 기준으로만 쓴다. 부분이면 분모를
같이 적는다 — *"작업 루프는 돌지만 트윈 색인은 2/3"* 처럼. 만든 것만 골라 범위를 좁힌 뒤
완결을 선언하면 다음 세션이 남은 일을 못 본다.
...
> 🔴 **1~5 는 2026-08-07 의 원래 제안이고, 아래 「진행 현황」이 실제 상태다.**
> 순서를 바꾼 지점이 있으므로 제안만 읽고 판단하지 말 것.

#### 진행 현황 (2026-08-08 저녁 실측)

| | 상태 |
|---|---|
| `task_queue` | ✅ 완료 — 독립 서비스 8101, 수명주기 주인 교체 포함 |
| `context_search` **검색 코어** | ✅ 완료 — 벡터+BM25+RRF, 961건 (원본 3,921줄 중 1,066줄) |
| `agy_query` (층2) | ✅ 완료 — `layer2_verify.py`, agy→claude 폴백 |
| `master_orchestrator` 분할 | ✅ 부분 — 등록·생성·인계는 `master/work/`, UE 부분은 `layer3_verify.py` |
| 이벤트 구동 (폴링 대체) | ✅ 완료 — Gitea 훅 → 스풀 → `ax-indexer.path`(inotify) |
| **도메인 색인** (`domain_index.py` 829줄) | 🔴 **다음** — 이게 없어 온톨로지 247개가 검색 0건이다 (§5.2-E ②-2) |
| 상태 임포트 (`state_transfer.py` 304줄) | ⬜ 다음 임포트 전에 |
| 도메인 뷰어 (`domain_viewer_routes.py` 555줄) | ⬜ §5.2-E ④-2 |
| 클래스 그래프 (`watcher/class_graph*` 1,341줄) | ⬜ 도메인 자동 추론이 죽어 있다 |
| 컨텍스트 MD 합성 (`watcher/context*` 1,036줄) | ⬜ 소스 변경이 색인에 반영되지 않는다 |
| 온톨로지 합성 (`watcher/ontology_*` 7,173줄) | ⬜ **당장 불필요** — yaml 을 이미 갖고 있다 |
