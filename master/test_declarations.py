...
[중요] 지키는 계약: **못 실은 것을 조용히 빼지 않는다.** 말없이 생략하면 모델이 *"선언부에
...
def test_missing_is_never_silent() -> None:
    ...
    check("[중요] 전부 못 찾아도 침묵하지 않는다", "UBar" in only.render(), only.render()[:120])
...
def main() -> int:
    for fn in (test_render_states_the_rule, test_missing_is_never_silent, test_budget,
        ...
        fn()
    total = PASS + FAIL
    print(f"{'OK' if not FAIL else 'FAIL'} test_declarations: {PASS}/{total} 통과")
