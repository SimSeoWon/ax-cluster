...
감사 **둘**이고 🔴 **전부 read-only 다. 자동 수정은 없다** (원전: *"자동 수정 X — 카나리 로그만"*).
...
MANIFEST = "domain.yaml"                 # ⚠️ 원전 `_MANIFEST_FILENAME` 의 우리 이름
...
def audit_domain_md(context_dir) -> dict:
    """`context/_domains/*.md` 일관성. 🔴 read-only.
    ...
    for p in direct + archive:
        try:
            ...
            # 🔴 조용히 건너뛰지 않는다 — 원전은 `except: continue` 였다
...
def md_gaps(result: dict) -> list:
    ...
    if result["duplicate_stems"]:
        gaps.append(f"🔴 중복 stem {len(result['duplicate_stems'])}건 (직속+archive, 정본 모호): "
                    + ", ".join(result["duplicate_stems"]))
    if result["missing_status"]:
        gaps.append(f"⚠️ frontmatter `status:` 누락 {len(result['missing_status'])}건: "
                    + ", ".join(result["missing_status"][:8]))
    if result["archive_active"]:
        gaps.append(f"🔴 `_archive` 안 `status: active` {len(result['archive_active'])}건 "
                    "(archive 시 flip 안 됨): " + ", ".join(result["archive_active"][:8]))
    if result.get("unreadable"):
        gaps.append(f"🔴 읽지 못한 파일 {len(result['unreadable'])}건 — 감사에서 빠졌다")
    return gaps
...
def audit_packages(ontology_dir) -> dict:
    """`ontology/domains/<D>/` walk → 도메인별 entity 카운트 + 레이어 분포. 🔴 read-only."""
    ...
    for pkg in sorted(root.iterdir()):
        ...
        if not (pkg / MANIFEST).exists():
            # ⚠️ 원전 판단 그대로 — *"옛 단일 yaml 잔재 또는 손상. audit 책임 밖"*.
            #    🔴 다만 **세어서 보고**한다(원전은 조용히 건너뛴다) — 몇 개를 안 봤는지 모르면
    ...
    # 🔴 갭은 「수치 0건」 신호일 뿐이다 — 원인을 구분하지 않는다 (머리말 참조)
...
def format_report(md: dict, pkg: dict) -> str:
    ...
    if md.get("error"):
        ...
        g = md_gaps(md)
        out += (["🔴 **갭 " + str(len(g)) + "건**", ""] + [f"- {x}" for x in g] + [""]
                if g else ["✅ 갭 없음", ""])
    ...
    if pkg.get("skipped"):
        out += [f"⚠️ `{MANIFEST}` 가 없어 세지 않은 디렉토리 {len(pkg['skipped'])}개: "
                + ", ".join(pkg["skipped"]), ""]
    ...
    out.append("")
    if pkg["gaps"]:
        out += [f"⚠️ **갭 {len(pkg['gaps'])}건** — 🔴 「수치 0건」 신호일 뿐이다. 사이클 미진입과 "
...
def main(argv: list | None = None) -> int:
    ...
    if args and args[0] in ("-h", "--help", "help"):
        ...
        print("  🔴 read-only 다 — 진단만 하고 아무것도 고치지 않는다")
        return 0
