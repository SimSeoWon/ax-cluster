...
🔴 **개념 계층은 사람이 요청한다** (사용자 확정 2026-08-09). 도메인 사이의 부모/자식은
**추상 → 구체**다 — `[미션 시스템]` 아래 `[미션 매니저]`·`[미션 에디터 편집창]`. 도구가
추측하지 않고 *"미션 시스템 하위 문서로 [미션 에디터]를 만들어줘"* 같은 **요청으로만**
`parent_domain` 이 정해진다(자동 승급 영구 비활성과 같은 정신).
