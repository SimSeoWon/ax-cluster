"""프로젝트 코드 규약 — [중요] **프로젝트의 `CLAUDE.md` 가 SSOT 다** (소 3.5.7).
...
코드를 **생성**할 때 지켜야 하는 것이다 — 그런데 지시서가 그것을 **전혀 나르지 않았다**
(도메인 규범(온톨로지)만 실었다).

[중요] **베껴 두면 어긋난다.** 프로젝트가 규약을 바꾸면 우리 복사본은 조용히 낡는다. 그래서 파일을
**읽어서** 싣는다 — 미러가 `git pull` 로 갱신되면 다음 골조가 자동으로 새 규약을 나른다.
[중요] 소비처는 `skeleton.py` 의 프롬프트다 (매니페스트 파일은 2026-09-02 에 걷었다).

[주의] **bounded·curated** (κ.3): 파일 전체(227줄)를 싣지 않는다. 이름을 지정한 절만 뽑는다 —
지시서가 부풀면 그것이 곧 토큰 비용이고, 원전이 *"raw 덤프 X"* 라고 못박은 자리다.

## [중요] 그리고 UE 특유의 금지 3종은 **우리가 싣는다**
...
[주의] `frozen_decls`(계약 동결)가 리네이밍의 **일부**를 막지만 **모듈 간 이동은 못 막는다** —
...
# 프로젝트 문서에서 실어 올 절 (이름 그대로). [주의] 없으면 조용히 넘기지 않고 보고한다.
WANTED_SECTIONS = ("Code conventions",)

# [중요] UE 특유라 프로젝트 문서와 무관하게 항상 싣는다 (위 절 참조).
UE_PROHIBITIONS = (
    ...
    "[중요] 위 셋은 판단 사항이 아니다. LLM 은 코드만 볼 수 있고 **UE5 에셋 의존 관계(Blueprint 상속·"
...
def extract_section(text: str, heading: str) -> str:
    ...
    [주의] 정규식으로 절을 자를 때 **다음 헤딩까지**로 끊는다 — 파일 끝까지 삼키면 매니페스트가
...
def project_doc(repo: Path) -> tuple:
    ...
    [중요] `CLAUDE.md` 는 프로젝트에서 **gitignore 대상**이다(AgentWatch 가 관리하는 로컬 문서).
