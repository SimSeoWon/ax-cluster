...
| `work/claimer.py` | [중요] **작업장 폴링 claimer** (#204, κ.0 CI 러너 — payload 로 작업장에 배달돼 그쪽에서 돈다). build 는 실행→submit(사실 보고), code(infer) 는 durable 실체화(blob 기록·재시도 5×15s — 등록↔publish 경합 실측)→claude→`result.json` 만 (**판단·submit 없음** — 태스크는 claimed 유지, 회수는 마스터 collect). 상주: `.ax/claimer.lock` 배타 잠금 싱글턴 + 감시자(.2 schtasks `AxClaimer` 5분 · .43 cron). [중요] 능력·types 는 config 실측/신고값 — 신고 안 한 유형을 훔치지 않는다. [주의] 프롬프트는 infer.py 정본과 글자 동일(테스트 대조) · 콘솔 출력은 CP949 안전(em-dash 가 프로세스를 죽였던 실측) |
...
| `work/ax_safety.py` | **작업장 상주 안전 가드** — 원전 `worker/safety.py` 글자대로 이식 (#165 부활 조건 = 상주 데몬 = claimer 가 충족). crash handler(비정상 종료 마커) · sleep guard · quick-edit guard (뒤 둘은 윈도우 전용, 아니면 **말하고** 넘어간다) |
