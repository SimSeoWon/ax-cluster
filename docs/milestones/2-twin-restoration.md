...
- 🔴 **전체 클래스를 분류하지 않는다** — **active 도메인 멤버십 합집합만**(원안 1,799 →
  24 클래스, **36배 절감**). *"도메인에 잡혀있는 것만"*. 우리 기준 = 7 도메인 112 객체
...
- **자동 4가드**(class_graph≥100·dep≥100·active≥1·미분류≥1) · **폴링 3가드**(자원·dirty
  fingerprint·skip_unchanged) · **PyYAML 0** · 디렉토리 패키지(**색인은 manifest 뿐**)
- 🔴 **액션은 함수 1:1 매핑이 아니다** — 누가/누구에게/무엇을/어떤 플로우의 **의미 단위 기술**.
  온톨로지는 컨텍스트 문서들을 **합치고 그 사이 관계·호출을 담는 개념 문서**다(사용자 정의)

| 소 | | 성격 |
|---|---|---|
| 1.3.1 | 도메인 생성 — 주제 기반 **수동**. 🔴 **상위 지정 포함**(아래) | 사람 |
...
🔴 **개념 계층은 사람이 요청한다** (2026-08-09). 도메인 사이 부모/자식은 **추상 → 구체**다 —
`[미션 시스템]` 아래 `[미션 매니저]`·`[미션 에디터 편집창]`. *"미션 시스템 하위 문서로
[미션 에디터]를 만들어줘"* 같은 **요청으로만** `parent_domain` 이 정해진다.

🔴 **멤버 수집은 대화형이다** — 도구가 **후보 클래스를 제안하고 묻는다**(*"관련 클래스로
이것들이 보인다 — 포함할까?"*, *"이 클래스들의 자식도 있다"*). 원본은 자식 1홉을 **자동**
확장하지만(`expand_seeds_with_one_hop_children`) 우리는 **제안 → 확인**이다. 휴리스틱을
최대한 모으려면 물어야 하고, 그게 시소러스 창구와 같은 패턴이다(silently auto-add 금지).

⚠️ **클래스 상속과 다른 축**이다 — `layers.py`(L1/L2/L3)는 도메인 **안의 클래스**를,
이건 도메인 **사이**를 배치한다. 실측: 받아온 7 도메인에 `parent_domain` **0건**.

🔴 **소 1.3.10 — 하위 온톨로지 문서를 object 로 갖는다** (사용자 제안 2026-08-09).
지금 objects 는 **클래스만** 담는데, 하위 문서도 담게 한다:

    [미션 시스템]  objects: [미션 매니저] · [미션 에디터 편집창] · UMissionTaskExecutor
    [미션 에디터 편집창]  objects: UEUW_InGameTaskEditor · …  (클래스 12개)

**후보 폭발의 구조적 해법이기도 하다.** 실측: 멤버 후보가 도메인당 184~190개였다.
이미 다른 도메인에 속한 클래스를 빼서 MissionEditor 18 · MissionRuntime 36 으로 줄었지만
**`GlobalEventSystem` 은 178 로 거의 안 줄었다** — 온 프로젝트가 include 하는 허브라
그렇다. 이런 허브는 클래스를 178개 물어볼 게 아니라 **하위 문서로 갈라야** 한다.

구현 시 주의 둘: ⑴ `kind: domain` 으로 종류를 구분할 것 — 레이어 추정·사실 게이트가
클래스 규칙으로 검사하면 전부 오탐이다 ⑵ **문서 참조 순환**을 막을 것(A→B→A).
