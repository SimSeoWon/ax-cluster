...
#### 5.5.4 🔴 워크숍 측 체크아웃 — 관례를 채택한다, 데몬은 두지 않는다 (2026-08-08 확정)
...
🔴 **단, 데몬이 하던 일 중 하나는 남는다.** UE5 에디터 내장 MCP 와 `gjc` 는 **loopback 전용**이라
...
```yaml
ModularStage:
  project_id: Sim/ModularStage         # ⚠️ 대소문자 — 아래 ④
  workshops:
    "192.168.0.2":
      user: janus
...
##### ⑥ ✅ loopback 프로세스 수명주기 — 세션 경계가 주인을 정한다 (2026-08-08 확정)
...
🔴 **윈도우 세션 0 은 데스크톱에 접근할 수 없다.** SSH 로 GUI 앱을 띄우면 세션 0 에서
...
|---|---|---|---|
| **Unreal MCP** (GUI 에디터 내장) | ✅ 세션 9 | ❌ **구조적으로 불가** | **세션 9 의 사람** (또는 그 세션의 Claude Code) |
| **`gjc`** (CLI 하네스) | ❌ | ✅ | **마스터** — 작업 단위 |
| **`UnrealEditor-Cmd`** (commandlet · 빌드 · RunTests) | ❌ | ✅ | **마스터** — 작업 단위 |
...
🔴 **"SSH 세션보다 오래 살아야 한다"가 나오면 그건 서비스가 필요하다는 신호다.**
...
🔴 **단, 진짜 위험은 충돌이 아니라 데이터 손실이다.** 자동화는 브랜치를 바꾸고 경우에
따라 리셋한다. 사람이 커밋하지 않은 작업을 남겨 뒀다면 그걸 날린다.
→ **fail-closed 로 막는다: 워킹트리가 더러우면 자동화는 시작하지 않는다.**
클론을 하나 더 파는 것보다 싸고, 방어하는 대상이 정확히 그 위험이다.
...
| 도구 | 상태 | 비고 |
|---|---|---|
| `node` | ✅ **v24.11.1** | `C:\Program Files\nodejs\node.exe`. `client/README.md` 의 "⬜ 미설치" 는 **낡았다** |
| `npm` 전역 | ✅ | `%APPDATA%\npm` — 현재 `gemini.cmd` 하나 |
| `claude` | ✅ 2.1.225 | `C:\Users\janus\.local\bin\claude.exe` |
| `git` | ✅ | PATH 에 있음 |
| **`gjc`** | ⬜ **미설치** | §9 의 전제. node 가 있으니 설치 자체는 막힌 게 없다 |
| **`bun`** | ⬜ 미설치 | `gjc` 가 Bun 런타임을 요구하면 선결 |

⚠️ **SSH 세션 PATH 가 최소라 `where`/`Get-Command` 로는 안 보인다** —
...
- [ ] **자동화 진입 전 더티 체크** — 워킹트리가 더러우면 시작하지 않는다 (⑥-1)
