...
## [주의] 이 파일이 하지 않는 것
    ...
    claimer 상주     [중요] 코드는 `#255` 다. **켤 수 있는지의 판정은 여기 적지 않는다** —
                     정본은 `client/init/residency.blocked_reason()` 이다 (`#311`).
                     [주의] 종전 이 자리엔 *"claim 에 프로젝트 축이 없다(`ClaimReq` 에 필드
                     없음 · `claimer.py:93` 이 읽고도 안 보낸다)"* 가 적혀 있었다 — **셋 다
                     지금은 거짓이다**: `#248` 은 닫혔고 `ClaimReq.project` 가 있고
                     `claimer` 가 `#258` 로 태그를 붙인다. 실제 제약은 **마운트 단일성**이다
                     (사용자 결정 ⓐ 2026-08-25 — 상주는 마운트를 따른다)
"""
