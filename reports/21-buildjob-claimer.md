...
## §10 병합 원격 브랜치 자동 삭제 (#219 — 사용자 결정, "네 의견대로 하자")

발단은 사용자 질문 *"메인 브랜치 머지할 때도 삭제 내가 해야해?"* — 실물 확인 결과 옛
규칙(#27)이 원격 삭제를 통째로 사람에게 남겨 두고 있었다. 원전(`cleanup_attempts`)은
머지 후 원격 ephemeral 을 서버가 지웠으므로, **병합 확인분에 한해** 원전으로 회귀:

    cleanup remote [--apply]   마스터 정본 저장소에서 gitea-write 의 attempt/*·task/* 를
                               세고, 팁이 origin/main 에서 도달 가능한 것만 삭제.
                               판정 불가(객체 부재·fetch 실패)의 방향은 언제나 **보존**
    이중 가드                   ① is-ancestor 판정 ② pre-push 훅(#125)이 삭제 순간 같은
                               판정을 한 번 더 — 병합 안 된 것이 지워질 길은 구조적으로 없다
    [중요] 미병합은 계속 사람 몫  — E2E 프로브·실패 증거가 그 부류. 안 병합된 것을 지우는
                               결정은 자동화하지 않는다

라이브 (2026-08-18): 병합 4건 삭제 (task/177293f3·49c34650·65b4fa9a·78a148b0) ·
미병합 6건 보존 보고. 테스트 3건 신규.

## §11 클러스터 배포물 위치 — 아이디어로 (#218, 사용자 보류)

"클러스터 부분도 ModularStage 에 포함?"을 셋으로 쪼갠 검토(페이로드/마스터/문서) 후,
사용자가 **저장소 옆 하위 디렉토리 배포안**을 제시하며 추후 결정으로 보류 — *"작업용
레포쪽에 브랜치 배포해서 처리하느니"*. 규약대로 일감이 아니라 아이디어 바구니로:
위키 [[아이디어수집]] 절 + 이슈 #218 (착수 조건: 팀 배포가 실제 일감이 될 때).
