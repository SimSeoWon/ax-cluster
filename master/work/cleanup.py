"""워커 찌꺼기 정리 — 🔴 **워커는 스스로 못 지운다. 마스터가 치운다** (레드마인 #27).
...
@dataclass
class HostPlan:
    """워커 한 대의 계획. 🔴 **지울 것과 남길 것을 둘 다 들고 있다.**"""
    host: str
    path: str = ""
...
def survey(facts, *, base_branch: str = "main", closed=None, runner=None) -> HostPlan:
    ...
    if rc != 0:
        # 🔴 fetch 를 못 했으면 origin/<base> 가 낡았을 수 있다 → 아무것도 지우지 않는다
    ...
    # 🔴 **형식 문자열에 공백을 넣지 않는다.** 원격 셸이 쪼개서 git 이 뒷부분을 *패턴*으로
    ...
    # ⚠️ 단 워킹트리가 깨끗할 때만 — 더러우면 체크아웃이 사람의 편집을 건드린다.
...
def _survey_dirs(facts, p: HostPlan, closed: dict, *, runner=None) -> None:
    """`.ax/work/<task_id>/` 를 훑는다. 🔴 **성공·취소만 지운다.**"""
...
def apply(facts, plan: HostPlan, *, runner=None) -> dict:
    """계획대로 지운다. 🔴 **survey 가 정한 것 외에는 아무것도 건드리지 않는다.**"""
    done = {"branches": [], "dirs": [], "parked": "", "errors": list(plan.errors)}
    # 🔴 **되돌리기를 먼저 한다** — 그래야 방금 서 있던 브랜치도 이번 회차에 지울 수 있다.
    if plan.park:
    ...
    for name, tip in plan.delete:
        # 🔴 `-D` 를 쓰되 **survey 의 ancestor 판정이 통과한 것만** 온다. 그 판정이
...
def closed_tasks(*, api=None) -> dict:
    """큐에서 `{task_id: status}`. 🔴 **못 읽으면 `None`** — 그러면 부산물은 손대지 않는다."""
...
#
# 🔴 **plan 을 먼저 본다.** 온톨로지의 `plan → dry → refresh` 와 같은 규칙이다 — 지우는 쪽은
...
def main(argv) -> int:
    ...
    for f in targets:
        ...
        if cmd == "apply":
            ...
            if done["parked"]:
                print(f"    ✅ {done['parked']} 로 복귀")
            print(f"    ✅ 브랜치 {len(done['branches'])} · 부산물 {len(done['dirs'])} 삭제")
            for e in done["errors"]:
                print(f"    🔴 {e}")
                rc = 1
            # 🔴 복귀했으면 **한 번 더 본다.** 방금까지 서 있던 브랜치는 1회차에 삭제
            ...
            if done["parked"]:
                ...
                if again.delete or again.drop_dirs:
                    print(_render(again))
                    d2 = apply(f, again)
                    print(f"    ✅ (2회차) 브랜치 {len(d2['branches'])} · 부산물 {len(d2['dirs'])} 삭제")
                    for e in d2["errors"]:
                        print(f"    🔴 {e}")
                        rc = 1
    if cmd == "plan":
        print("\n  🔴 아무것도 지우지 않았다. 실행하려면 `apply`.")
    return rc
