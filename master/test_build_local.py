"""요청자 검수 빌드 어댑터(`#135`) 계약 테스트 — [중요] **판정이 한 벌인가.**
...
되는 것**이다. 그래서 여기서 재는 것은 빌드가 되느냐가 아니라:
    ...
    ② 판정을 `parse_build_log` 와 **공유**하는가 ([중요] 종료 코드를 보지 않는가)
...
def test_build_bat_derivation_is_one_rule():
    ...
    check("[중요] Epic Games 하위를 가정하지 않는다 (실측된 경로 그대로)", "Epic Games" not in bb)
    # [중요] **두 배치가 실제로 공존한다** (프로브 실측 2026-08-16):
    ...
    check("[중요] Epic Games 하위 배치도 그대로 유도된다 (.33 실측)",
    ...
    class F:
        ue5 = UE5
    check("[중요] facts 판은 같은 규칙에 위임한다 (규칙이 두 벌이 아니다)",
...
# ── ② 판정 공유 — [중요] 종료 코드를 보지 않는다 ──────────────────

def test_verdict_ignores_exit_code():
    """[중요] 실측: `Build.bat` 은 **실패해도 rc=0** 을 낸다. 그래서 로그로 판정한다."""
...
def test_adapter_blocks_when_engine_missing():
    try:
        ...
        check("[중요] 엔진을 못 찾으면 **만들 때** 죽는다", False, "조용히 만들어졌다")
    except B.BuildSetupError as e:
        check("[중요] 엔진을 못 찾으면 **만들 때** 죽는다", True)
...
def test_adapter_reports_missing_uproject():
    ...
    with tempfile.TemporaryDirectory() as d:
        out = fn(d)
    check("[중요] uproject 가 없으면 빌드하지 않고 막는다", out["ok"] is False, str(out))
...
def test_adapter_passes_verdict_through():
    ...
    with tempfile.TemporaryDirectory() as d:
        (Path(d) / "ModularStage.uproject").write_text("{}", encoding="utf-8")
        out2 = fn2(d)
    check("[중요] 실패도 그대로 전달된다 (review 의 게이트가 이걸 본다)", out2["ok"] is False, str(out2))
...
def main() -> int:
    for fn in (test_build_bat_derivation_is_one_rule, test_command_is_shared_with_workshop_path,
        ...
        fn()
    total = PASS + FAIL
    print(f"{'OK' if not FAIL else 'FAIL'} test_build_local: {PASS}/{total} 통과")
