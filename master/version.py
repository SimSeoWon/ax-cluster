...
원전 `get_server_version` 이식. [중요] **원전이 이것을 만든 계기가 우리에게 그대로 있다:**
...
[중요] **그리고 트윈 커밋을 함께 낸다.** §8.4 가 *"durable 을 만드는 기준 커밋 == 매니페스트의
`twin_commit` == `index.last_indexed_commit`"* 로 세 값이 같음을 정의해 뒀다 — 코드 버전만
말하고 트윈 버전을 빼면, **같은 코드가 다른 트윈을 보고 있는 상태**를 못 본다.

## [주의] 「모른다」를 버전으로 접지 않는다

git 을 못 읽으면 `commit=""` 과 사유를 낸다. [중요] `"unknown"` 같은 문자열을 넣으면 소비자가
...
# [중요] 계약 버전 — **코드 커밋과 다른 것**이다. MCP 도구 이름·응답 형태처럼 **소비자가 기대는
...
def identity(*, paths=None) -> dict:
    """마스터의 정체. [중요] **모르는 값은 빈 문자열 + 사유**다 (`"unknown"` 을 넣지 않는다).
    ...
    *"같은 코드가 다른 트윈을 본다"* 를 못 본다(§8.4 의 세 값 동일 규약).
    ...
    out = {
        ...
        "commit_short": short,
        "committed_at": when,
        # [중요] 작업트리가 더러우면 **커밋이 정체를 다 말하지 못한다** — 그 사실을 싣는다
...
def compare(theirs: dict, ours: dict | None = None) -> dict:
    """상대가 말한 정체와 우리를 맞춘다. [중요] **판정을 세 가지로 가른다.**

        "same"        커밋이 같다
        "drifted"     둘 다 알고 있고 다르다 — 배달·동기가 필요하다
        "unknown"     [중요] 한쪽이라도 모른다 → **같다고도 다르다고도 하지 않는다**

    [주의] `unknown` 을 `drifted` 로 접으면 없는 드리프트를 보고하고, `same` 으로 접으면 **있는
