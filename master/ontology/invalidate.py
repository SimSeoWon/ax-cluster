...
## 우리 데이터 실측 (2026-08-15) — 이식이 값을 내는지 먼저 쟀다
    ...
    manifest md_hash   🔴 7개 중 3개만 보유 — **전부 받아온 스냅샷이 찍은 것**이고,
...
⚠️ 해시식이 원전과 같은지도 쟀다 — `UiManagement` 의 스냅샷 `md_hash` 와 우리
...
선삭제해도 잃을 것이 없지만, 🔴 **우리가 그대로 옮기면 재생성되지 않는 스냅샷 본문을
...
# CamelCase/UE 식별자 토큰. 🔴 **원전 정규식 그대로** — 바꾸면 같은 입력에 다른 무효화
...
def manifest_md_hash(paths: ProjectPaths, domain: str) -> str | None:
    ...
    🔴 원전은 정규식으로 긁지만 우리는 **이미 있는 파서**로 읽는다. manifest 를 읽는
...
class Invalidation:
    ...
    locked_kept: int = 0                              # 🔴 잠금·보호라 무효화 안 한 것
    ...
    def summary(self) -> str:
        ...
        if self.unreadable:
            s += f" · ⚠️ 못 읽어 무효화 {self.unreadable}"
        return s
