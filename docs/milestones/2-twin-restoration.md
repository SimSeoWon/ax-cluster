...
⚠️ 역방향 include 는 basename 근사(겹치는 헤더 9종).

**중 1.2 컨텍스트 문서** — ✅ **5/5 + 사실 게이트 + 색인기 배선** (테스트 102). grounding 3채널
= 관련 문서 3(요약 절) + 도메인 1(중 1.3 대기) + 관계 그래프. σ.7 게이트 · **사실 게이트**
(주석에만 있는 식별자 주장 거부 — 환각 2건 적발) · 서킷브레이커. 모델 `gemma4:e4b`.
실측·모델 비교 = [`../../machines/llm-fitness.md`](../../machines/llm-fitness.md), 사고 = 리포트 10 §8.
⚠️ 품질 표본이 얇다(3~4 그룹). **909개 일괄 재생성은 하지 않는다** — 용도는 변경분 채우기.
