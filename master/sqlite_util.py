...
[중요] **왜 필요한가.** 우리 DB 는 **두 프로세스**가 연다 — 색인기(`master.events.consumer`, 쓰기)와
...
[중요] **왜 한 자리에 모으나.** 값(10초)이 세 곳에 흩어져 있으면 어긋난다. 실제로 2026-08-14 에
...
[주의] **원전이 명시적으로 기각한 것도 옮긴다**: *"`locked` 명시 재시도 루프는 미채택 —
...
[주의] `*.db-wal` / `*.db-shm` 은 **삭제 금지**(실행 중 삭제 시 미체크포인트 쓰기 유실). 모든 프로세스
...
BUSY_TIMEOUT_MS = 10_000        # [중요] 원전 실측값 `SQLITE_BUSY_TIMEOUT_MS`. 바꾸려면 근거를 남길 것.
...
def apply(conn: sqlite3.Connection, *, synchronous: str = "") -> sqlite3.Connection:
    ...
    """
    conn.execute("PRAGMA journal_mode = WAL")      # [중요] 읽기와 쓰기가 서로 막지 않게
    conn.execute(f"PRAGMA busy_timeout = {BUSY_TIMEOUT_MS}")   # [중요] 쓰는 중이면 기다린다
    if synchronous:
