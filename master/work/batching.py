...
## 🔴 헤더와 구현은 한 몸이다 — 그러나 **basename 만 보면 안 된다**
...
⚠️ 원전(`AgentTest`)은 **정반대 방향으로 틀렸다** — `analyzer._parse_cpp_header` 가 `.cpp` 를
...
def _stem(path: str) -> str:
    ...
    🔴 **basename 이 아니라 경로다.** `Public`/`Private`/`Classes` 만 빼서 `.h`/`.cpp` 짝은
