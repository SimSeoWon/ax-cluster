"""읽기 전용 화면 서빙 — [중요] **다른 머신에서 보려면 서빙해야 한다** (중 3.2 후속).

## [중요] 왜 로컬 파일로는 안 되나 (사용자 지적 2026-08-13)
...
표면을 공유한다"** 는 뜻이고, 경고는 *서비스 증식*에 대한 것이었다. 그래서:
    ...
    [중요] **새 포트·새 유닛·새 ufw 규칙 = 0.** 이미 열려 있고 이미 인증된 8103 에 붙인다.
       두 화면(`cluster.html`·`domains.html`)이 **같은 표면**에서 나온다.

## [주의] `/view` 는 없앴다 (사용자 지시 2026-08-13)
...
[중요] `cluster.html` 을 만드는 것은 `master.status html` 이고, 서빙은 `webui.routes.cluster_page`
가 한다 — 낡음 배너·"요청이 수집을 유발하지 않는다" 원칙은 그대로다.

## [중요] 인증이 없다 (사용자 결정 2026-08-13)

> *"왜 로그인해야해?"* / *"내가 내한테 뭘 해킹한다는거야 머신 4대 운용하기 위해서 인프라를
> 만드는데"* / *"그냥 있는거 보여주는 뷰어를 원하는거라니까."*

토큰 규칙은 **조작 가능한 API** 를 위해 쓰였다 — 큐 조작·추론 소모·프로젝트 전환. [중요] 이 경로는
**아무것도 못 한다**: GET/HEAD 만, 화이트리스트된 파일만, 요청이 수집을 유발하지도 않는다.
규칙의 글자를 지키려고 **쓰이지 않는 화면**을 만드는 것이 더 나쁘다 — 마찰을 만들면 안 본다.

[주의] 남는 방어는 **ufw LAN 한정** 한 층이고 그것이 의도된 상태다. 같은 포트의 **API 경로는 그대로
토큰으로 잠겨 있다**(공개는 이 접두어에만 적용된다). [중요] `AX_VIEW_REQUIRE_TOKEN=1` 로 잠글 수
...
[주의] 그 대가로 화면이 **낡을 수 있다.** 낡은 것을 최신처럼 보이게 두는 것이 최악이므로,
...
PREFIX = "/view"
STALE_SEC = 900          # [중요] 이보다 오래된 스냅샷은 낡았다고 **화면에** 적는다
NO_STORE = b"no-store, must-revalidate"

# [중요] 화이트리스트. 값은 (파일명, 사람이 읽는 제목, 만드는 명령).
PAGES: dict = {
...
def page_path(root: Path, name: str) -> Path | None:
    """[중요] 이름이 화이트리스트에 없으면 **경로를 만들지도 않는다.**"""
...
def stale_banner(seconds: float, cmd: str) -> str:
    """[중요] 낡은 것을 최신처럼 보이게 두지 않는다."""
    when = f"{seconds / 60:.0f}분" if seconds < 5400 else f"{seconds / 3600:.1f}시간"
    return (f'<div style="{_BANNER_CSS}">[주의] 이 화면은 <b>{html.escape(when)} 전</b>에 수집된 '
...
def inject(body: str, banner: str) -> str:
    ...
    [주의] 문자열 수술이지만, 대안(파일을 다시 렌더)은 **요청이 수집을 유발**하게 만든다.
...
def index_html(root: Path) -> str:
    ...
    for name, (title, cmd) in PAGES.items():
        ...
        sec, when = _age(p)
        if sec is None:
            rows.append(f'<li><b>{html.escape(title)}</b> — [중요] 아직 만들어지지 않았다. '
                        f'마스터에서 <code>{html.escape(cmd)}</code></li>')
            continue
        mark = "[주의] " if sec > STALE_SEC else ""
        # [중요] **절대경로로 쓴다** — 상대경로는 `/view`(슬래시 없음)에서 루트로 풀려 401 이 됐다.
    ...
    return (
        ...
        + "".join(rows) +
        "</ul><div class=\"sub\">[중요] <b>읽기 전용이다.</b> 이 화면을 여는 것이 수집을 유발하지 "
        "않는다 — 새로고침이 BC-250 에 SSH 폴링을 걸면 주기 가드(120초)를 만든 이유가 사라진다. "
        "생성은 마스터에서 사람이 부를 때만 돈다.<br>"
        "[중요] <b>인증이 없다</b>(LAN 한정) — 이 경로는 조작이 불가능하고, 같은 포트의 API 경로는 "
        "그대로 토큰으로 잠겨 있다."
        "</div></body></html>")
    ...
    async def go():
        await send({"type": "http.response.start", "status": status,
                    "headers": [(b"content-type", ctype),
                                ...
                                # [중요] 캐시하지 않는다 — 새로고침이 항상 최신 파일을 보여야 한다
...
class ViewApp:
    """순수 ASGI. [중요] 새 의존성을 들이지 않는다 (미들웨어와 같은 방식)."""
    ...
    async def __call__(self, scope, receive, send):
        ...
        try:
            ...
            return await _respond(send, 503, (
                f"<p>[중요] 프로젝트를 해석하지 못했다: {html.escape(str(e))}</p>"
            ).encode())
...
class Router:
    """경로로 갈라 준다 — [중요] **한 포트, 한 유닛**. 서비스를 늘리지 않는다."""

    # [중요] 목록은 `webui.app` 이 소유한다 — 여기서 다시 적으면 또 어긋난다(실측 2026-08-13).
    ...
    async def __call__(self, scope, receive, send):
        ...
        if self.webui is not None and any(path.startswith(p) for p in self.WEBUI_PREFIXES):
            return await self.webui(scope, receive, send)
        if path == FAVICON:
            # [주의] 401 을 주면 브라우저 콘솔이 매 요청 빨개진다 — 없다고만 말한다
            await send({"type": "http.response.start", "status": 204,
                        "headers": [(b"content-length", b"0")]})
            return await send({"type": "http.response.body", "body": b""})
        # [중요] **`/view` 는 없앴다** (사용자 지시 2026-08-13: *"복각한 페이지에서 연결하고 View를
        #    제거해"*). 같은 이름의 화면이 둘이면 헷갈린다 — 클러스터 상태는 `/cluster`,
        #    도메인 뷰어는 원전 `/ontology` 하나로 모았다.
        #    [주의] 옛 주소를 404 로 두지 않는다 — 북마크가 있을 수 있으니 **보내 준다.**
        if path == self.prefix or path.startswith(self.prefix + "/"):
            ...
            return await send({"type": "http.response.body", "body": b""})
        if False:
            # [중요] **슬래시 없는 `/view` 를 `/view/` 로 보낸다** (실측 2026-08-13).
