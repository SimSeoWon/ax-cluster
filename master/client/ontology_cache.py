...
## [중요] 왜 우리에게 **더** 필요한가 (사용자 정정 2026-08-15)
...
[중요] **우리는 온톨로지가 마스터에만 있다** — 소비자가 마스터에 닿지 못하면 대안이 없다.
그래서 이 캐시는 우리 구조에서 **덜 필요한 것이 아니라 더 필요하다.**

## [중요] 진짜 소비자는 요청자(`.33`)다 — 작업장이 아니다
    ...
    requester (.33)     스킬 `ax-ontology` 가 `search_context_tool` 등 **MCP 조회**를 쓴다
                        = [중요] **클라이언트 모드 소비자.** 마스터가 멈추면 **사람이 멈춘다**

[주의] **코드 배달 경로는 아직 없다** — 우리 번들(`client/bundle.py`)은 config·CLAUDE.md·스킬
**텍스트만** 나른다(원전은 `.exe` 를 배포했고 그건 안 옮겼다). 그래서 이 모듈은 **여기(클라이언트
코드의 집)에 놓이고**, 배달은 별개 칸이다. [중요] 없는 경로를 지금 발명하지 않는다.

## [중요] 3분기 `outcome` 이 이 캐시의 핵심이다 (원전 그대로)

    "ok"           캐시 워밍(put) 후 응답 그대로 — 정상
    "absent"       [중요] **서버가 도달돼 「없다/오류」라고 답했다** → 응답을 그대로 넘긴다.
                   **폴백하지 않고 캐시하지도 않는다** — 진짜 부재 신호를 보존한다
    "unreachable"  전송 실패 → last-known-good 을 `_stale` 표기해 내준다. 없으면 `None`

[주의] **`absent` 에 폴백하면 지워진 개념이 되살아난다.** 사람이 온톨로지에서 지운 항목이 캐시에서
...
def is_error_payload(body: str) -> bool:
    """last-known-good 이 **아닌** 응답인가. [중요] **파싱 불가도 error 로 본다.**
...
class OntologyCache:
    """엔드포인트 → 마지막 정상 응답. [중요] **한 번만 초기화를 시도한다.**
    ...
    def _conn_or_none(self):
        ...
        try:
            ...
            # [중요] fail-open — 캐시가 없어도 호출자는 계속 가야 한다. 다만 사유를 남긴다.
    ...
    def put(self, endpoint: str, body: str) -> dict:
        """성공·비에러 응답만 적재(upsert). [중요] **에러는 캐시에 들어가지 않는다.**"""
        if not body:
    ...
    def get_stale(self, endpoint: str) -> str | None:
        ...
        [중요] 표기를 주입하는 것이 계약이다 — 소비자가 *"이건 낡은 값이다"* 를 알아야 한다.
    ...
    def serve(self, endpoint: str, fetch_fn, *, logf=None) -> tuple:
        ...
        if outcome == "ok":
            ...
            return body, "server"
        if outcome == "absent":
            log(f"[온톨로지/원격] {endpoint} → 서버 응답: 부재/오류 — [중요] 폴백하지 않는다")
            return body, "absent"
