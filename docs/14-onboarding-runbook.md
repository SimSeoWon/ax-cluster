...
| 6 | `conventions` | 미러의 `CLAUDE.md` — **컨벤션의 원천**. [중요] **정본에서 와야 한다** (`payload/claude_md/<P>.md` 또는 `<트윈>/claude_md.md`) — 정본이 없으면 이 단계는 `[남음]` 이다 | 워커 매니페스트의 컨벤션이 **영구히 빈다.** 에러가 아니라 침묵이다 — 워커가 관례 없이 코드를 쓴다. [중요] **더 나쁜 경우**: 정본 없이 통과하면 골격+실측이 규약 자리에 굳어 워커가 그것을 계약으로 받는다 (NS 실측 2026-09-01: *"한글 주석 0개 → 주석은 영문"* 이 골조 계약과 충돌) |
...
[주의] **온보딩이 끝나도 워커는 조용하다 — 정상이다.** 큐를 폴링하는 프로세스는 없다.
...
**「트리거 유닛이 무장돼 있나」**다:
    ...
    journalctl -u ax-dispatch -u ax-dispatch-catchup -n 30      「안 건 이유」도 찍힌다

[주의] **침묵은 정상이 아니다.** 골조를 올렸는데 `[dispatch]` 줄이 하나도 없으면 배선이 죽은
것이다 — 그 침묵이 2026-08-29~31 이틀을 태웠다(리포트 44 §1).
[주의] 세션 한도로 멈추면 **재개는 사람 지시**다(사용자 결정 2026-09-01) —
