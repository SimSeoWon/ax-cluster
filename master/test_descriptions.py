...
🔴 **LLM 은 주입으로 막는다.** 같은 세션에서 `dry_run` 이 LLM 을 안 막는다는 것을 겪었고
...
def _run(tmp: Path) -> int:
    ...
    check("🔴 멤버 없는 L2 는 안 들어간다", dm.LAYER_LABELS[2] not in p)
    ...
    check("🔴 상한을 넘으면 자르고 「외 N건」을 적는다", "외 15건" in pm, pm[-300:])
    ...
    for bad, tag in ((""              , "응답이 비었다"),
        ...
        check(f"🔴 실패를 값으로 — {tag}", g == {} and tag.split()[0] in w, f"{bad!r} → {w!r}")

    print("\n[4] 🔴 비용 가드 — 부르지 말아야 할 때 안 부른다")
    calls = []
    ...
    got = dm.read(d1)
    check("🔴 자동 초안은 verified_by_user: false", got["verified_by_user"] is False, str(got))
    ...
    check("🔴 전부 잠기면 LLM 0", r3.llm_calls == 0 and not calls, r3.summary)
    check("그 사유가 값에 실린다", "잠김" in r3.reason, r3.reason)

    print("\n[6] 🔴 환각 레이어 제거 — 멤버 없는 레이어의 서술은 버린다")
    P4 = _paths(tmp, "M4")
    _domain_md(P4, "D")
    ...
    check("🔴 본문을 갱신해도 created 는 보존", dm.read(p3)["created"] == "2026-03-04", str(dm.read(p3)))
    check("본문은 바뀐다", dm.read(p3)["body"] == "고친 서술")

    print("\n[9] 🔴 `protected` — 받아온 스냅샷 서술을 우리 초안이 덮지 않는다")
    ...
    check("🔴 is_locked 가 둘을 함께 본다", dm.is_locked(meta))
    st8 = dm.write(P8, "D", {3: "기계가 새로 뽑은 것"})
    check("🔴 보호된 서술을 덮지 않는다", st8.preserved_verified == 1 and st8.written == 0, st8.summary)
    check("본문이 스냅샷 그대로", dm.read(d3)["body"] == "받아온 스냅샷 서술")
    calls.clear()
    r8 = dm.extract(P8, "D", call=call)
    check("🔴 전부 보호면 LLM 도 안 부른다", r8.llm_calls == 0 and not calls, r8.summary)

    print("\n[10] 🔴 서술이 실패해도 재합성을 죽이지 않는다 (합성기 배선)")
    ...
    check("🔴 그리고 조용하지 않다", "노드 죽음" in note, note)
