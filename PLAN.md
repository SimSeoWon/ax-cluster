...
- ⚠️ **오케스트레이터가 셋이라 큐·git·RAG 에 동시 접근이 발생한다**(2026-08-08 정정).
  ...
  브랜치와 task_queue 를 두고 경쟁**할 수 있다. claim/lease/fencing 을 1인이라고 걷어내면
  안 된다(§2.2-5, §5.4.5). ⬜ **미결: 두 작업장이 같은 UE5 프로젝트를 보는지, 서로 다른
  프로젝트인지** — 전자면 위 경쟁이 실재하고, 후자면 라우팅(`requires:`)에 작업장 식별이 필요하다.
