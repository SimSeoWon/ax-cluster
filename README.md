...
> **현재 상태: 이식 착수.** `master/` 에 층2 판정 계약(`verdict.py`·`layer2_verify.py`, 테스트 19건)과
> **`task_queue` 이식본**(2,717줄, 기동·claim/fencing 실측 완료)이 들어갔다. `worker/`·`client/` 는 아직 스텁.
> **하드웨어·추론 노드·전원 제어는 실제로 동작한다.**
...
| 역할 | 머신 | 상태 |
|---|---|---|
| **마스터 / 인프라** | `sim-desktop` (192.168.0.57, Ubuntu 22.04) | **인프라 — 작업장이 아니다.** RAG·온톨로지·큐·추론 브로커. ✅ **task_queue systemd 가동 중**(`ax-task-queue.service`, :8101), RAG·브로커는 대기. GPU 증설 검토 중(1070 Ti) |
...
| **작업 PC ×2** | 윈도우 UE5 PC **2대** (`client/`) | **작업장** — 각각 오케스트레이터(사용자 접점) + **파일 소유·코드 적용·빌드/테스트·코드 등록** |
...
**왕복 1회 ≈ 100초**다. 보드가 2대가 되면 **모델을 보드별로 고정 배정**해 이 비용을 없앤다.
...
- **능력 기반 라우팅**(`requires: ue5`) — 빌드 게이트를 CI 러너 패턴으로 돌리기 위한 선결.
  현재 task_queue 는 `job_kind`(write/verify) 분기뿐 (이식은 끝났으나 라우팅은 미구현)
- [x] ~~🔴 task_queue 에 인증 계층이 0건~~ — **해소 2026-08-08.** 세 서비스 전부 공유
  베어러 토큰(`master/auth.py`, `~/.config/ax-cluster/token`, fail-closed). 방화벽은 그대로 2중 방어.
  8101 은 `ufw` 로 **LAN 한정 개방**돼 있고(`from 192.168.0.0/24`, BC-250 에서 도달 확인),
  출발지 한정이라 포트포워딩 사고까지 막는다. 단 **규칙을 `Anywhere` 로 넓히지 말 것**이고,
  LAN 안 기기를 신뢰할 수 없게 되면 인증을 먼저 붙여야 한다 ([`PLAN.md`](PLAN.md) §9.5.6)
- **이벤트 큐 설계** — Gitea 웹훅 전환의 전제(영속 + 단일 소비자 + 중복 합치기). 폴링이 공짜로 주던
  직렬화·재시도·합치기를 직접 만들어야 한다 ([`PLAN.md`](PLAN.md) §5.4.5)
- **`bun`/`node` 설치 — 마스터가 아니라 윈도우 PC 쪽 선결**(2026-08-08 정정). gjc 가 loopback
  제약상 파일을 소유한 윈도우에서 돌기 때문. 마스터엔 `claude`·`agy` 설치 완료로 충분
