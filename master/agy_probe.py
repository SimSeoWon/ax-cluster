...
## [중요] 기제는 옮기고 **신호원은 우리 것**으로 바꾼다

원전의 핵심 게이트는 **가상 PTY(ConPTY via pywinpty)** 라운드트립이다. 근거는 원전 환경의
실측이다 — *"agy 는 비-TTY(파이프/파일 캡처)면 출력 0 + hang."* [주의] **그 전제는 윈도우 것이고
우리는 리눅스다.** 그래서 PTY 를 전제로 박지 않고 **재서** 고른다(`pty_fallback`).

[중요] **우리 실패 모드는 따로 기록돼 있다** — 저장소 하드룰: *"위임한 백엔드의 「done」을 믿지
...
그래서 게이트를 우리 실패 모드에 맞춰 셋으로 둔다:
    ...
    ② 라운드트립 마커를 넣으면 **그 마커가 돌아오는가** (출력 0·hang 을 가른다)
                [중요] 셀프테스트의 NONCE 증명과 **같은 논리**다 — 응답이 왔다가 아니라
                   *"내가 보낸 것이 돌아왔다"* 를 본다
    ③ 산출물    파일을 쓰라고 시키면 **파일이 생기는가**
                [중요] 이것이 우리 하드룰이 겨눈 그 실패다. status 를 보지 않는다
...
class ProbeResult:
    ...
    def summary(self) -> str:
        ...
        return (f"agy 프로브 {n}/{len(self.gates)} 통과"
                + (f" · [중요] 실패: {', '.join(self.failed)}" if self.failed else ""))


def marker() -> str:
    """돌아오는지 볼 토큰. [중요] **프롬프트 밖에서 만든다** — 모델이 지어낼 수 없는 값이어야 한다."""
    return "AGYPROBE" + os.urandom(4).hex().upper()


def build_command(agy: str, prompt: str, *, model: str = "", add_dir: str = "") -> list:
    """실 호출 형태. [중요] **`-p` 다음이 프롬프트다** — 순서가 어긋나면 조용히 다르게 동작한다.

    [주의] 이 저장소의 운영 메모 둘이 여기 박혀 있다: `-p` 위치 함정, 그리고 **웹 검색을 시키면
...
def probe(*, model: str = "", timeout: int = DEFAULT_TIMEOUT, runner=None, which=None,
    ...
    # ② 라운드트립 — [중요] "응답이 왔다" 가 아니라 "내가 보낸 것이 돌아왔다"
    tok = marker()
    try:
        ...
        # [주의] 원전 환경에서 비-TTY hang 이 정확히 이 모양이었다. 우리 환경이 그런지는 **재서** 안다.
        ...
        return r
    if not out.strip():
        r.add("마커 라운드트립", False, "[중요] 출력이 비었다 — 호출은 끝났는데 아무것도 안 왔다")
        return r
    r.add("마커 라운드트립", tok in out,
          "마커가 돌아왔다" if tok in out else f"[중요] 마커가 없다 — 받은 앞부분: {out.strip()[:120]!r}")

    # ③ 산출물 — [중요] 우리 하드룰이 겨눈 실패: SUCCESS 라면서 파일을 안 쓴다
    ...
    # [중요] **status 를 보지 않는다.** 파일이 있고 그 안에 마커가 있어야 통과다.
    r.add("산출물 생성 ([중요] status 가 아니라 파일을 본다)", wrote and tok in body,
          "파일과 내용 확인" if wrote and tok in body
          else ("[중요] 파일이 없다 — 이것이 실측된 그 실패다(SUCCESS 라면서 안 씀)" if not wrote
...
def main(argv=None) -> int:
    ...
    for g in r.gates:
        print(f"  {'OK' if g.passed else 'FAIL'} {g.name}" + (f" — {g.detail}" if g.detail else ""))
    print(r.summary())
