...
| 🔴 `라운드 종결` · `전진시켜` · `서브 브랜치 머지해서 세워봐` | **작업 브랜치를 한 칸 전진** (정본 ⑺) | [중요] **`ax-advance`** |

🔴 [중요] **마지막 행을 놓치지 말 것.** 이 스킬은 전진한 것을 **`main` 에 반영**할지
판단하는 자리다. **라운드가 끝난 직후에 부르는 것이 아니다** — 그때는 `ax-advance` 다.
실측 2026-09-04: 라운드가 4/4 완주한 공지가 *"`main` 에 올린다"* 를 권했고 사용자가
*"리뷰 요청은 왜한건데? 전혀 다른 스킬을 요청한거 같은데"* 로 잡았다.

신호가 없으면 **묻는다.** 추측으로 검수 절차를 돌리면 사람 시간만 버린다.

🔴 [중요] **부르는 것은 셋이고, 고르는 것은 너다** (사용자 결정 2026-09-05). 마스터는 추론하지
않는다 — 갈래를 **입력으로** 받는다:

    ① 등록      `register_work(title, specs)`                 새 분산 작업 · round = 1
    ② 개선요청   `improve_work(work_id, round=현재+1, specs)`   ⑺ 전진 뒤 더 고칠 것이 있을 때
    ③ 마감      **이 스킬** → `finalize_work(confirm=True)`    `main` 머지 · 큐 `merged` ·
                                                              레드마인 이슈를 **「완료」로 닫는다**

마감은 **작업 전체가 끝났을 때 한 번**이다 — 사용자가 *"만족한다 · 메인에 올려라"* 라고 할 때.
라운드가 하나 끝났을 뿐이면 ②다.
