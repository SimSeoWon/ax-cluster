#!/bin/sh
# 마스터의 쓰기 표면 가드 — 🔴 **봉인을 여는 비용이지 개선이 아니다.**
#
# 🔴 **이 파일이 원본이고, `attempt.py install-hook` 이 정본 클론의 `.git/hooks/pre-push` 로
...
# 것** — 을 대신할 자리가 여기다.
#
# 🔴 왜 서버가 아니라 여기냐 (실측 2026-08-14, 리포트 16 §5):
...
# 막는 것:        그 외 전부 (특히 main)
# 삭제:           🔴 **`<remote>/main` 에서 도달 가능한 것만** (사용자 결정 2026-08-14, `#125`)
#
# 🔴 삭제 기준은 `work/cleanup.py` 가 로컬 브랜치에 쓰는 것과 **같은 것 하나**다:
#   *"그 팁이 origin/<기본브랜치> 에서 도달 가능한가"* = 정말 병합됐다 = 지워도 잃는 것이 없다.
#   도달 불가면 **그 커밋이 여기에만 있을 수 있으므로** 지우지 않는다. 판정 자체를 못 하면
#   (tracking ref 없음·git 실패) 역시 지우지 않는다 — 🔴 **fail-closed 의 방향이 "보존"** 이다.
#
# ⚠️ 실측이 이 기준을 골라 준 근거 (2026-08-14, 리포트 16 §9): 원격 attempt 4건이 **전부 main
...
while read -r local_ref local_sha remote_ref remote_sha; do
	...
	# 삭제 (local_sha 가 전부 0) — 🔴 도달 가능한 것만 통과시킨다
	case "$local_sha" in
	*[!0]*) ;;
	*)
		case "$remote_ref" in
		...
		*)
			echo "🔴 pre-push 거부: $remote_ref 삭제 — 우리 브랜치가 아니다" >&2
			status=1
			continue
			;;
		esac
		...
		if ! git rev-parse --verify --quiet "$tracking" >/dev/null; then
			echo "🔴 pre-push 거부: $remote_ref 삭제 — $tracking 이 없어 도달 여부를 못 잰다" >&2
		...
		if git merge-base --is-ancestor "$remote_sha" "$tracking"; then
			...
			echo "🔴 pre-push 거부: $remote_ref 삭제 — $tracking 에 병합되지 않았다" >&2
	...
	refs/heads/attempt/*|refs/heads/task/*)
		;;
	*)
		echo "🔴 pre-push 거부: $remote_ref → remote '$remote_name'" >&2
