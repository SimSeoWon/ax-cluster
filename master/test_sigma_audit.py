...
def test_report_only_surfaces_undocumented():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        check("🔴 사람이 볼 것은 1건", len(r.undocumented) == 1, str(len(r.undocumented)))
...
def test_syntax_error_is_reported_not_swallowed():
    with tempfile.TemporaryDirectory() as td:
        ...
        r = S.scan(Path(td))
        check("⚠️ 파싱 실패를 삼키지 않고 보고한다", len(r.parse_failed) == 1, str(r.parse_failed))
...
def main() -> int:
    for fn in (test_catches_broad_silent, test_narrow_exception_is_not_flagged,
        ...
        fn()
    total = PASS + FAIL
    print(f"{'✅' if not FAIL else '🔴'} test_sigma_audit: {PASS}/{total} 통과")
