...
🔴 **왜 있나 — 실측된 실패다.** 우리 35B 로 `WNDMontageManager` 문서를 만들었더니
...
🔴 **중 1.3 에서 재사용한다.** 인수인계 문서가 `methods` 테이블의 존재 이유를 *"온톨로지
...
class Report:
    ...
    def reason(self) -> str:
        parts = []
        if self.ghosts:
            parts.append("🔴 주석에만 있는 식별자를 살아 있는 것처럼 서술: "
                         + ", ".join(self.ghosts[:6]))
        if not self.mentioned_declared:
            parts.append("🔴 실측 선언 클래스를 하나도 언급하지 않았다 (다른 파일을 서술?): "
