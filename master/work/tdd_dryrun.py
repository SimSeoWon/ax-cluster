...
→ **분산 파이프라인(브랜치·레드마인·큐) 밖에서** TDD 기제를 검증·튜닝하는 자리다.
    ...
    ⑥ [중요] **red-green 판정** — 위반은 잡히고(red) 준수는 통과(green) 하는가

## [중요] 이 모듈의 값은 ⑥ 에 있다 — 실측으로 얻은 교훈 넷이 거기 박혀 있다
...
## [주의] 지금 입력이 없다 — 그래서 이식이 먼저다

드라이런은 `contracts` 가 등록된 **태스크 템플릿**을 소비한다(`ontology/tasks.py` · `#139`).
등록은 **사람의 자리**라 아직 0건이다. [중요] 그래도 옮기는 이유는 마일스톤 최우선 규칙이다 —
...
그래야 판정 로직을 실 LLM·실 UE 없이 잰다. [중요] 다만 *"주입은 계약을 재고 실물을 재지 않는다"* —
...
# [중요] 필터 이름 규약 — 원전 그대로. 드라이런 테스트가 실 automation 필터와 섞이지 않게 접두어를 준다.
...
def refs_text(example) -> str:
    """참조 경로. [중요] **코드를 싣지 않고 경로를 준다** — 원전 규약(example=참조 경로)이다.
    ...
    if not refs:
        return head + "\n- 참조 경로: (없음) — [중요] 이 상태면 에이전트가 API 를 지어낼 수 있다"
...
def test_gen_prompt(task_type: str, data: dict) -> str:
    ...
    [중요] **「UE 자동화 테스트 규약」 절을 반드시 싣는다** — 원전이 이것 없이 돌렸다가 생성 테스트가
    ...
    return (
        ...
        "## 참조 클래스 (실제 API — [중요] 반드시 Read 로 시그니처를 확인하고 쓸 것. 환각 금지)\n"
        ...
        "## [중요] UE 자동화 테스트 규약 (컴파일 필수 — 위반하면 빌드가 깨진다)\n"
...
def impl_prompt(task_type: str, data: dict, scenario: str, spec: str = "") -> str:
    ...
    if scenario == "compliant":
        ...
        goal = (f"{subject} 구현하되 계약 중 **후행/인수 하나를 의도적으로 위반**하라 "
                ...
                "[중요] 단 **컴파일은 되어야 한다** — 안 되면 red-green 을 못 잰다.")
    ...
    return (
        ...
        + ("\n[중요] 서술 미션의 범위를 벗어나지 말 것 — 명시된 동작만 구현한다." if spec else "")
    )


# ─────────────────────────────────────────────────────────────
# [중요] 판정 — 이 모듈의 값이 여기 있다
# ─────────────────────────────────────────────────────────────

def ue_view(result: dict, with_ue_test: bool) -> tuple:
    """`(ran, passed)` — `ran` 은 *"실제 테스트가 **판정 가능하게** 돌았나"* 다.

    [중요] **`ok=False` 는 fail-open(인프라 미가용)이라 결과를 믿지 않는다** — `ran=False` 로 본다.
...
def compute_verdict(scenario_results, with_ue_test: bool) -> dict:
    ...
    [중요] 실제 테스트가 판정 가능하게 돌았으면 그 결과를 **루브릭과 대조**해 어긋남을 problem 으로
    ...
    if comp is not None and comp_passed is False:
        if viol_caught:
            # [중요] 위반은 정상 포착되는데 준수만 실패 = 계약 과엄격보다 **샘플 자체 결함**이 흔하다
    ...
    if with_ue_test:
        for sc, r in (("violating", viol), ("compliant", comp)):
            ...
            if ut.get("build_ok") is False:
                # [중요] 컴파일이 안 되면 red-green 은 **미검증**이다. 루브릭 폴백이 이것을 조용히
...
def run(task_type: str, *, template: dict, spawn, rubric=None, build=None, tests=None,
    ...
    if not (data.get("contracts") or {}):
        # [주의] 원전과 같다 — 계약이 없으면 잴 것이 없다. **조용히 통과시키지 않는다.**
    ...
    # [중요] `ok` 는 *"하니스가 완주했다"* 이지 *"red-green 이 닫혔다"* 가 아니다 — 둘을 섞으면
