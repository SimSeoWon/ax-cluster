                     ...
                     "properties": {
                         ...
                         "specs": {"type": "array",
                                   "description": (
                                       ...
                                       "header_file · "
                                       "depends_on(**보통 비운다** — 클래스 단위 조각은 "
                                       "서로 **다른 파일**이라 병렬이다. 골조가 모든 클래스의 "
                                       "선언을 미리 다 만들어 두므로 조각끼리 기다릴 이유가 "
                                       "없다. **같은 파일**을 `[PSEUDO:N]` 뎁스로 쪼갤 때만 "
                                       "줄을 세운다) · "
                                       "requires(배열 — **능력 라우팅**이다. 추론에는 UE5 가 "
                                       "필요 없다; `ue5` 는 빌드·층3 판정을 누가 하느냐를 정한다. "
                                       "보통 비운다) · "
