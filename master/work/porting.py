...
## [중요] 실측 — 동명 매칭만으로는 절반도 못 잡는다 (2026-08-12)

    파일 1,654개 (`.h`/`.hpp`/`.cpp`)
    stem 그대로 같은 쌍            9      ← 문서에 적혀 있던 그 9쌍
    [중요] 접두사 정규화 후 같은 쌍     21     ← 2.3배. 전부 진짜 대응이었다
...
포팅이 **개명을 거쳤기** 때문이다 — 측정으로 확인된 대응 형태:
    ...
    Alpha_HexagonTileItem ↔  Alpha_HexagonTileItem  [중요] 그대로 옮긴 것은 접두사가 **남아 있다**

[주의] **퍼지 매칭은 하지 않는다.** 접두사 정규화까지가 결정적으로 방어할 수 있는 선이고, 편집
...
# [중요] 실측으로 고른 접두사 (위 § 참조). **긴 것이 먼저** — `Alpha_` 가 `Alpha` 보다 앞이어야
#    `Alpha_HexagonTileItem` 에서 밑줄이 남지 않는다.
PREFIXES = ("ModularStage", "Alpha_", "Alpha", "MS")

# 원본 모듈. [중요] 코드에 박지 않고 기본값으로만 둔다 — 다른 프로젝트는 다른 이름을 쓴다.
...
@dataclass
class Match:
    """대상 파일 하나에 붙는 원본들. [중요] **어떻게 맞췄는지**까지 들고 다닌다."""
...
def _index(root, source_dirs: tuple) -> tuple:
    """`(원본 stem→[경로], 원본 정규stem→[경로])`. [중요] **읽기 전용 — 열지 않고 이름만 본다.**

    [주의] 한 번 등록에 태스크가 여러 개면 같은 스캔이 반복된다(파일 1,654개). 프로세스 수명 동안
...
def counterparts(paths, target_files: list, *, source_dirs=DEFAULT_SOURCE_DIRS,
    ...
    [중요] **못 찾은 것은 못 찾았다고 둔다.** 억지로 붙이지 않는다 — 관계없는 원본에서 의도를
    ...
    for t in [f for f in (target_files or []) if f]:
        ...
        if len(hits) > max_each:
            # [주의] 자른 것을 말한다 — 조용한 절단은 "다 봤다" 로 읽힌다
...
def manifest_section(p: Porting) -> list:
    ...
    lines.append("이 작업은 **포팅**이다. 아래 원본이 *무엇을 하려던 것인지* 읽고, 그 의도를 "
                 "대상 파일에 **현재 구조로** 다시 쓴다.")
    lines.append("")
    lines.append("[중요] **금지 셋** — 원본은 참조일 뿐이다:")
    lines.append("")
    ...
    lines.append("")
    for m in p.matches:
        tag = "" if m.confident else " [주의] (접두사 정규화로 맞췄다 — 대응이 맞는지 먼저 확인)"
        lines.append(f"- 대상 `{m.target}`{tag}")
        for s in m.sources:
            lines.append(f"    - 원본 `{s}`")
    if p.notes:
        lines.append("")
        lines.extend(f"[주의] {n}" for n in p.notes)
    return lines
...
def _all_pairs(paths, source_dirs=DEFAULT_SOURCE_DIRS) -> None:
    ...
    print(f"stem 그대로 같은 것 {len(e)} · [중요] 접두사 정규화 후 {len(n)}")
