...
## 🔴 우리 트리는 받은 스냅샷과 갈라져 있다

    zip 에만 있는 필드   없음
    🔴 우리에만          protected · protected_at · protected_reason      (우리가 만든 것)
    형식 밖 자산         `_thesaurus.tsv`                                 (별칭 폴백 저장소)

⚠️ `aliases` 는 **원본의 필드다** — 시소러스의 원천이 *"objects yaml 의 `aliases`"* 라고 원본이
...
# 🔴 우리가 만든 필드. 호환본에서 뺀다 — 원본이 모르는 키다.
AX_FIELDS = ("protected", "protected_at", "protected_reason")
# 🔴 임포트가 **이월해야** 하는 필드. `aliases` 는 원본 것이지만 우리 등록분을 잃으면 안 된다.
...
class ExportResult:
    ...
    def summary(self) -> str:
        ...
        if self.abs_path_warnings:
            s += f" · ⚠️ 절대경로 {len(self.abs_path_warnings)}건"
        return s
...
def _readme(variant: str, res: ExportResult, project: str) -> str:
    ...
    body += ["```", "",
             "🔴 **벡터·BM25·class_graph 는 들어 있지 않다.** 로컬 연산만으로 다시 만들 수 있는",
    ...
    if variant == "compat":
        body += [
            ...
            "⚠️ `aliases` 는 **빼지 않았다.** 그건 원본의 필드다(시소러스의 원천).", "",
        ]
    else:
        body += [
            "## 🔴 임포트할 때 지켜야 할 것",
            ...
            "",
            "🔴 **새 스냅샷을 받을 때 통째로 덮으면 이것들이 사라진다.** 임포트는 파일을 쓰되",
    ...
    body += [
        "## 되돌리기",
        "",
        "🔴 `ModularStage/` 는 **git 밖**이다(gitignore). 임포트 전에 반드시 백업할 것 —",
        ...
        "python -m master.transfer import <zip>      # 🔴 계획만 (아무것도 안 쓴다)",
...
def export(paths: ProjectPaths, out_dir=None, *, variant: str = "full") -> ExportResult:
    ...
    with zipfile.ZipFile(out, "w", zipfile.ZIP_DEFLATED) as zf:
        ...
        if variant == "full":
            # 🔴 호환본의 manifest 는 **원본과 같은 모양**으로 둔다 — 여분 키가 그쪽 파서를
...
def export_both(paths: ProjectPaths, out_dir=None) -> list:
    """두 벌 다. 🔴 **compat 만 받아 가면 AX 상태를 못 되살린다** — README 에 적어 둔다."""
...
class ImportPlan:
    ...
    def summary(self) -> str:
        ...
        if self.removed_here:
            s += f" · ⚠️ zip 에 없는 우리 항목 {len(self.removed_here)}(남겨 둔다)"
        if self.backup:
            s += f" · 백업 {self.backup}"
        return s
...
def import_(paths: ProjectPaths, zip_path, *, apply: bool = False,
    ...
    """zip 을 우리 트리에 들인다. 🔴 **기본은 계획만.**

    🔴 **통째로 덮지 않는다.** 들어온 파일을 쓰되 우리 필드(`protected*`·`aliases`)를
    **이월**한다 — 원본의 `force` 는 백업 후 통째 교체라 그대로 하면 보호 표식 240건과
    별칭이 사라진다. 재합성 경계에서 이미 같은 문제를 풀었다(`package.carry_human_fields`).

    🔴 **zip 에 없는 우리 항목은 지우지 않는다.** 상대가 안 보낸 것인지 지운 것인지 여기선
    ...
    with zipfile.ZipFile(zp) as zf:
        ...
        try:
            ...
            plan.notes.append(f"🔴 {MANIFEST_NAME} 을 읽지 못했다 — 형식이 다를 수 있다")
        ...
        # 🔴 `ModularStage/` 는 git 밖이다 — 쓰기 전에 반드시 백업한다
        stamp = datetime.now().strftime("%Y%m%d-%H%M%S")
        ...
        for rel, member in sorted(incoming_yaml.items()):
            ...
            if isinstance(data, dict) and keep:
                for k, v in keep.items():
                    data.setdefault(k, v)          # 🔴 들어온 값이 있으면 그쪽을 존중한다
        ...
        # 🔴 형식 밖 자산은 zip 에 있을 때만 덮는다 — 없다고 지우지 않는다
        if SIDE_FILE in names:
            (paths.ontology / SIDE_FILE).write_bytes(zf.read(SIDE_FILE))

        plan.applied = True
        plan.notes.append("🔴 임포트 뒤 재색인이 필요하다: python -m master.context_search.rebuild")
    return plan
...
#   python -m master.transfer import <zip> [--apply]    🔴 기본은 계획만
#
# 🔴 `ModularStage/` 는 git 밖이라 임포트가 되돌릴 수 없다 — `--apply` 는 **먼저 백업**한다.
...
def main(argv) -> int:
    ...
    if cmd == "export":
        ...
        for r in export_both(paths, rest[0] if rest else None):
            print("  " + r.summary)
            print(f"      {r.path}")
        print("\n  🔴 compat 으로는 AX 상태를 복원할 수 없다 — 그 용도는 full 이다.")
        return 0
    if cmd == "import":
        rest = [a for a in argv[1:] if not a.startswith("-")]
        if not rest:
