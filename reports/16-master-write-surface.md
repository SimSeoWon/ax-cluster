...
> 🔴 **정정 (같은 세션) — 나는 이것을 `#167`(개명 cascade)의 실증이라고 적었고, 그것은 틀렸다.**
>
...
### §12.6 🔴 우리에게는 원전에 없는 층이 있다 — 「한글 베이스」 (사용자 지적)
...
있었어"* 라고 했다. 전수로 재니 **내 진단이 틀렸다.**
    ...
    BM25 KR 라우팅     bm25kr_* 테이블 존재 · 🔴 AX_BM25_KR **꺼짐**

한글 별칭 10개는 **현행 클래스에 정확히** 붙어 있다(`UManager_Mission`=미션 매니저 ·
`UMissionTaskExecutor`=태스크 실행 …). 🔴 **기제는 작동한다** — `ax-ontology` 스킬 실측:
...
#### 🔴 내 진단이 두 번 틀렸다
    ...
    ❌ 그래서 처방이 "문서 112건 이동" 이었다
       → 실제 원인은 **시소러스 폴백 한 줄**: 「미션시스템」 → UAlphaManager_Mission(레거시).
         온톨로지에는 「미션 매니저」 → UManager_Mission(현행)이 제대로 있다.
         🔴 이동했다면 별칭은 그대로 남아 여전히 레거시로 확장되고, 대상 문서만 사라져
         **검색이 더 나빠졌을 것**이다

🔴 **그 TSV 한 줄을 「낡았다」고 판정하지 않았다** — `add_alias_tool` 은 **사람이 답한 것만**
...
🔴 **순수 복각 먼저, 한글 베이스와의 정합은 그 뒤** → `#194`(중 3.7) 신설, 분모 77 → 78.
맞춰야 할 지점 넷을 그 이슈에 적었다(분석기 언어 갭 처방 · `has_ue_prefix`/`query_shape` 의
영문 전제 · 시소러스 두 경로의 우선순위 · `AX_BM25_KR` 판단).

⚠️ **이 순서를 택하면 `#160` 2단계는 이미 그 형태로 끝나 있다** — 기제는 이식됐고(테스트 41/41)
**적용은 안 했다.** 🔴 **이식(기제)과 적용(정책)을 가른 것이 이 경우에 값을 냈다**: 기제는
남았고 잘못된 적용은 일어나지 않았다.

### §12.7 σ.9.0 4분기 (`#160` 3단계) — 🔴 원전 신호원이 우리 환경에서 무의미했다

빈 문서를 소스 활동 기준으로 4분기한다. 🔴 **「빈 문서 N건」을 「고쳐야 할 N건」으로 읽으면
틀린다** — 그것을 가려 주는 장치다.

    recent_but_empty  🔴 소스가 최근 바뀌었는데 문서가 비었다 — 원전이 Level 5 라 부른 것
    old_inactive      소스가 오래 조용하다 — 트리거 정책의 **정상 산물**
    orphan_md         링크는 있는데 그 파일이 없다 (좀비)
    no_source_link    링크 자체가 없다

#### 🔴 첫 판은 39건 **전부** 결함이라고 했다 — 계측 오류였다
...
**전부 틀렸다.** 무료 신호 둘이 갈랐다:
    ...
    🔴 ⓓ    `infra_state_20260808.zip` 로 실려 온 **원전 환경의 σ.7 손상 유산**
...
원전은 팀원 PC 가 로컬 데이터를 가질 수도 있는 3모드 구조였다. 🔴 **우리는 온톨로지가 마스터에만
...
#### 🔴 3분기 `outcome` 을 합치면 지워진 개념이 되살아난다

    ok           캐시 워밍 후 응답 그대로
    absent       🔴 서버가 도달돼 「없다」고 답함 → **폴백·캐시 안 한다**
...
### §12.9 소 2.3.1 (`#142`) — 대조: 🔴 우리가 더 큰데 능력 5개가 미이식이었다

원전 `cluster_selftest.py` **416줄** vs 우리 `work/selftest.py` **595줄**. 🔴 **줄 수가 완성도를
말하지 않는다** — 능력 단위로 맞추니:

    ✅ 이식됨      worktree 격리 · base1/base2 2단 커밋 · work 등록·종결
    ✅ 더 촘촘함    NONCE 왕복(우리 50 : 원전 33) · PSEUDO 제거 확인
    🔴 미이식 5    폴링 중 워커 수(`last_seen`) · `force_backend` 태깅 ·
...
같은 워커의 순차 처리를 병렬로 **오판한다.** 🔴 우리 실측 *"워커 2대 = 12% 빠르고 90% 비쌈"* 을
이 장치가 **자동으로 재는** 것이다.

② **`force_backend` + 백엔드별 격리 검증.** 원전 근거(사용자 2026-07-05): 로컬 LLM / agy 가
**각각** 실제 파일 편집을 내는가. 🔴 실측 주석까지 달려 있다: *"`task_data` 안에 넣는다 —
...
갭 1건 · 패키지 7도메인 obj112 act72 inv56. 🔴 그 갭이 **이전 마일스톤의 한글 지원과 만나는
...
버전**. 원전 판단 넷이 전부 「과설계 방지」다 — 세션 강제 안 함 · 🔴 메타는 staging **밖**
(안이면 rename 때 pkg 오염) · 라이브 변경을 **막지 않고 알림** · 백업 1세대.
⚠️ **여기서 `mtime` 을 쓰는 것은 σ.9.0 과 다르다** — 비교 대상이 `domain.yaml`, **우리 도구가
직접 쓰는 파일**이고 git checkout 이 아니다. 🔴 **같은 도구라도 어디에 쓰는지가 판정을 바꾼다.**

### §12.11 🔴 한글 지원은 「충돌」이 아니다 — 보관하고 복각 후 머지 (사용자 확정)
...
> 메모를 남겨뒀다가, 복각이 끝난 뒤 머지**하자."* (사용자 2026-08-15)
    ...
    ontology/create.py   🔴 한글 도메인명 **거부** — "경로·URL·검색 식별자에서 깨진다"
    webui/board.py       ✅ 한글 도메인명 **허용**(의도). 실측 기록: "[A-Za-z0-9_-] 로 좁혔더니
...
능력 단위로 맞추니 **이식할 것이 하나로 줄었고, 그것도 머지 후 결정**이 됐다(사용자 지시).
    ...
    edit_ontology_item / remove_ontology_item   ✅ **이미 있다** (잠금 보호까지)
    remove_domain_members 의 **DB 절반**          🔴 **대상이 없다** (아래)
    edit_domain_field                            🔴 없다 — 유일한 후보, **머지 후 결정**

🔴 **DB 절반을 옮기면 우리가 없앤 드리프트를 되살린다.** 원전은 `class_ontology` 표에
`DELETE` 를 치는데 실측하니 그 표는 **행 0 · 쓰는 코드 0건**이다. 우리 `graph/db.py` 가 의도를
적어 뒀다: *"스키마를 처음부터 완성형으로 둔다 … 빈 `class_ontology` 는 원본의 구 DB 호환
경로와 같은 결과(빈 추론)를 낸다."*

⚠️ **그리고 마일스톤 문구를 정정했다** — *"DB 온톨로지 표는 **안 옮겼다**"* 는 부정확하다.
정확히는 **스키마는 옮겼고 채우지 않았다.** 🔴 **「안 옮겼다」와 「옮겼지만 안 쓴다」는 다르다** —
...
🔴 **반복된 형태 하나: 「기제는 옮기고 신호원·구성요소는 우리 것」.** σ.9.0 의 `mtime`→git ·
폴백 임베더 · 버전 핸드셰이크의 exe 빌드번호→git 커밋 — 셋 다 같은 판단이다. ⚠️ 그리고
스테이징 세션에서 **그 반대**가 나왔다(`mtime` 을 그대로 써도 되는 자리) — **같은 도구라도
어디에 쓰는지가 판정을 바꾼다.**

### 🔴 내가 틀렸다가 잡힌 것 여섯 (전부 함정 목록에 등재)
...
🔴 **셋은 사용자 지적이었다.** 그리고 ②③⑥ 은 전부 *"우리 것을 모르고 원전 기준으로만 봤다"* 는
