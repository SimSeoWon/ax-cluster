...
### 6.7 🔴 실제 사망 1건 — 감지는 됐고, **브로커가 회복을 시도하다 악화시켰다** (2026-08-15)

§6.5 의 *"빈도는 미지수"* 에 첫 데이터가 생겼다. 온톨로지 배치 중 BC-250 이 죽었다:

    03:04  OOM 킬러 · 03:11 저널 정지 · 03:22 **보드 자체 재부팅** · 이후 SSH 배너 정지 ~10분
    감지    ✅ 브로커가 unhealthy 로 내리고 그 모델 요청을 503 으로 거절했다 (설계대로)
    페일오버 ❌ 불가 — 35B 는 그 보드에만 있다. **모델 이름이 노드를 고르는 구조의 대가**다
...
| 「상주 없음」 ↔ 「못 읽었다」 혼동 | `/api/ps` 실패도 `resident=None` → 힘들어하는 노드에 **35B 적재**를 걸었다 | `resident_known` — 🔴 **판정 불가는 보존** |

⚠️ **자동 전원 차단(§6.6)을 붙일 때 이 사례를 기준으로 삼을 것** — 이 보드는 **핑도 TCP accept 도
정상인데 유저스페이스만 멈춘** 상태로 10분 이상 있었다. 「무응답」 판정을 핑·포트로 하면 못 잡고,
반대로 성급히 끊으면 이번처럼 **스스로 재부팅해 살아나는** 경우를 죽인다.

🔴 **그리고 §6.5 의 마지막 줄이 실제 질문이 됐다** — 원인이 특별한 사고가 아니라 **평소 부하가
