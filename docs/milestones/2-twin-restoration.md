...
`source_commit` 갱신 → `compute_stale_domains` 가 "오브젝트 저장값 ≠ 문서값" 으로 stale 판정.
**컨텍스트 문서가 없으면 온톨로지는 자기가 낡은 줄 모른다.**
그리고 **LLM 을 쓰는 것만 게이트 뒤**다 — *"event-based race 본질 해소"*.

## 🔴 이번 마일스톤의 메인 = **데이터를 만드는 것** (대 1)

대 2(소비 배선)는 목표 ②라 이 마일스톤 안이지만 **생성 뒤에 붙는다.** 그리고 그보다 위
계층은 **예정 상태로만** 남긴다 — 아래 「예정」 참조.

## 선행 조건 (태스크가 아니다 — 없으면 1.1 이 아예 안 돈다)
...
트윈을 **먹는 쪽**이다. 백엔드 `cluster_context.py` 가 하는 일이 *"매니페스트 수집
RAG+온톨로지"* 라, 생성부가 스냅샷 고정인 채 세우면 **고정된 8월 8일치 근거로 코드를 짜게
된다.** 그래서 이름과 규모만 남기고 **상세는 쓰지 않는다** — 읽는 비용이 매 세션 붙는다.
...
🔴 **골조 생성이 없다는 것만은 기억할 것** — 우리 `register_work` 는 골조를 **인자로 받는다**
(사람이 써 주는 전제). 원본은 `generate_skeletons` 가 만든다. §4.5 가 "골조 → 동결 → 병렬
본문" 으로 확정했는데 **첫 단계가 비어 있다.**
...
(+ 범위 밖 `distributed-workers.md` 425). 위 분류는 `watch.py` 실측 기반이라 유효하다.
🔴 **중 1.2·1.3 착수 전 `watcher-internals.md`** — 컨텍스트 MD 생성·LLM 폴백·서킷브레이커의
내부 동작이 거기 있다.
