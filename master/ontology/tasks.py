...
🔴 **prescriptive 다 — descriptive 가 아니다.** 원전이 이 자리에서 방향을 한 번 틀었고
...
문서와 경합해 **한쪽이 묻힌다.** ⚠️ 그래서 이 모듈은 색인기를 부르지 않는다 — 부르고 싶어지면
그 불변식을 먼저 다시 읽을 것.
...
🔴 **형식은 우리 `yaml_io` 를 쓴다.** 원전도 자기 `_yaml_dump`/`parse_domain_yaml` 을
...
⚠️ 우리 층3(`RunTests`)이 **fail-open + loud warning** 인 것은 이 프로젝트에 자동화 테스트가
...
# 🔴 이름은 **영문 PascalCase** 다 — 디렉토리·URL 경로·도구 인자에 그대로 박히기 때문이다.
...
def _header(data: dict) -> str:
    return (
        ...
        f"# 🔴 검색 인덱싱 안 함 — 이름으로 조회하는 구조 레지스트리(채널 분리 불변식).\n"
        f"\n"
    )
...
def _ordered(task_type: str, *, title, summary, related_domains, structure, example,
    ...
    return {
        ...
        "example": dict(example or {}),          # {instance, refs:[…]}  ← 🔴 참조 경로(코드 stale 회피)
        "conventions": list(conventions or []),  # [str, …]
        "contracts": dict(contracts or {}),      # {preconditions:[], postconditions:[], acceptance:[]}
        # 🔴 TDD opt-in — 기본 false. true 일 때만 게이트가 test_spec.filter 를 실제로 돌린다
        "tdd": bool(tdd),
...
def create(ontology_root, task_type: str, *, title: str = "", summary: str = "",
    ...
    """명시 스펙으로 `task.yaml` 을 만든다. 🔴 **추론하지 않는다.**"""
    safe = safe_task_type(task_type)
    if not safe:
        return {"status": "error", "reason": "task_type 이 비었다"}
    if not safe.isascii():
        # ⚠️ 이름이 디렉토리·URL·도구 인자에 박히므로 여기서 막는다. 한글은 title 로.
...
def edit(ontology_root, task_type: str, *, title=None, summary=None, related_domains=None,
    ...
    """🔴 **준 필드만 갱신한다** — 통째 덮어쓰기가 아니다. 안 준 필드는 보존된다."""
    safe = safe_task_type(task_type)
    path = task_path(ontology_root, safe)
...
def listing(ontology_root) -> dict:
    """등록된 템플릿 목록. 🔴 **검색이 아니라 디렉토리 스캔이다**(채널 분리 불변식)."""
...
def collect_test_filters(ontology_root, task_types) -> dict:
    """🔴 **층3 게이트의 입구** — `tdd:true` 인 템플릿의 `test_spec.filter` 만 모은다.

    원전 `executor_helpers.collect_work_test_filters` 의 판정부. 원전 규칙 그대로:

        tdd 가 꺼져 있거나 미선언   → 조용히 건너뛴다 (대부분의 task 가 여기)
        tdd 는 켰는데 filter 가 없다 → ⚠️ **말한다** — 켜 놓고 안 도는 것이 최악이다

    ⚠️ 두 번째 줄이 이 함수의 존재 이유다. 원전이 같은 자리에서 *"TDD 도는 줄 알았는데 안 돈"*
    ...
    for t in task_types or []:
        ...
        if not f:
            warnings.append(f"{t}: 🔴 tdd=true 인데 test_spec.filter 가 없다 — 켜 놓고 안 돈다")
            continue
        if f not in filters:
            filters.append(f)
