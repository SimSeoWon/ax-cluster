...
**중 1.2 컨텍스트 문서** · 🔴 LLM — **5/5 + 사실 게이트** (`master/context_synth/`, 테스트 86)

프롬프트·근거(1.2.1) · 스탬핑(1.2.2) · 프론트매터(1.2.3) · 배치 서킷브레이커(1.2.4) · 색인(1.2.5).
원본 σ.7 펜스 제거 + σ.7-B 저장 전 게이트를 옮기고 **게이트를 스탬프보다 앞에** 뒀다(원본 사고의
원인이 그 순서였다).

🔴 **품질을 모델에 맡기지 않고 결정적 게이트로 바꿨다** — 우리 35B 가 **주석 처리된 코드를 살아
있는 기능처럼** 요약했다. 그런데 같은 응답의 「개선 필요 사항」 절은 그것을 정확히 짚었다.
**사실을 모르는 게 아니라 절마다 다르게 쓴다** → 검증을 모델에게 맡길 이유가 없다
(설계 규칙: **결정적 게이트 > LLM 판단**). `verify.py` 가 주석 안에만 있는 식별자를 특정해
요약·`related_classes` 의 주장과 대조한다. 상세·오탐 방지 근거는 그 모듈 독스트링.
✅ 실제 데이터에서 **내가 눈으로 찾은 것과 같은 세 식별자**를 독립적으로 잡아 거부했다.
🔴 **중 1.3 에서 재사용한다** — 인수인계 문서가 `methods` 테이블을 *"온톨로지 추출의 오탐
교차검증"* 용이라고 적어 뒀다. 같은 환각 위험이다.

**`sample N`(dry-run)** — 스냅샷을 건드리지 않고 통과율만 잰다. 20~80초/그룹.
**미결**: ⑴ 통과율에 따라 프롬프트 조이기 vs 모델 교체(`devstral:24b` — BC-250 35B 핀을 흔든다)
⑵ 색인기에 물릴 시점. **게이트가 있어 나쁜 문서는 쌓이지 않고** 그 그룹만 문서가 안 생긴다.
