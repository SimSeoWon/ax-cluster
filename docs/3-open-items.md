...
- [ ] 🔴 **도메인 색인 이식** (`domain_index.py` 829줄, stdlib 의존만) — 온톨로지 247 yaml 이
      **검색 0건**인 원인. "미이식" 이 아니라 `sync_domain_index` **명시 호출 누락**이었다
      (§5.2-E ②-2 — 원본 `import_state` 가 이 함정을 문서로 경고해 두었다)
- [ ] **상태 임포트** (`state_transfer.py` 304줄) — 다음 `--export-state` 를 받기 전에
- [ ] **도메인 뷰어**(웹) — §5.2-E ④-2 에서 "마스터엔 화면이 없다" 판단을 뒤집었다
- [ ] **클래스 그래프** (`watcher/class_graph*` 1,341줄) — 없어서 검색의 도메인 자동 추론이
      **항상 빈 결과**다. 검색은 돌지만 설계된 성능이 아니다
- [ ] **컨텍스트 MD 합성** (`watcher/context*` 1,036줄) — 없어서 색인기가 소스 변경에
      반응하지 못한다(푸시를 받아도 재색인할 MD 변화가 없다)
...
- [x] ~~온톨로지 기반 디지털 트윈 설계 — 어떤 데이터를 온톨로지화할지~~ —
      **범위 확정 2026-08-08 (사용자 지시)**: **"RAG 부터 디지털 트윈 전반 모두 소스 부분에
      ...
      검색↔온톨로지는 서로 import 0건이라 **단계를 나눌 수 있다**(1단계 검색 3,552줄 →
      2단계 온톨로지 4,832줄).
