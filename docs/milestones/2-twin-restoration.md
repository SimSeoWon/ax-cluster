...
✅ **tree-sitter 설치 완료** (2026-08-08, `tree-sitter` 0.26.0 + `tree-sitter-cpp`).
UE 매크로 전처리가 리눅스에서 같은 결과를 내는지 **실측 확인** — ModularStage 1,654 파일
→ 클래스 1,806 · 메서드 6,380 · 2.7초.

---

## 작업 분류 — 대 2 · 중 8 · 소 32 → **완료 5 / 32**

### 대 1. 트윈 데이터 생성 (목표 ①) — 4 / 22 · 🔴 **이번 마일스톤 메인**

**중 1.1 관계 그래프** · LLM 0 · 커밋 즉시 — **4/5** (`master/graph/`, 테스트 54)

| 소 | | 상태 |
|---|---|---|
| 1.1.1 | C++ 파서 — UE 매크로 전처리·클래스/메서드 추출 | ✅ `graph/parse.py` |
| 1.1.2 | DB 코어 — 스키마·락·동시성 PRAGMA | ✅ `graph/db.py` |
| 1.1.3 | `build_full` / `update_incremental` / 질의 | ✅ `graph/class_graph.py` |
| 1.1.4 | `methods` 테이블 + **비었을 때 묻지 않는 게이트** | ✅ 6,380건 |
| 1.1.5 | `#include` 의존 그래프 build/증분 | ❌ `dependency.py` 이식 필요 |

🔴 **이식 중 원본 버그 하나를 고쳤다.** `_find_function_declarator` 가 재귀를 멈추지 않아
**중첩 클래스의 메서드가 바깥 클래스 소유로 기록된다**(실측: `AMonster` 목록에 `InnerOnly`).
`methods` 의 존재 이유가 오탐 교차검증인데 그 표에 오탐이 섞이면 게이트가 반대로 작동한다.
중첩 스코프에서 하강을 멈추게 고쳤고, 중첩 클래스는 원래대로 자기 항목으로 잡힌다.

🔴 **원본과 다르게 한 것 넷** — ⑴ 위치 `<Name>/class_graph.db` ⑵ **파서 부재·git 실패·소스
0건은 예외**(원본은 로그 한 줄 남기고 `return` → "돌았는데 결과 0") ⑶ `Source/` 경계를
pathspec + 접두 **두 겹** ⑷ 파싱 건수와 DB 건수를 구분해 **이름 중복 41건을 표시**한다.
