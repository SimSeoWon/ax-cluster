...
| 분산 워커 풀 | **윈도우 PC N대**(팀원 각자) | **리눅스 기계가 인계** |
...
```
              ...
              ▼                       ▼
┌──────────────────────────┐   ┌─────────────────────────────────────────┐
│ 윈도우 작업 PC             │   │ 마스터 (192.168.0.57, 리눅스)              │
│ = 오케스트레이터 ①         │   │ = 오케스트레이터 ② + 인프라                │
│  · Claude Code            │   │  · 워커 프로세스 N개 (파일 소유·코드 생성)   │
│    (사용자 접점)           │   │  · RAG 인덱스 / 온톨로지 / 디지털 트윈       │
│  · UE5 빌드 + RunTests    │   │  · task_queue (분배·claim·능력 라우팅)      │
│    ← 엔진 바운드, 여기만 가능│   │  · 추론 브로커 (노드 라우팅·페일오버)        │
│                           │   │  · Claude / Antigravity(agy) 호출        │
│  이 문서의 client/         │   │  · gajae-code 드라이버                    │
└───────────┬───────────────┘   └──────────────┬──────────────────────────┘
            │  공유: Gitea(git 정본) · task_queue · RAG 인덱스              │
            └───────────────────┬──────────────┘                          │
                                                    내부망 (인터넷 비노출)  │
                                        ┌────────────────┬────────────────┘
                                        ▼                ▼
...
**읽는 법:**
- **오케스트레이션 지점이 두 곳**이다. 윈도우에서 지시하면 그쪽 Claude Code 가 오케스트레이션하며
  리눅스 인프라를 병렬 처리 용량으로 활용하고, 마스터도 자기 모델 자원(Claude·agy·BC-250)으로
  독립적으로 오케스트레이션한다.
- **워커 풀은 마스터가 인계했다.** AgentTest 에서 팀원 윈도우 PC 들이 돌던
  `claim → write → submit` 루프가 마스터의 워커 프로세스로 옮겨온다(§5.2-A).
- **윈도우에 남는 것은 두 가지뿐** — 사용자 접점(Claude Code)과 **엔진 바운드 작업**
  (UE5 빌드 · `RunTests` = 3층 게이트의 층3). 다른 곳에서 할 수 없기 때문이다.
- **BC-250 은 순수 추론 노드다** — 워커가 아니다(§2.3).

### 2.2 설계 원칙 (AgentTest 2026-07-07 합의 계승)

1. BC-250 추론 노드는 파일을 직접 읽고 쓰지 않는다 — 텍스트(컨텍스트/코드조각/diff)만 주고받는다.
...
### 2.3 BC-250 은 워커가 아니라 추론 노드다 — 근거 (2026-08-07 결정)

"리눅스가 워커 풀을 인계한다"면 BC-250 도 워커여야 하지 않나? **아니다.** 인계 주체는 **마스터**다.
...
| **메모리** | 14B 적재 시 `MemAvailable` 5.3GB. 거기에 git clone + 워커 프로세스 + `claude`/`agy` CLI 를 얹으면 빠듯하고, 안정성 검증의 메모리 단조 감소(-620MB/30분)가 미해결이다 |
| **일회용성** | 노드에 CLI·자격증명·저장소 사본을 심으면 재구축이 어려워진다. 지금은 보드가 죽어도 전원만 다시 넣으면 복귀한다(§6.6) |

**따라서 병렬성은 보드를 늘려서 얻는다** — 워커 프로세스 N개가 마스터에서 돌며 BC-250 #1/#2 로
요청을 분산하고, 보드가 늘면 병렬성이 자동으로 는다.
...
- [x] ~~오케스트레이션 주체·워커 풀 배치~~ — **확정 2026-08-07** (§2.0~§2.3).
      **오케스트레이터 2곳**(윈도우 Claude Code + 마스터), **워커 풀은 마스터가 인계**,
      **BC-250 은 순수 추론 노드로 유지**(병렬성은 보드 수로 얻는다).
...
### 4.2 루프 (2026-08-07 개정 — 워커 풀이 마스터로 이전)

```
사용자 ─지시─▶ 오케스트레이터 (윈도우 Claude Code 또는 마스터)
                  │ ① 작업 분해 → task_queue 에 잡 투입
                  ▼
       ┌──────────────────────────────────────────────┐
       │ task_queue (마스터)  · 능력 라우팅 requires:  │
       └───────┬──────────────────────────┬───────────┘
      claim(코드 생성 잡)          claim(requires: ue5)
               ▼                          ▼
  마스터의 워커 프로세스 N개          윈도우 작업 PC
   · RAG/온톨로지로 매니페스트 조립     · UE5 빌드 + RunTests  ← 층3
   · ② stateless 추론 요청 ─────▶ BC-250 추론 노드
   │                          ③ 결과 텍스트(diff/구현) 반환
   · ④ 층1 자기검증(동결 선언·펜스 잔재)
   · ⑤ 층2 상용 모델 문법·API 검사 (agy → Claude 폴백)
   · ⑥ 제출
               └──────────┬───────────────┘
                          ▼
        git `task/<task_id>` 브랜치에 커밋으로 누적
                → 다음 라운드가 읽어감
```

- **워커 프로세스가 마스터에서 돈다** — 파일 I/O(저장소 클론·코드 작성)도 여기서 일어난다.
  AgentTest 에서 팀원 윈도우 PC 들이 하던 일이다(§2.1).
- **BC-250 은 여전히 파일을 만지지 않는다** — 텍스트만 왕복(§2.2-1). 이건 안 바뀐다.
- **윈도우가 하는 일은 두 가지뿐** — 오케스트레이션(Claude Code)과 **층3 UE5 빌드/테스트**.
  후자는 `requires: ue5` 태그가 붙은 잡으로 큐에 들어가고 윈도우만 claim 한다(§5.2-C).
  ⚠️ **능력 기반 라우팅은 아직 없다** — `job_kind`(write/verify) 분기만 있다. §3 미결 항목.
...
- **마스터 브로커 확정**: BC-250 을 호출하는 것은 마스터뿐이다. 노드 라우팅·헬스체크·페일오버가
  ...
  **오케스트레이션(무엇을 할지)과 브로커(추론 중계)는 다른 층이다** — 오케스트레이터가 둘이어도
  브로커는 하나다.
...
| **`worker/` 전체** | **3,657줄** | 14줄 | 🔴 **2026-08-07 이전으로 변경** — 팀원 윈도우 PC 에서 돌던 워커 풀을 마스터가 인계(§2.1). `claim → write → submit` 루프·`validator`(층1)·`claude_spawn`(층2)·`heartbeat`·`git_ops`. **엔진 바운드 부분 없음** |
...
> **⚠️ `worker/` 전체가 B 에서 A 로 옮겨졌다 (2026-08-07).** 팀 환경에선 이게 "작업자(윈도우)
> 하네스 자체"라 윈도우 잔류가 당연했지만, 1인 클러스터에선 **워커 풀을 리눅스가 인계**한다(§2.1).
> 윈도우에 남는 것은 위 표의 **엔진 바운드 5종 + 사용자 접점 Claude Code** 뿐이다.
...
4. **`worker/` 이전 (2026-08-07 신규 단계)** — 워커 풀을 마스터가 인계(§2.1·§5.2-A).
   Windows 의존 14줄뿐이고 엔진 바운드 코드가 없다. 함께 필요한 것:
   `mcp/agy_query/`(층2 검증용, **`agy` 설치 선결**) + 능력 기반 라우팅(`requires: ue5`)이
   없으면 마스터 워커가 UE5 잡을 집어간다.
