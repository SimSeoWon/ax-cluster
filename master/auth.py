...
REQUESTER_SCOPE: tuple = (
    ...
    # [중요] **등재** (`#336` ④, 사용자 결정 2026-08-30). 요청자가 분산 작업을 등재하는
    #    자리다 — `ax-request` §3 은 오래 등재를 지시했는데 **요청자에게 그 경로가 없었다**
    #    (실측 2026-08-29: 클라 로스터에 `register_work` 0건 · 이 스코프에 등재 경로 없음 →
    #    403 이 정상 동작이었다). 절차서가 못 하는 것을 지시하던 `#297` 부류.
    # [중요] **경로가 `/api/v1/` 밖인 것이 의도다.** 8103 의 `/api/v1/*` 는 무인증 공개라
    #    (`webui.app.PREFIXES`) 거기 두면 **토큰 없이 등재가 열린다.** 그래서 `/work/` 로 뺐다.
    # [주의] 스코프는 **경로 단위**라 MCP(`/mcp`)를 통째로 여는 것과 다르다 — 그쪽을 열면
    #    온톨로지 쓰기·프로젝트 등록까지 딸려 온다(갈래 ⓐ를 버린 이유).
    ("POST", "/work/register"),
    # 🔴 **개선요청** (사용자 결정 2026-09-05) — 등재는 최초 전용이 됐고, 이미 있는
    #    분산 작업의 다음 라운드는 이 경로다. 같은 요청자가 같은 이유로 부른다.
    ("POST", "/work/improve"),
    # [중요] **컨벤션 읽기** (실측 2026-08-30 — `#336` ④ 와 **같은 결함이 하나 더 있었다**).
    #    작업장 `CLAUDE.md:515` 가 요청자에게 지시한다: *"작업을 시작하기 직전에 `find_recipes` ·
    #    `get_anti_patterns` · `find_history` 를 부른다(`#267`) … 도구가 있어도 안 부르면
    #    컨벤션 루프는 한 방향이다"*. 그런데 이 둘이 `WORKER_SCOPE` 에만 있어 **요청자는 403**
    #    이었다. `.33` 실측(2026-08-30): 첫 실전 요청에서 *"find_recipes·get_anti_patterns 는
    #    이 기계 토큰 스코프로 403"* 을 내고 **컨벤션·안티패턴 없이 설계를 냈다.**
    # [주의] `#267` 이 워커에 이 둘을 연 근거(*"코드를 추론하는 것은 워커이고 레시피·안티패턴은
    #    그 자리에서 필요하다"*)는 요청자에게도 그대로 성립한다 — **요청자도 코드를 쓴다**
    #    (직접 처리 경로 · 골조 작성). 읽기만 열고 쓰기 대행은 종전대로 큐가 한다.
    ("POST", "/api/v1/recipes/search"),
    ("GET", "/api/v1/anti_patterns"),
    # [중요] **「왜 아무것도 안 도나」를 요청자가 읽을 수 있어야 한다** (실측 2026-09-01 —
    #    `#336` ④ · recipes/anti_patterns 와 **같은 결함이 세 번째로** 났다). 세션 시작 훅
    #    (`pending_works.py`)이 `dispatch_hold` 를 읽어 「재개 대기」를 고지하도록 만들었는데
    #    이 경로가 스코프에 없어 **403 → 훅이 조용히 아무 말도 못 했다.** 절차서(여기서는 훅)가
    #    못 하는 것을 지시하던 `#297` 부류다.
    # [주의] 읽기 전용 집계 + 보류 사유뿐이다 — 본문·경로·비밀이 없다.
    ("GET", "/api/v1/status"),
)
...
WORKER_SCOPE: tuple = (
    ...
    ("GET", "/api/v1/tasks"),
    # [중요] **컨벤션 읽기** (`#267`, 사용자 결정 2026-08-23). 코드를 추론하는 것은 워커이고,
    #    레시피·안티패턴은 그 자리에서 필요하다 — 없으면 컨벤션 루프가 반 바퀴만 돈다.
    #    [주의] **읽기만 넣는다.** 쓰기 대행(시소러스·기록·레드마인)은 여기 없고, 그래서 워커가
    #    부르면 403 이 사유와 함께 난다 — κ.0 경계(판단은 마스터)를 토큰이 강제한다.
    ("POST", "/api/v1/recipes/search"),
    ("GET", "/api/v1/anti_patterns"),
    # [중요] **과거 결정 읽기도 같은 자리다** (`#306`, `#309` 감사에서 잡혔다). 도구는
    #    `PAYLOAD_BY_ROLE` 로 워커에 배달되고 배달 문서가 *"작업 시작 직전에 부른다"* 고
    #    지시하는데, 스코프에 없어 **403** 이었다(실측 2026-08-24: worker 403 / requester 200).
    #    [주의] **선언과 배선이 두 벌이면 조용히 갈린다** — `#261` 이 겪은 자리이고 한 세션
    #    안에서 같은 일이 재현됐다. 읽기 표면을 늘릴 때 이 표를 같이 보라.
