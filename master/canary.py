...
⚠️ **이 함수는 `events/consumer.py` 에 있었다.** 옮긴 이유는 층이다 — `context_search/rebuild`
...
## 🔴 승격 기준 — 원전이 except ~33개 중 딱 2개만 카나리로 올렸다
...
⚠️ **「전진하지 않는다」를 코드 한 줄로 판단하지 말 것.** 실측 2026-08-16(`#182` 이식):
...
🔴 즉 **다음 회차가 오지 않으면 self-heal 도 오지 않는다.** 리포트 17 §12 의 *"트윈은 자랐는데
...
# 🔴 원전 kind 식별자와 같은 뜻으로 쓴다 — 원전은 `context-md-lost` / `vector-index-gap` 이고,
