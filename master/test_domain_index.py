"""소 2.1.1~2.1.3 도메인 색인 — 페이로드 · 지문 · 🔴 노이즈 3중 차단.
...
def test_payload() -> None:
    with tempfile.TemporaryDirectory() as t:
        ...
        check("🔴 도메인명이 태그 채널에 들어간다", "MissionRuntime" in pl["tags"], str(pl["tags"]))
        check("objects 경로 → 클래스명(stem)",
              set(pl["related_classes"]) == {"UMissionTaskExecutor", "UMissionTaskBase"},
              str(pl["related_classes"]))
        # 🔴 이게 이 모듈의 핵심 — 항목 본문이 안 녹으면 액션·invariant 로 검색이 안 된다
        check("🔴 액션 본문이 body 에 녹는다", "태스크 한 건을 실행한다" in pl["body"])
        check("🔴 invariant 본문이 body 에 녹는다", "서버 권한에서만 등록된다" in pl["body"])
...
def test_fingerprint_and_sync() -> None:
    with tempfile.TemporaryDirectory() as t:
        ...
        again = di.sync(paths)
        check("🔴 안 바뀌면 건너뛴다", again.skipped_unchanged, again.summary)
        check("🔴 건너뛸 때 0 을 보고하지 않는다", again.synced == 2, again.summary)
...
def test_empty_is_an_error() -> None:
    with tempfile.TemporaryDirectory() as t:
        ...
        try:
            di.sync(paths)
            check("🔴 대상 0이면 예외", False, "예외가 안 났다")
        except di.DomainIndexError:
            check("🔴 빈 색인을 성공으로 보고하지 않는다", True)


def test_noise_gate() -> None:
    """🔴 이 절이 소 2.1.2 다 — 빼면 조용히 나빠진다."""
    with tempfile.TemporaryDirectory() as t:
        paths = _project(Path(t))
        ...
        check("🔴 ③ 개수 상한이 걸린다", len(many) <= 1, str(len(many)))

        # 액션 본문으로도 도메인이 잡혀야 한다 (payload 에 녹인 이유)
        check("🔴 액션 이름으로 도메인이 잡힌다",
