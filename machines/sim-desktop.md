...
있으면 **멈추고 묻는다.** 🔴 **갈림길만이 아니다 — 「작업 지시인지 단순 질문인지」도 애매하면
묻는다** (원전 `.agents/AGENTS.md` 3조 이식, 소 4.1.1). ⚠️ **짧은 수긍(`응`)이 가장 위험하다**:
앞에 물음이 없었으면 승인이 아니라 수긍일 수 있다 — 실측 2026-08-14, 한 세션에서 `응` 이 두 번
왔고 하나만 승인이었다.

🔴 **그리고 이 파일과 게임 소스는 고치기 전에 계획을 말하고 동의를 받는다** (원전 1조 이식,
범위는 사용자 결정 2026-08-14). 🔴 **이 파일은 매 세션 자동 로드되므로 한 줄이 모든 다음 세션의
행동을 바꾼다** — 되돌리는 비용이 가장 큰 자리다. 도구 코드는 커밋 승인 게이트가 그 역할을 한다.

그리고 **만들기 전에 원전(`~/AgentTest`)과 기존 자산을 읽는다** —
